...
class PopBox {
	...
	function getToolbarButton( ) {
		 return "<div style='float:left; '><input id='weave_button' type='image' src='" . wfGetPad('/extensions/PopBox/button_related.png') . "' accesskey='" .wfMsg('popbox_accesskey') ."' onclick='javascript:PopIt(document.editform.wpTextbox1)'></div>\n"  ;		
	}
...
<script language='javascript' type='text/javascript'>
	...
	function Revise() {
		...
		if (document.getElementById('wpTextbox1') && ( (agent.indexOf('firefox') >= 0) || (agent.indexOf('msie 8.0') >= 0) )) {
			document.getElementById('popbox_inner').innerHTML = '<input id=\"revise_text\" type=\"text\" name=\"revise\" onKeyUp=\"fakeSubmit(event);\" onclick=\"\" value=\"' + searchtext + '\"/><img src=\"" . wfGetPad('/images/a/a8/Search_button.png') . "\" onclick=\"return searchFormSubmit();\"><p id=\'nothanks\'><a href=\'javascript:PopIt()\'>". wfMsg('popbox_close') . "</a></p>';
		} else {
	...
	#popbox_hdr {
	    background: url(" . wfGetPad('/extensions/PopBox/WeaveLinks_hdr.gif') . ") 0 0 no-repeat;
	    font-size: 120%;
	    font-weight: bold;
	    width: 195px;
	    height: 44px;
