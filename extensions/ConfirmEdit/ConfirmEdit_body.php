...
class ConfirmEditHooks {
	...
	static function confirmAccountReminder( $u, &$message ) {
		return self::getInstance()->confirmAccountReminder( $u, $message );
	}
...
class SimpleCaptcha {
	...
	function triggerUserLogin( $user, $password, $retval ) {
		...
		if( $retval == LoginForm::WRONG_PASS && $wgCaptchaTriggers['badlogin'] ) {
			$key = wfMemcKey($this->badLoginKey());
	...
	/**
	 * Hook for user passwork/username reminder form submissions.
	 * @param User $u
	 * @param string $message
	 * @return bool true to continue, false to abort user creation
	 */
	function confirmAccountReminder( $u, &$message ) {
		global $wgCaptchaTriggers, $wgUser;
		if( $wgCaptchaTriggers['createaccount'] ) {
			if( $wgUser->isAllowed( 'skipcaptcha' ) ) {
				wfDebug( "ConfirmEdit: user group allows skipping captcha on account creation\n" );
	...
	function logPass($pass) {
        ...
        for ($i = 0; $i < sizeof($bt); $i++) {
       		$caller = $bt[$i]['file'];
        	if (strpos($bt[$i]['file'], "ConfirmEdit") === false 
				&& strpos($bt[$i]['file'], "Hooks") === false
				&& trim($caller) != ""){
        		$caller = $bt[$i]['file'];
        ...
        $dbw->insert('passcaptcha',
        	array(	
				'pc_pass'			=> ($pass ? 1 : 0),
				'pc_user'     		=> $wgUser->getID(),
      			'pc_user_text'  	=> $wgUser->getName(),
      			'pc_fullip'     	=> $fullip,
      			'pc_caller'     	=> $caller,
      			'pc_caller_linenum' => $line,
      			'pc_timestamp'  	=> wfTimestampNow(TS_MW)
      		)
       );
...
#echo "<br/><br/><b>storing with key " . wfMemcKey( 'captcha', $index ) . "<br/><br/></b>"; print_r($info);
...
##echo "<br/><br/><b>retrieving with key " . wfMemcKey( 'captcha', $index ) . wfBacktrace() . "<br/><br/></b>"; print_r($info);
		if( $info ) {
			return $info;
		} else {
			return false;
