...
class SpamBlacklist {
	...
	function getRegexes() {
		...
		if ( $this->regexes === false || $recache ) {
			if ( !$recache ) {
				$this->regexes = $wgMemc->get( wfMemcKey("spam_blacklist_regexes") );
			}
			if ( $this->regexes === false || $this->regexes === null ) {
				...
				foreach ( $this->files as $fileName ) {
					if ( preg_match( '/^DB: ([\w-]*) (.*)$/', $fileName, $matches ) ) {
						...
						# conditions.
						$key = wfMemcKey("spam_blacklist_file:$fileName");
						$warningKey = wfMemcKey("$wgDBname:spamfilewarning:$fileName");
				...
				$wgMemc->set( wfMemcKey("spam_blacklist_regexes"), $this->regexes, $this->expiryTime );
			} else {
		...
		if ( is_array( $regexes ) ) {
			...
			foreach( $regexes as $regex ) {
				if ( preg_match( $regex, $links, $matches ) ) {
					wfDebug( "Match!\n" );
					wfRunHooks( 'SpamBlacklistFoundSpam', array($matches[0]));
