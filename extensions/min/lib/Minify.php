...
class Minify {
    ...
    public static function serve($controller, $options = array())
    {
        ...
        $headers['Content-Length'] = $cacheIsReady
            ? $cacheContentLength
            : strlen($content);
        $headers['Content-Type'] = self::$_options['contentTypeCharset'] && false
        ...
        if (! self::$_options['quiet']) {
            ...
            //header('X-Vary-Options: Cookie;string-contains=wiki_sharedUserID;string-contains=wiki_shared_session,Accept-Encoding;list-contains=gzip');
    ...
    protected static function _combineMinify()
    {
        ...
        //XXCHANGED
        if (self::$_options['contentType'] === self::TYPE_CSS) {
            $content = self::_addCssCdn($content);
        }
        return $content;
    }
    ...
    //XXCHANGED
    /**
     * Added by Reuben -- insert padx.whstatic.com URLs into CSS for
     * better browser downloading parellelization.
     * @param string $css the bundle of CSS to modify
     */
    protected static function _addCssCdn($css)
    {
        // A big hack so we don't have to re-define wfGetPad()
        global $wgServer;
        $wgServer = 'http://'.@$_SERVER['SERVER_NAME'];
        define('MEDIAWIKI', true);
        require_once('../../includes/GlobalFunctions.php');

        return preg_replace('@url\((/[^)]*)\)@e', "'url(' . wfGetPad('\\1') . ')'", $css);
    }
}
