<?
class YouTubeAuthSub extends UnlistedSpecialPage {

    function __construct() {
        UnlistedSpecialPage::UnlistedSpecialPage( 'YouTubeAuthSub' );
    }

	
	function post ($url, $content, $headers = null) {
	...
	function execute($par) {	
		...
		if ($wgYTAS_UseClientLogin) {
			...
			if (!$token) {
				$result = $this->post("https://www.google.com/youtube/accounts/ClientLogin?"
	...
	";
			$headers = array (
					...
					"X-GData-Client: {$wgYTAS_ClientId}",
