...
class DraftHooks {
	...
	public static function discard( &$article, &$user, &$text, &$summary, &$m, &$watchthis, &$section) {
		global $wgRequest;
	...
	public static function loadForm( &$editpage ) {
		...
		if ( $numDrafts  > 0 ) {
			if ( $wgRequest->getText( 'action' ) !== 'submit' ) {
				...
				$link = Xml::element( 'a',
					array(
						'href' => $wgTitle->getFullURL( "action=edit" . $adv ),
	...
	public static function controls( &$editpage, &$buttons ) {
		...
		if ( $wgUser->isAllowed( 'edit' ) && $wgUser->isLoggedIn() ) {
			...
			$buttonAttribs = array(
            	...
            	'class'         => 'button white_button_100 submit_button disabled ',
            	'onmouseout'    => 'button_unswap(this);',
            	'onmouseover'   => 'button_swap(this);',
            	'style'         => 'button_swap(this);" style="float: left;',
			...
			$ajaxButton = Xml::escapeJsString(
				Xml::element( 'input',
					...
					+ ( $wgRequest->getText( 'action' ) !== 'submit' ?
						array ( 'disabled' => 'disabled' ,'class' => 'disabled')
						: array()
					)
				)
			);
	...
	public static function addJS( $out ) {
		global $wgScriptPath, $wgJsMimeType, $wgDraftsStyleVersion, $wgRequest;
		...
		switch ($action) {
			...
			case 'submit':
				...
				$out->addScript( "<script type=\"$wgJsMimeType\" src=\"" . wfGetPad('/extensions/min/f/extensions/Drafts/Drafts.js?rev=') . WH_SITEREV . "\"></script>\n" );
			break;
		}
	...
	public static function addCSS( $out ) {
		global $wgScriptPath, $wgDraftsStyleVersion, $wgRequest;
            ...
            case 'submit':
				$out->addLink(
					array(
						'rel' => 'stylesheet',
						'type' => 'text/css',
						'href' => wfGetPad("/extensions/min/f/extensions/Drafts/Drafts.css?rev=") . WH_SITEREV,
				)
			);
		}
