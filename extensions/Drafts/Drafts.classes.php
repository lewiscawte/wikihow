...
class Draft {
	...
	private function load() {
		...
		$this->_htmlfive= $row['draft_htmlfive'];
	...
	public function save() {
		...
		$data = array(
			...
			'draft_htmlfive' => $this->_htmlfive ? 1 : 0,
	...
	public static function newFromRow( $row ) {
		...
		$draft->setHtml5( $row['draft_htmlfive'] );
	...
	public static function listDrafts( &$title = null, $user = null ) {
		...
		//XXADDED
		$adv = $wgRequest->getVal('guidededitor') == true ? "" : "&advanced=true";
			...
			foreach ( $drafts as $draft ) {
				...
				$wgOut->addHTML(
					Xml::element( 'td',
						...
						$wgLang->timeanddate( $draft->getSaveTime(), true)
					)
				);
				...
				$wgOut->addHTML(
					Xml::element( 'a',
						array(
							'href' => $urlDiscard,
							'onclick' => "if( true || !wgAjaxSaveDraft.insync ) return confirm('" . Xml::escapeJsString( wfMsgHTML( 'drafts-discard-warn', $draft->getTitle()->getText() ) ) . "')"
						),
	...
	public function getHtml5() {
		return $this->_htmlfive;
	}
	public function setHtml5( $htmlfive) {
		$this->_htmlfive = $htmlfive;
	}
