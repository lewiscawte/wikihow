 ...
 */
 ...
 class IheartwikiHow extends UnlistedSpecialPage{
	...
	function execute($par){
		...
		if(isset($target) && $target != ''){
			...
			$sql = "INSERT INTO `badge_views` (bv_referrer, bv_size, bv_count) VALUES ('" . $_SERVER['HTTP_REFERER'] . "', " . $target . ", 1) ON DUPLICATE KEY UPDATE bv_count = bv_count + 1";
			$dbr->query($sql);
 ...
 class IheartwikiHowAdmin extends UnlistedSpecialPage{
	...
	function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage( 'IheartwikiHowAdmin' );
	}
	...
	function execute($par){
		...
		while($row = $dbr->fetchObject( $res )){
			if($i % 2 == 0)
				...
				$style = "";
			$wgOut->addHTML("<tr><td style='padding:5px 0;$style'>$row->bv_referrer</td><td style='padding:5px 0;$style'>$row->bv_count</td></tr>");
			$i++;
		}
