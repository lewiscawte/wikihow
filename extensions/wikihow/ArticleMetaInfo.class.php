...
class ArticleMetaInfo {
	...
	/**
	 * Pass in $dbkey only if we have it available.
	 */
	/*public function __construct($namespace, $articleID, $dbkey = '') {
		$this->namespace = $namespace;
		$this->articleID = $articleID;
		$this->dbkey = $dbkey;
		$this->cachekey = wfMemcKey('md-' . $this->articleID);
	}*/
...
print_r($m);exit;
			$imgTitle = Title::newFromText($m[2], NS_IMAGE);
			if ($imgTitle) {
	 ...
	 *
	 CREATE TABLE article_meta_info (
	   ...
	   ami_img varchar(255) not null default '',
	   primary key (ami_id)
