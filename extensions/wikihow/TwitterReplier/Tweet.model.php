...
class TweetModel
{
	...
	function getTweets( $lastTweetId = null )
	{
		...
		if( !empty( $lastTweetId ) ) {
			$sql .= ' AND tweet_id > ' . $this->dbw->addQuotes( $lastTweetId );
		}
	...
	function insertTweet( $tweet )
	{
		...
		if ( !empty( $tweet ) && is_object( $tweet ) ) {
			...
			try {
				$resp = $this->dbw->insert( self::TWEETS_TABLE, $field_values, 'Database::insert', array( 'IGNORE' ) );

				// check if we actually inseted it
				if ( $resp ) {
	...
	function insertReplyTweet( $tweet, $replyStatusId )
	{
		...
		if ( !empty( $tweet ) && is_array( $tweet ) ) {
			...
			try {
				$resp = $this->dbw->insert( self::REPLY_TABLE, $field_values );
				// check if we actually inseted it
				if ( $resp ) {
					$affectedRows = $this->dbw->affectedRows();
					$resp = $affected > 0 ? true : false;
