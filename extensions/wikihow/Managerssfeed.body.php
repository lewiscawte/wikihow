...
class Managerssfeed extends SpecialPage {
	function __construct() {
        SpecialPage::SpecialPage( 'Managerssfeed');
    }

	function getButtons($index, $approved) {
		$s = "<input type='image' src='/extensions/wikihow/arrow-up.png' height='16px' onclick='moveUp($index);'> 
			<input type='image' src='/extensions/wikihow/arrow-down.png' height='16px' onclick='moveDown($index);'>
			<input type='image' src='/extensions/wikihow/edit-icon.png' height='16px' onclick='renameTitle($index);'>
			<input type='image'id='minus_{$index}' onclick='removefeeditem({$index});' src='/extensions/wikihow/minus.png' height='16px'/>
			<img id='check_{$index}' src='/extensions/wikihow/CheckMark" . (!$approved ? "Grey" : "") . ".png' height='16px' onclick='approve($index);'/>
	";
		if ($approved) 
	...
	function execute($par) {
		...
		$wgOut->addHTML("  <script type=\"text/javascript\" src=\"/skins/common/ac.js\"></script> ");
		$wgOut->addHTML("  <script type=\"text/javascript\" src=\"/extensions/wikihow/managerssfeed.js\"></script> ");
		$wgOut->addHTML('<link rel="stylesheet" type="text/css" href="/extensions/wikihow/managerssfeed.css" />');
		...
		while (gmdate("n", $now) == $month) {
			...
			$wgOut->addHTML("<tr class='day'>
				...
				<table class='addicon'><tr><td>	<input type='image' src='/extensions/wikihow/plus.png' height='24px' onclick='add($d)'/></tr></td></table>
				</td>
				<td class='feeditem'>
