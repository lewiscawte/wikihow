...
class ListRequestedTopics extends SpecialPage {
	...
	if($key){
	    ...
	    $wgOut->addHTML("<table width='100%' class='suggested_titles_list' cellspacing='0' cellpadding='0'>");
	    $wgOut->addHTML("<tr class='st_top_row'><td class='st_icon'><img src='" . $this->getCategoryImage($category) . "' alt='{$catString}' /></td><td class='st_title'><strong>{$catString}</strong> Requests</td><td>Requested By</td></tr>");
	}
	else{
	    $wgOut->addHTML($this->getCategoryBox());
	    $wgOut->addHTML("<table width='100%' class='suggested_titles_list' cellspacing='0' cellpadding='0'>");
	...
	function getTabs($section) {
	    ...
	    $tabs .= '<a href="/Special:ListTopicCategories" onmousedown="button_click(this);" class="' . $topic . '">Find a Topic</a>';
...
class RecommendedArticles extends SpecialPage{
	...
	function getTopLevelSuggestions($map, $cats) {
		...
		$s .= "</p><a href='/Special:CreatePage/{$suggests[0]->getPartialURL()}' onmouseout='button_unswap(this);' onmouseover='button_swap(this);' class='button button52'>Write</a>
		<td class='requestor'>";
    ...
    function execute($par){
		...
		foreach ($cats as $key) {
			$cat = $map[$key];
			$wgOut->addHTML("<table width='100%' class='suggested_titles_list' cellspacing='0' cellpadding='0'>");
	    	$wgOut->addHTML("<tr class='st_top_row'><td class='st_icon'><img src='" . ListRequestedTopics::getCategoryImage($cat) . "' alt='{$cat}' /></td><td class='st_title'><strong>{$cat}</strong> Requests</td><td>Requested By</td></tr>");
...
class YourArticles extends SpecialPage{
    ...
    function execute($par){
		...
		foreach ($articles as $cat=>$article_array) {
	    	if ($top) {
				$wgOut->addHTML("<tr class='st_top_row'><td colspan='2' class='your_cat_top'><img src='" . ListRequestedTopics::getCategoryImage($cat) . "' alt='{$cat}' />{$cat}</td><td class='header_views'>Number of Views</td></tr>");
				$top = false;
			} else {
			...
			foreach ($article_array as $row) {
				...
				$wgOut->addHTML("<tr><td class='article_image'></td>"
