    ...
    function execute ($par) {
		...
		if(!$wgRequest->getVal('st_search') && !$wgRequest->getVal('category')){
			...
			if($key){
				...
				$wgOut->addHTML("<table width='100%' class='suggested_titles_list' cellspacing='0' cellpadding='0'>");
				$wgOut->addHTML("<tr class='st_top_row'><td class='st_icon first'>" . $this->getCategoryImage($category) . "</td><td class='st_title second'><strong>{$catString}</strong> Requests</td><td class='third'>Requested By</td></tr>");
			}
			else{
				$wgOut->addHTML($this->getCategoryBox());
				$wgOut->addHTML("<table width='100%' class='suggested_titles_list' cellspacing='0' cellpadding='0'>");
...
class RecommendedArticles extends SpecialPage{
	...
	function getTopLevelSuggestions($map, $cats) {
		...
		<td style='width:260px' class='category'>{$catresults[1]}</td>
		...
		if(count($cats) > 0){
			foreach ($cats as $key) {
				...
				if($dbr->numRows($res) > 0){
					...
					$suggestions .= "<tr class='st_top_row'><td class='st_icon first'>" . ListRequestedTopics::getCategoryImage($cat) . "</td><td class='st_title second'><strong>{$cat}</strong> Requests</td><td class='third'>Requested By</td></tr>";
					while ($row = $dbr->fetchObject($res)) {
						$t = Title::makeTitle(NS_MAIN, $row->st_title);
						$user = "Anonymous";
						if ($row->st_user > 0) {
						...
						$suggestions .= "<tr><td class='st_write'><a href='/Special:CreatePage/{$t->getPartialURL()}'>Write</td><td class='st_title'>{$t->getText()}</td><td class='st_requestor'>{$user}</td></tr>";
...
class SuggestCategories extends SpecialPage {
    ...
    function execute($par){
		...
		foreach ($catmap  as $cat) {
			...
			$wgOut->addHTML("<td id='{$safekey}' class='{$c}'><div style='position:relative'><a onclick='choose_cat(\"{$key}\");' class='categorylink'><input type='checkbox' id='check_{$safekey}' {$s} />" .  ListRequestedTopics::getCategoryImage($cat) . "<br />{$cat}</a></div></td>");
			$index++;
			if ($index % 5 == 0)
	    		$wgOut->addHTML("</tr><tr>");
