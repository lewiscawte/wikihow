...
class ListRequestedTopics extends SpecialPage {
    ...
    function execute ($par) {
		...
		if(!$wgRequest->getVal('st_search') && !$wgRequest->getVal('category')){
			...
			if ($wgRequest->getVal('st_search') != "all") {
			...
			if($key){
				...
				$wgOut->addHTML("<tr class='st_top_row'><td class='first'></td><td class='st_title second'>Topics for <strong>\"{$key}\"</strong></td><td class='third'>Requested By</td></tr>");
			}
			else if($category != ""){
				...
				$wgOut->addHTML("<table width='100%' class='suggested_titles_list' cellspacing='0' cellpadding='0'>");
				$wgOut->addHTML("<tr class='st_top_row'><td class='st_icon first'><img src='" . $this->getCategoryImage($category) . "' alt='{$catString}' /></td><td class='st_title second'><strong>{$catString}</strong> Requests</td><td class='third'>Requested By</td></tr>");
			}
			else{
				$wgOut->addHTML($this->getCategoryBox());
				$wgOut->addHTML("<table width='100%' class='suggested_titles_list' cellspacing='0' cellpadding='0'>");
	...
	function getCategoryBox($categoryName = ""){
		...
		$catBox = '<div class="st_box2">';
		 $catBox .= '<p>' . $categoryName . '<br /><a href="/Special:ListRequestedTopics">&larr; Show all categories</a></p>
	...
	function getTabs($section) {
	    ...
	    $tabs .= '<a href="/Special:SuggestCategories" onmousedown="button_click(this);" class="' . $cats . '">Categories</a>';
		$request = $sk->makeLinkObj(Title::makeTitle(NS_SPECIAL, 'RequestTopic'), wfMsg('requesttopic'));
		$request = preg_replace('@^<a@', '<a class="notab" style="float:right; margin-right:10px; font-size:1em; width:auto;"', $request);
		$tabs .= $request;
...
class RecommendedArticles extends SpecialPage{
	...
	function getTopLevelSuggestions($map, $cats) {
		...
		<tr><td style='width:260px' class='category'>{$map[$cat1]}</td>
		<td rowspan='3' class='middle'>OR</td>
		<td style='width:260px' class='category'>{$map[$cat2]}</td>
    ...
    function execute($par){
		...
		if($wgRequest->getVal('surprise') == 1 || sizeof($cats) == 0)
		...
		if($wgRequest->getVal('surprise') == 1 || sizeof($cats) == 0)
			$wgOut->addHTML("<hr style='border-top:1px solid #cccccc; margin-top:15px;' />");
			//$wgOut->addHTML("<a onclick='reloadTopRow();' id='tryagain'>Nope, Try Again</a>");

		foreach ($cats as $key) {
			$cat = $map[$key];
			$wgOut->addHTML("<table width='100%' class='suggested_titles_list' cellspacing='0' cellpadding='0'>");
	    	$wgOut->addHTML("<tr class='st_top_row'><td class='st_icon first'><img src='" . ListRequestedTopics::getCategoryImage($cat) . "' alt='{$cat}' /></td><td class='st_title second'><strong>{$cat}</strong> Requests</td><td class='third'>Requested By</td></tr>");
			...
			$wgOut->addHTML("<tr><td class='st_write'><a href='/Special:CreatePage/{$t->getPartialURL()}'>Write</td><td class='st_title'>{$t->getText()}</td><td class='st_requestor'>{$user}</td></tr>");
    ...
    function execute($par){
       ...
       /* if ( $wgUser->getID() == 0 ) {
		...
		$res = $dbr->query("select * from firstedit left join page on fe_page=page_id
				left join suggested_titles on page_title=st_title and page_namespace= 0 where fe_user={$wgUser->getID()} and st_id is not null group by st_category");
		...
		foreach ($articles as $cat=>$article_array) {
	    	if ($top) {
				$wgOut->addHTML("<tr class='st_top_row'><td colspan='2' class='your_cat_top first'><img src='" . ListRequestedTopics::getCategoryImage($cat) . "' alt='{$cat}' />{$cat}</td><td class='header_views third'>Number of Views</td></tr>");
				$top = false;
			} else {
				$wgOut->addHTML("<tr class='st_middle_row'><td colspan='2' class='your_cat_top'><img src='" . ListRequestedTopics::getCategoryImage($cat) . "' alt='{$cat}' />{$cat}</td><td class='header_views'>Number of Views</td></tr>");
			}
...
class SuggestCategories extends SpecialPage {
	...
	function getSubscribedCats() {
		global $wgUser;
		$dbr = wfGetDB(DB_SLAVE);
		$field = $dbr->selectField('suggest_cats', array('sc_cats'), array('sc_user'=>$wgUser->getID()));
		if ($field) {
    ...
    function execute($par){
		...
		ListRequestedTopics::addTabs('SuggestCategories');
		...
		$wgOut->addHTML(wfMsg('SuggestCategories_header'));

		$catmap = self::getCatMap();
		$cats =self::getSubscribedCats();

		// get top categories
		$wgOut->addHTML("<form method='post' action='/Special:SuggestCategories' name='suggest_cats'><input type='hidden' name='cats' value='" . implode($cats, ",") . "'/>");
		$wgOut->addHTML("<table width='100%' class='categorytopics'><tr>");
		$index = 0;
		foreach ($catmap  as $cat) {
			$key = strtolower(str_replace(" ", "-", $cat));
			$c = $cats && in_array($key, $cats) ? "chosen" : "not_chosen";
			//$wgOut->addHTML("<td id='{$key}' class='st_icon {$c}'><img onclick='choose_cat(\"{$key}\");' src='" . ListRequestedTopics::getCategoryImage($cat) . "' alt='$cat' /><br/><a onclick='choose_cat(\"{$key}\")';>$cat</a></td>");
			$wgOut->addHTML("<td id='{$key}' class='{$c}'><a onclick='choose_cat(\"{$key}\");' class='categorylink'><img src='" .  ListRequestedTopics::getCategoryImage($cat) . "'/><br />{$cat}</a></td>");
			$index++;
			if ($index % 4 == 0)
	    		$wgOut->addHTML("</tr><tr>");

		}
		$wgOut->addHTML("</tr></table><br/><br/>" . '<a onmouseout="button_unswap(this);" onmouseover="button_swap(this);" style="float: right; background-position: 0% 0pt;" class="button button136" onclick="document.suggest_cats.submit();">' . wfMsg('save') . '</a></form>');
