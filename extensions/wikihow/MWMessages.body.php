...
class MWMessages extends UnlistedSpecialPage {
    ...
    function __construct() {
        UnlistedSpecialPage::UnlistedSpecialPage( 'MWMessages' );
    }
	...
	function execute($par) {
    	...
    	$wgOut->addScript('<link rel="stylesheet" href="/extensions/wikihow/mwmessages.css" type="text/css" />');
    	$wgOut->addHTML('<link rel="stylesheet" href="/skins/WikiHow/popupEdit.css" type="text/css" />');
		$wgOut->addScript('<script type="text/javascript" src="/skins/WikiHow/popupEdit.js"></script>');
			...
			if ($search) {
				...
				foreach ($sortedArray as $key=>$val) {
					...
					if (stripos($val, $search) !== false) {
						...
						$qe_url = '<a href="#" onclick="initPopupEdit(\'/Special:Newarticleboost?type=editform&target=' . $t->getPrefixedURL() . '\') ;">' . $key .'</a>';
				...
				while ($row = $dbr->fetchObject($res)) {
					...
					$val = $r->getText();
					if (stripos($val, $search) !== false) {
						$qe_url = '<a href="#" onclick="initPopupEdit(\'/Special:Newarticleboost?type=editform&target=' . $t->getPrefixedURL() . '\') ;">' . $row->page_title .'</a>';
						$wgOut->addHTML("<tr><td class='mw_lang'>{$lang}</td><td class='mw_key'>{$qe_url}</td><td class='mw_val'>" . htmlspecialchars($val) ."</td></tr>");
					}
				}
				...
				foreach ($messages as $lang=>$arrs) {
					foreach ($arrs as $key=>$val) {
						...
						if ($newval != "&lt;{$key}&gt;")
							$val = $newval;
						$t = Title::makeTitle(NS_MEDIAWIKI, $key);
						$qe_url = '<a href="#" onclick="initPopupEdit(\'/Special:Newarticleboost?type=editform&target=' . $t->getPrefixedURL() . '\'); setText(' . $index . ');">' . $key .'</a>';
...
class MWMessagesDecode extends UnlistedSpecialPage {
    ...
    function __construct() {
        UnlistedSpecialPage::UnlistedSpecialPage( 'MWMessagesDecode' );
    }
