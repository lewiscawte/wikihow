...
$.fn.extend({
  ...
  $.fn.sqlquerybuilder = function(options) {
  	$.fn.sqlquerybuilder.defaults = {
		...
		columntitle:'I want these data: (defaults to all columns)',
		addnewcolumn:'[Click to a add new column]',
		showcolumn:true,
		wheretitle:'With the following conditions:',
		...
		operators:[
		 ...
		 {name:'bigger or equal to',op:"%f>='%s'",multipleval:false},
		 {name:'smaller than',op:"%f<'%s'",multipleval:false},
		 {name:'smaller or equal to',op:"%f<='%s'",multipleval:false},
	  ...
	  if(opts.presetlistdiv && opts.reporthandler){
        ...
        $.ajax({    
            ...
            success: function(data) {
              ...
              //alert($(opts.presetlistdiv).find('ul.treeview li.list').html());
               ...
               /*************************************************************************************************************/ 
				...
				$(".addnewsqlcolumn,.addnewsqlsort,.addnewsqlgroup").sqlsimplemenu({
				    ...
				    var sqlline=
				                 ...
				                 '<a class="addnewsql'+menutype+'" id='+(opts.counters[countertype])+' href="#'+action+'">'+opts.fields[action].name+'</a>'+ (countertype==2?'':'&nbsp;labeled as&nbsp;')+
