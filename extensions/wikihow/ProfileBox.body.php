...
<script language='javascript'>
	...
	var pbfavs_check = false;
		...
		if ($u->getOption('profilebox_startedEdited') == 1) {
			...
			$display .= "<tbody>" . ProfileBox::fetchCreated("User:" . $wgTitle->getPartialURL(), 5) . "</tbody>";
			...
			$display .= "
				<table class='pb-articles' id='pb-edited' cellspacing='0' cellpadding='0'>
					...
					<tr>
						...
						<th class='middle pb-feature'>Featured</th>
						<th class='last pb-view'>Views</th></tr></thead>";
			$display .= "<tbody>" . ProfileBox::fetchEdited("User:" . $wgTitle->getPartialURL(), 5) . "</tbody>";
...
*/
	...
	function fetchCreated($pagename, $limit = '') {
		...
		// DB CALL
		$res = self::fetchCreatedData($t->getText(), $limit);
	...
	function fetchEdited($pagename, $limit = '') {
		...
		// DB CALL
		$res = self::fetchEditedData($t->getText(), $limit);
