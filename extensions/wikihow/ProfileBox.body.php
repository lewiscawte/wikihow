...
class ProfileBox extends UnlistedSpecialPage {
	...
	function getStatsDispalay($stats, $username){
		...
		$display .= "
		<div id='profileBoxStats' class='pb-stats' style='display:block;'>";
		...
		$display .= "
		<span><strong>My wikiHow Stats:</strong></span>
		<table id='profileBoxStatsContent'>";

		$contribsPage = SpecialPage::getTitleFor( 'Contributions', $username );

		$display .= "<tr>";
		$display .= "<td>" . $stats['created'] . " " . $sk->makeKnownLinkObj( $contribsPage , 'Articles Started' ) . "</td>";
		$display .= "<td>" . $stats['edited'] . " Article Edits</a></td>";
		$display .= "</tr><tr>";
		$display .= "<td>" . $stats['patrolled'] . " <a href='/Special:Log&type=patrol&user=" . $username . "'>Edits Patrolled</a></td>";
		$display .= "<td>" . $stats['viewership'] . " Views</td>";
...
<script language='javascript'>
	...
	var pbfavs_check = false;
		...
		if ($u->getOption('profilebox_startedEdited') == 1) {
			...
			$display .= "<tbody>" . ProfileBox::fetchCreated("User:" . $wgTitle->getText(), 5) . "</tbody>";
			$display .= "<tfoot><tr><td class='pb-title'><a href='#' id='created_more' onclick='pbShow_articlesCreated(\"more\"); return false;'>View most recent 100 &raquo;</a><a href='#' id='created_less' style='display:none;' onClick='pbShow_articlesCreated(); return false;'><< View Less</a></td><td colspan='3' class='pb-view'>&nbsp;</td></tr>";
...
*/
			if (class_exists('ThumbsUp')) {
				...
				$display .= "<tbody>" . ProfileBox::fetchThumbed("User:" . $wgTitle->getPartialURL(), 6) . "</tbody>";
				$display .= "<tfoot><tr><td class='pb-title'><a href='#' id='thumbed_more' onclick='pbShow_Thumbed(\"more\"); return false;'>View most recent 100 &raquo;</a><a href='#' id='thumbed_less' style='display:none;' onClick='pbShow_Thumbed(); return false;'><< View Less</a></td><td colspan='1' class='pb-view'>&nbsp;</td></tr>";
...
*/
	...
	function fetchStats($pagename) {
		...
		if ($updateflag) {
			...
			$viewership = 0;
			$vsql = "select sum(distinct(page_counter)) as viewership from page,revision where page_namespace=0 and page_id=rev_page and rev_user=".$u->getID()." and rev_user_text='". mysql_real_escape_string($u->getName()) ."'";
		...
		if( in_array('newarticlepatrol', $rights ) )
			...
			$response['nab'] = 0;
		$resFA = $dbr->select(array('firstedit', 'templatelinks'), '*', array('fe_page=tl_from', 'fe_user' => $u->getID(), ('tl_title = "Fa" OR tl_title = "FA"') ) );
		$resRS = $dbr->select(array('firstedit', 'pagelist'), '*', array('fe_page=pl_page', 'fe_user' => $u->getID() ) );
	...
	function fetchThumbed($pagename, $limit = 5) {
		global $wgMemc, $wgUser;

		$pagename = urldecode($pagename);
		$cacheKey = wfMemcKey("pb-$pagename-$limit");
		...
		if (!$profileOwner && $result) {
			...
			$dbr = wfGetDB(DB_SLAVE);
			$t = Title::newFromText($pagename);
			$res = self::fetchThumbsData($t->getText(), $limit);
			//$topRes = self::fetchTopEditData($t->getText());
			$result = self::getThumbsTableHtml($topRes, $res, $dbr);

			$wgMemc->set($cacheKey, $result, 60*10);
	...
	function getThumbsTableHtml(&$topRes, &$dbRes, &$dbr) {
		$html = "";
		...
		while ($row = $dbr->fetchObject($dbRes)) {
			$t = Title::makeTitle($row->page_namespace, $row->page_title);
			if ($t->getArticleID() > 0 && $topRevId != $row->thumb_rev_id)  {
				$html .= self::getThumbsRowHtml($t, $row);
			}
		}
		...
		if (strlen($html) == 0) {
			$html .= "<tr><td class='pb-title'>No thumbed up edits yet</td><td class='pb-view'>&nbsp;</td></tr>";
		}
		return $html;
	}
	...
	function getThumbsRowHtml(&$t, &$row) {
		...
		$diff = $sk->makeKnownLinkObj($t, date('m/d/y', wfTimestamp(TS_UNIX, $row->rev_timestamp)),'diff=' . $row->thumb_rev_id . '&oldid=PREV');
	...
	function getTableHtml(&$dbRes, &$dbr){
		$html = "";
		while ($row=$dbr->fetchObject($dbRes)) {
	...
	function fetchEdited($pagename, $limit = '') {
		...
		// DB CALL
		$res = self::fetchEditedData(mysql_real_escape_string($pagename), $limit);
	...
	function execute ($par ) {
		...
		if ($type == 'display') {
			...
			if (($element != '') && ($pagename != '')) {
				switch($element) {
					case 'thumbed':
						echo $this->fetchThumbed($pagename, 6);
						break;
					case 'thumbedall':
						echo $this->fetchThumbed($pagename, 101);
						break;
					case 'stats':
						echo $this->fetchStats($pagename);
						break;
					case 'created':
						echo $this->fetchCreated($pagename, 5);
						break;
					case 'createdall':
						echo $this->fetchCreated($pagename, 100);
						break;
					case 'edited':
	...
	static function getPageTop($u){
		...
		if ($u->getOption('profilebox_display'))  {
			...
			if ($u->getID() == $wgUser->getID())   {
				$pbDate .= "<span style='float:right'><a href='/Special:Profilebox' id='gatProfileEditButton' >Edit $profilebox_name</a> | <a href='#' onclick='removeUserPage();'>Remove $profilebox_name</a></span>";
			}
			$pbDate .= "</div>";
	...
	static function getMemberLength($joinDate){
		...
		if($difference < 1){ //today
			...
			return "for over " . $difference . " " . ($difference>1?$periods[0]:$period[0]);
		}
		else{
			$difference = round($difference/7);
			if($difference < 4){ //this month (weeks)
				return "for over " . $difference . " " . ($difference>1?$periods[1]:$period[1]);
			}
			else{
				$difference = round($difference/4.35);
				if($difference < 12){ //this year (months)
					return "for over " . $difference . " " . ($difference>1?$periods[2]:$period[2]);
				}
				else{
					$years = floor($difference/12);
					$months = $difference % 12;

					return "for over " . $years . " " . ($years>1?$periods[3]:$period[3]) . " " . $months . " " . ($months>1?$periods[2]:$period[2]);
				}
			}
