...
$wgSpecialPages['LogHeight'] = 'LogHeight';
...
$wgAutoloadClasses['LogHeight'] = dirname( __FILE__ ) . '/CreatePage.body.php';
...
function wfLogheightforpage() {
	global $wgTitle, $wgUser;
	$ret = "";

	if (!$wgTitle || $wgTitle->getNamespace() != NS_MAIN || $wgUser->getID() > 0) 
		return $ret;

	$dbr = wfGetDB(DB_SLAVE);
	$ts = substr(wfTimestampNow(TS_MW), 0, 8) . "000000";
	$num_today = $dbr->selectField('pageheight', array('count(*)'), 
		array('ph_id' => $wgTitle->getArticleID(), 'ph_timestamp>' . $dbr->addQuotes($ts) )
		);
	if ($num_today == 0) {
		$ret = "<script type='text/javascript' src='/extensions/wikihow/pageheight.js'></script>";
	}
	return $ret;
}

function wfGetPageHeightForArticle ($t) {
	if (!$t) return -1;
	$dbr = wfGetDB(DB_SLAVE);
	$res = $dbr->query("select ph_height from pageheight where ph_id={$t->getArticleID()} order by ph_timestamp desc limit 1;");
	if ($row = $dbr->fetchObject($res)) 
		return $row->ph_height;
	else
		return -1;
}
...
function wfGetSuggestedTitles($t) {
    ...
    $key = "suggested_titles_:" . $t->getArticleID();
    if ( $wgMemc->get($key)) {
	...
	try {
		...
		if (
			...
			$wgOut->addHTML('<script type="text/javascript" language="javascript" src="/extensions/wikihow/winpop.js"></script>
								<script type="text/javascript" language="javascript" src="/extensions/wikihow/authoremails.js"></script>
					...
					<script type="text/javascript">
						var whNewLoadFunc = function() {
							if ( getCookie("aen_dialog_check") != "" ) {
							    jQuery("#dialog-box").load("/Special:CreatepageFinished");
