...
function avatarReset() {
	...
	imgblock += "                  <img src=\""+ imgServer + imgIn +"/"+wgUserID+".jpg' id='avatarIn' />\n";
	imgblock += "               </div> \n";
...
function dynLoadPrototype() {
	...
	proto.setAttribute("src", "/extensions/wikihow/cropper/lib/prototype.js");
	...
	scriptac.setAttribute("src", "/extensions/wikihow/cropper/lib/scriptaculous.js?load=builder,dragdrop");
	...
	cropper.setAttribute("src", "/extensions/wikihow/cropper/cropper.js");
	document.getElementsByTagName("head")[0].appendChild(cropper); 

  var fileref=document.createElement("link")
  fileref.setAttribute("rel", "stylesheet")
  fileref.setAttribute("type", "text/css")
  fileref.setAttribute("href", "/extensions/wikihow/cropper/cropper.css")
...
function initNonModal() {
	...
	if (avatarReload){ 
		imageName = imgServer + imgIn + '/tmp_' + wgUserID + '.jpg?' + Math.floor(Math.random() * 99999);
	} else {
		imageName = imgServer + imgIn + '/' + wgUserID + '.jpg?' + Math.floor(Math.random() * 99999);
	}
...
function setActionDIV() {
	...
	initimg.name = imgOut + '/' + wgUserID + '.jpg';
	initimg.onload = function() {
		$('avatarULaction').innerHTML = "<a href onclick=\"removeButton();return false;\" onhover=\"background-color: #222;\" >remove</a> | <a href onclick=\"editButton();return false;\" onhover=\"background-color: #222;\">edit</a>";
	};
	initimg.onerror = function() {
		$('avatarULaction').innerHTML = "<div class='avatarULtextBox'><a href id='gatUploadImageLink' onclick=\"uploadImageLink();return false;\">Upload Image</a></div>";
		$('avatarCrop').style.display = 'none';
		$('avatarULimg').style.display = 'none';
	};
	initimg.src = imgServer + imgOut + '/' + wgUserID + '.jpg?' + Math.floor(Math.random() * 99999);
...
function getWidthAndHeight() {
	...
	$('avatarImgBlock').style.marginLeft = newmarg + 'px';
	document.getElementById('avatarIn').src = imgIn + '/tmp_' + wgUserID + '.'+ extension +'?'+ Math.floor(Math.random() * 99999);
...
function completeCallback(response) {
	...
	if (msg.match('has been uploaded')) {
		...
		newImg.name = imgServer + imgIn + '/tmp_' + wgUserID + '.'+ extension +'?'+ Math.floor(Math.random() * 99999);
		newImg.onload = getWidthAndHeight;
		newImg.onerror = loadFailure;
		newImg.src = imgServer + imgIn + '/tmp_' + wgUserID + '.'+ extension +'?'+ Math.floor(Math.random() * 99999);
