...
class AdminNoIntroImage extends UnlistedSpecialPage {
	...
	public function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage('AdminNoIntroImage');
	}
		...
		foreach ($urls as &$url) {
			...
			if (!$url['title']) {
				...
				if (!$introText) {
					...
					$introText = self::removeIntroImages($introText,$url['title']);
					if (!empty($introText)) {
						$wikitext = Wikitext::replaceIntro($wikitext, $introText, true);
						$comment = 'Adding {{nointroimg}} template and removing intro images';
	...
	/**
	 * Remove any intro images and insert the nointroimg template in
	 */
	private static function removeIntroImages($text) {
		//strip current images
		$text = preg_replace('@\[\[Image[^\]]*\]\]@','',$text);
		
		//add the nointroimg template
		if (!preg_match('@{{nointroimg}}@',$text)) {
			$text = '{{nointroimg}}'.$text;
		}
