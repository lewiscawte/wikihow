...
class AddRelatedLinks extends UnlistedSpecialPage {
	...
	function addLinkToRandomArticleInSameCategory($t, $summary =  "Weaving the web of links") {
		global $wgOut;
		...
		while (sizeof($cats) > 0) {
			...
			$id  = $dbr->selectField(array('categorylinks', 'page'), 
					array('cl_from'), 
					array('cl_to'=>$cat, 'page_id = cl_from', 'page_namespace'=>NS_MAIN),
			...
			#MarkRelated::addRelated($src, $t, $summary, true);
		}
	}
...
END
		...
		foreach ($urls as $url) {
			...
			if (sizeof($good) == 0)  {
				self::addLinkToRandomArticleInSameCategory($t);	
			} else {
				$x = rand(0, min(4, sizeof($good) - 1));
				$src = $good[$x];
				$wgOut->addHTML("<li>Linked from <b>{$src->getText()}</b> to <b>{$t->getText()}</b></li>\n");
