...
class MobileWikihow extends UnlistedSpecialPage {
	...
	const NON_MOBILE_COOKIE_NAME = 'whnombl';
	...
	private function displayHtml() {
		...
		if ($isMainPage) {
			...
			$articleVars = array(
				...
				'width' => $width,
				'height' => $height,
				'DEVICE_WIDTH' => $this->getDeviceWidth(),
			);
...
DONE;
		...
		*/
		...
		foreach ($nodes as $img) {
			...
			if ($width > self::MAX_IMAGE_WIDTH) {
				...
				$newHeight = (int)round(self::MAX_IMAGE_WIDTH * $height / $width);
				$a = $img->parentNode;
				...
				if ($title) {
					...
					$thumb = $image->getThumbnail($deviceWidth - 50, $this->getDeviceHeight() - 70);
					$newWidth = $thumb->getWidth();
					$newHeight = $thumb->getHeight();
					$url = wfGetPad($thumb->getUrl());
		...
		foreach ($nodes as $node) {
			...
			if ($width > $deviceWidth - 20) {
				$node->nodeValue = $deviceWidth - 20;
			}
		}
	...
	private static function getDeviceWidth() {
		return self::DEVICE_WIDTH;
	}

	const DEVICE_HEIGHT = 480;
	private static function getDeviceHeight() {
