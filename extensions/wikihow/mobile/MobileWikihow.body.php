...
class MobileWikihow extends UnlistedSpecialPage {
	...
	private function displayHtml() {
		...
		$titleBar = $isMainPage ? wfMsg('mobile-mainpage-title') : wfMsg('pagetitle', wfMsg('howto', $articleName));
		$canonicalUrl = 'http://' . self::getNonMobileSite() . '/' . $wgTitle->getPartialURL();
		$headerVars = array(
		...
		if ($isMainPage) {
			...
			$wgOut->addHTML( EasyTemplate::html('main-page.tmpl.php', $vars) );

			$wgOut->addHTML( EasyTemplate::html('footer.tmpl.php', $footerVars) );
			...
			$wgOut->addHTML( EasyTemplate::html('language-select.tmpl.php', $vars) );

			$wgOut->addHTML( EasyTemplate::html('footer.tmpl.php', $footerVars) );
		} elseif (!$pageExists) {
			$wgOut->addHTML( EasyTemplate::html('header.tmpl.php', $headerVars) );
			$nfVars = array( 'mainPage' => wfMsg('mainpage') );
			$wgOut->addHTML( EasyTemplate::html('not-found.tmpl.php', $nfVars) );
			$wgOut->addHTML( EasyTemplate::html('footer.tmpl.php', $footerVars) );
		} else { // article page
			...
			$articleVars = array(
				...
				'height' => $height,
				'deviceOpts' => $deviceOpts,
				'showAds' => $deviceOpts['show-ads'], // temporarily placing ads just under the tabs
			);
			$wgOut->addHTML( EasyTemplate::html('article.tmpl.php', $articleVars) );

			$footerVars['redirMainUrl'] = $redirMainBase . urlencode($partialUrl);
	...
	/**
	 * Parse and transform the document from the old HTML to the new mobile
	 * style.
	 */
		...
		foreach ($nodes as $img) {
			...
			if ($title) {
				...
				if ($grandparent && $grandparent->nodeName == 'div') {
					...
					else if($class && ($class->nodeValue == 'thumb tcenter' ||  $class->nodeValue == 'thumb tleft') ) {
