...
class RelatedVideoApi {
	...
	function loadFromDB() {
		...
		$row = $dbr->selectRow('video_data',
			array('vd_data'),
			array('vd_id' => $dbr->strencode($this->mId))
		);
...
class RelatedVideos {
	...
	function getResults() {
		...
		$key = wfMemcKey("Relatedvideos:" . $this->mTitle->getArticleId());
		if ($this->mSource)
...
class Video extends UnlistedSpecialPage {
	...
	function getRelatedVideos($target = null) {
		...
		$key = wfMemcKey("Related_videos_video_id_{$target}");
...
class Flagvideo extends UnlistedSpecialPage {

	function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage( 'Flagvideo' );
	}
	...
	function execute($par) {
		...
		$wgOut->clearHTML();
		$target = isset($par) ? $par : $wgRequest->getInt('target');
		if (!$target) {
			$wgOut->addHTML('error: must specify video target');
			return;
		}
...
class ManageFlaggedVideos extends UnlistedSpecialPage {
	...
	function execute($par) {
		...
		$sql = "SELECT COUNT(DISTINCT(vf_user_text)) AS C, vf_id, vt_title
					FROM video_flag LEFT JOIN video_titles 
						ON vf_id=vt_id 
						GROUP BY vt_id HAVING C >= 3";
		$res = $dbr->query($sql);
		while ($row = $dbr->fetchObject($res)) {
			$result = array(
				'title' => $row->vt_title,
