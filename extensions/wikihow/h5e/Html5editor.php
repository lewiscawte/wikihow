...
$wgHooks['SpamBlacklistFoundSpam'][] = array('Html5Editor::setSpamMsg');
...
function hasHtml5Browser() {
	...
	if (!H5E_DEBUG)
		$match = preg_match('@(firefox/[3-9])@i', $userAgent);
	else
...
$wgIgnoreTemplates = array("{{SITENAME}}");
...
function HtmlWrapASinglePart($part) {
	...
	while ($x = array_shift($parts)) {
		if (strpos($x, "{{") === 0 && trim($x) != "" && !in_array($x, $wgIgnoreTemplates)) {
...
function Html5WrapTemplates(&$parser, &$text, &$stripstate) {
	...
	while (sizeof($nowikiparts) > 0) {
		...
		if (strtolower($n) == "<nowiki>") {
			...
			$newcontent .= "<div class='nowiki' val=\"{$key}\"><nowiki>{$n}</nowiki></div>";
