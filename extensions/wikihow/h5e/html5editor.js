	...
	function Editor() {
		...
		function startEditing(section, postEditFunc) {
			...
			*/
			...
			if (browser.settings['monitor-keyup-events']) {
				$('#bodycontents').keyup(keyInput.onKeystroke);
			}
		...
		this.displaySavingNotice = function (msg) {
			...
			$('.saving-message', savingNotice).html(msg);
		...
		function attachEditWikitextListener() {
			$('.h5e-adv-link').click(function () {
				...
				drafts.saveDraft(function() {
					...
					var url = wgServer + '/index.php?title=' + wgPageName + '&action=edit&advanced=true';
