...
WH.h5e = (function ($) {
	...
	/**
	 * Load the template for a new article.
	 */
	function loadNewArticleHTML() {
		// This DOM node will exist (it's placed there in the server-side
		// php on page load)
		var html = $('#h5e-new-article-template').html();
		if (html != '') {
			$('#bodycontents').html(html);
		}
	}
	...
	function dialogClickListeners() {
		...
		$('#h5e-link-article').keypress(function() {
			$('#h5e-link-article').removeClass('h5e-url-warning');
		});
	...
	function getEditSettingsForBrowser() {
		...
		var createNewArticle = !!document.location.href.match(/[?&]new-article=true/);
		config['create-new-article'] = createNewArticle;

		// temporary: if e=true param is in url, throw us into edit 
		// right away, for speedy testing
		var startEditing = document.location.href.match(/[?&]e=true/);
	...
	return {
		...
		init: function() {
			...
			if (isHtml5EditingCompatible() && wgNamespaceNumber == 0) {
				...
				if (settings['create-new-article'] || settings['start-editing']) {
					startEditing('intro');
				}
			} else {
alert('gothere');
