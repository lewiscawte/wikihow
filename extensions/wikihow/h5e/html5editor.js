...
WH.h5e = (function ($) {
	...
	function getCursorLi() {
		var select = window.getSelection();
		var currentli = $(select.anchorNode).parentsUntil('#steps ol').last();
		if (!currentli.length && select.anchorNode.nodeName == 'LI') {
			currentli = $(select.anchorNode);
		}
		return currentli;
	}
	...
	function removeStepOnBackspace(evt) {
		...
		// store the rest of the content of li which the cursor is on, then remove it
...
//console.log('ch:'+key);
	...
	function isStepsCursorTopLevel() {
		...
		if (gotUL && anchor.nodeName == 'LI' && parents.length == 2 && parents[0].nodeName == 'UL') {
			return true;
		} else {
			return !gotUL;
		}
	...
	function isCursorBold() {
		...
		if (anchor.nodeName == '#text') {
			anchor = $(anchor).parent()[0];
		}
		var isBold = 
			anchor.nodeName == 'B' || anchor.nodeName == 'STRONG' ||
			anchor.nodeName == 'SPAN' && $(anchor).css('font-weight') == 'bold';
		return isBold;
	} */
	...
	function focusCurrentSection() {
		...
		$(node).parentsUntil('#bodycontents').last().focus();
	}
	...
	function getOrigRefText(refnode) {
		var refnum = $('a', refnode).attr('href').replace(/^#_note-/, '');
		var li = $('li#_note-' + refnum).clone();
		$('a:first', li).remove();
		var reftext = '';
		...
		$(sections).each(function(i, section) {
			...
			if (sectionPresent && !checked) {
				...
				if (typeof removedSections[ section['key'] ] == 'undefined') {
					var addSection = $('<h2><span>' + section['name'] + '</span></h2><div id="' + section['key'] + '" class="article_inner editable" contenteditable="true"><ul><li>' + wfMsg('h5e-new-section') + '</li></ul></div>');
				} else {
	...
	function startEditing(section, postEditFunc) {
		...
		// TODO: Bebeth, instead of slideDown here I should be using animate.  
		// The problem with slideDown is the button squish as they're sliding 
		// in.
	...
	function dialogClickListeners() {
		...
		var showAnchorEditLink = false;
		editLink.hide();
		$('#bodycontents').bind('keypress click', function(e) {
			currentLinkNode = $(getCursorNode());
			var startNode = currentLinkNode;
			var newShowLink = startNode.is('a');

			// check to see if showAnchorEditLink has changed, modify the
			// hrefs and css only if it has
			if (newShowLink !== showAnchorEditLink) {
				showAnchorEditLink = newShowLink;
				if (showAnchorEditLink) {
					editLink
						.css({
		...
		$('#h5e-link-query').keypress(function(evt) {
			if (evt.which == 13) { // 'Enter' key pressed
				return $('#h5e-link-search-button').click();
			}
		});
