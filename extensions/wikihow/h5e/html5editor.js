...
WH.h5e = (function ($) {
	...
	function createNewStepOnEnter(evt) {
		...
		var b_open = settings['auto-bold'] ? '<b>' : '<span>';
		var b_close = settings['auto-bold'] ? '</b>' : '</span>';
		var stepFilling = settings['non-empty-steps'] ? '&nbsp;' : '';
		var newstep_tmpl = '<li class="steps_li"><div class="step_num" contenteditable="false">1</div>' + b_open + stepFilling + b_close + '<div class="clearall"></div>';
		...
		if (settings['auto-bold']) {
			var node = $('b', newli).first();
		} else {
			var node = $('span', newli).first();
		}
		if (node.length) {
			setCursorNode(node.get(0), 0);
		}
...
//console.log('here');
	...
	/**
	 * If user deletes all of the step section, or tips or warnings, we want
	 * to add the correct structure back to the document automatically.
	 */
	function replaceRemovedLi(sectionName) {
		...
		if (html == '' || html == '<br>' || !hasLi) {
			if (sectionName == 'steps') {
				var b_open = settings['auto-bold'] ? '<b>' : '';
				var b_close = settings['auto-bold'] ? '</b>' : '';
				...
				//if (getCurrentCursorSection() == 'steps') {
					...
					loadCursorPos({'node': newli.contents().get(1), 'offset': len});
				//}
				...
				loadCursorPos({'node': newli.contents().get(0), 'offset': len});
			}
		}
	}
...
//		if (evt.type == 'keyup' && evt.which != 8) return;
//		if (evt.type == 'keypress' && evt.which == 8) return false;
//console.log('y',evt.type,evt.which,evt);
		...
		if (isEnterKeyEvent(evt)) {
			...
			var section = getCurrentCursorSection();
			if (section == 'steps') {
	...
	function onCursorCheck() {
		// static vars used only in this function
		onCursorCheck.cursorHasItalics = false;
		onCursorCheck.cursorHasIndent = false;

		var italics = document.queryCommandState('italic');
		if (italics && !onCursorCheck.cursorHasItalics) {
			onCursorCheck.cursorHasItalics = true;
			$('#h5e-toolbar-italics').addClass('h5e-active');
		} else if (!italics && onCursorCheck.cursorHasItalics) {
			onCursorCheck.cursorHasItalics = false;
		...
		if (indented && !onCursorCheck.cursorHasIndent) {
			onCursorCheck.cursorHasIndent = true;
			$('#h5e-toolbar-outdent').removeClass('h5e-disabled');
		} else if (!indented && onCursorCheck.cursorHasIndent) {
			onCursorCheck.cursorHasIndent = false;
...
//console.log('xqq',node, 'ce',isNodeContentEditable(node),currentNode);
	...
	/**
	 * Toolbar hook just calls into addImage
	 */
	function tbAddImage() {
		addImage();
	}
	 ...
	 */
	function addImage(preAddCallback, preAddToIntro) {
		focusCurrentSection();

		// These static vars store the position of the cursor and 
		// cursor-related details before entering the Image Upload dialog.
		addImage.preEIUSection = getCurrentCursorSection();
		addImage.preEIUCursorLi = getCursorLi();
		addImage.preEIUCursorNode = getCursorNode();
		addImage.preEIUAddToIntro = typeof preAddToIntro != 'undefined' && preAddToIntro;
	...
	function insertImageHtml(details, html) {
		...
		if (!addImage.preEIUAddToIntro && addImage.preEIUSection == 'steps') {
			var currentli = addImage.preEIUCursorLi;
			...
			if (addImage.preEIUAddToIntro) {
				var node = $('#bodycontents').find('p:first');
			} else {
				var node = $(addImage.preEIUCursorNode);
			}
			var container = null;
			if (node.is('li')) {
				container = node;
	...
	function addOrRemoveSections(toAdd, toRemove, saveRemoved) {
		...
		addOrRemoveSections.removedSections = {};
		...
		$(sections).each(function(i, section) {
			if ($.inArray(section['key'], toRemove) >= 0) {
				...
				if (saveRemoved && nodes.length) {
					addOrRemoveSections.removedSections[ section['key'] ] = nodes;
				}
			} else if ($.inArray(section['key'], toAdd) >= 0) {
				// Add a new section
				if (typeof addOrRemoveSections.removedSections[ section['key'] ] == 'undefined') {
					...
					var addSection = addOrRemoveSections.removedSections[ section['key'] ];
				}
	...
	function slideToolbar(direction, func) {
		...
		// static var a default value.  this variable tracks whether
		// the toolbar is showing so that is can't be "double shown" or
		// "double hidden" as I've seen in certain weird error cases
		if (typeof slideToolbar.slideToolbar == 'undefined') {
			slideToolbar.isToolbarShowing = false;
		}

		var willToolbarShow = direction == 'down';
		if (slideToolbar.isToolbarShowing == willToolbarShow) {
			return;
		}
		slideToolbar.isToolbarShowing = willToolbarShow;
	...
	function addOrRemovePostPublish(add) {
		...
		if (add) {
			...
			$('.h5e-publish-save-draft', copy)
				.unbind('click')
				.click(function() {
					var saveDraft = $('#h5e-toolbar-savedraft');
					if (saveDraft.length) {
						var a = $('a', saveDraft);
	...
	/**
	 * Grey out most non-editable parts of the article.
	 */
	...
	/**
	 * Un-grey most non-editable parts of the article.
	 */
	...
	function startEditing(section, postEditFunc) {
		...
		// TODO - put focus on text node if non-text isn't available
		$('.step_num').click(function() {
			var next = $(this).next();
			if (next.length) {
				setCursorNode(next.get(0), 0);
			}
			return false;
		});
	 ...
	 */
	function loadDraft(id, callbackFunc) {
		var contents = $('#bodycontents');
		$.post('/Special:Html5editor',
			{ eaction: 'load-draft',
			  userid : wgUserID,
	...
	function tbClickListeners() {
		...
		$('#h5e-loaddraft-confirm-load').click(function() {
			...
			var url = wgServer + '/index.php?title=' + wgPageName + '&h5e=true&draft=' + loadDraftConfirm.loadDraftID + '&create-new-article=true';
	...
	/**
	 * This is used to clear any auto-complete results that linger or appear
	 * after a dialog's been closed.
	 */
	...
	function tooltipLinkListener() {
		...
		tooltipLinkListener.tooltipCurrentLinkNode = getAnchorNode(startNode);
		...
		$('#h5e-editlink-change').click(function() {
			...
			showEditLinkDialog('change', text, href,
				function(text, link) { // when 'Change' button is clicked
					...
					tooltipLinkListener.tooltipCurrentLinkNode.attr('title', getArticleFromLink(text));
	...
	function init() {
		...
		if (WH.h5e.browser.isHtml5EditingCompatible()
			&& isArticle
			&& !wgForceAdvancedEditor
			&& WH.h5e.browser.isArticleEditAllowed())
		{

			settings = WH.h5e.browser.getEditSettingsForBrowser();
			...
			var autoEditNow = settings['create-new-article'] || settings['start-editing'] || draftid > 0;
...
WH.h5e.browser = (function() {
	...
	function getEditSettingsForBrowser() {
		...
		// We only want to do "auto-bolding" on edited articles, not new ones.
	...
	// public interface
	return {
		isHtml5EditingCompatible: isHtml5EditingCompatible,
		getEditSettingsForBrowser: getEditSettingsForBrowser,
		isArticleEditAllowed: isArticleEditAllowed
	};
