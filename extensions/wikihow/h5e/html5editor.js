		...
		if (key == 13 && typeof evt.isSimulated == 'undefined') { // 'Enter' key
			...
			window.setTimeout(function() {
				...
				if (html == '' || html == '<br>') {
					...
					var b_open = settings['auto-bold'] ? '<b id="h5e-new-first-step">' : '';
					var b_close = settings['auto-bold'] ? '</b>' : '';
					var tmpl = '<ol class="steps_list_2"><li class="steps_li final_li"><div class="step_num" contenteditable="false">1</div>' + b_open + wfMsg('h5e-first-step') + b_close + '</li></ol>';
					$('#steps').html(tmpl);
...
//console.log('xqq',node, 'ce',isNodeContentEditable(node),currentNode);
	...
	function tbClickListeners() {
		...
		$('#h5e-loaddraft-confirm-load').click(function() {
			...
			var url = wgServer + '/index.php?title=' + wgPageName + '&h5e=true&draft=' + loadDraftID + '&create-new-article=true';
