...
WH.h5e = (function ($) {
	...
	function addNewArticleFeatures() {
		...
		$('#bodycontents h2').each(function() {
			...
			if (heading != wfMsg('Steps') && heading != wfMsg('relatedwikihows')) {
				$(this).append('<a class="h5e-first-remove-section" href="#">' + wfMsg('h5e-remove-section') + '</a>');
			}
		});
		$('.h5e-first-remove-section').click(function() {
			setPageDirty();
			var removeID = $(this).parent().next().attr('id');
			addOrRemoveSections([], [removeID], false);
			return false;
		});
		$('.edit_article_button').after('<a class="h5e-adv-link" href="/index.php?title=' + wgPageName + '&action=edit&advanced=true">' + wfMsg('h5e-switch-advanced') + '</a>');
	}
	...
	function startEditing(section, postEditFunc) {
		...
		var editingTitle = wfTemplate(wfMsg('h5e-editing-title'), howtoTitle);
	...
	function stopEditing(saveIt, overrideChecks) {
		...
		if (saveIt) {
			...
			// if REST call lasts more than 15 seconds, we fail with an error
			// message and ignore the result
