...
DONE;
		...
		$nodes = $xpath->query("//div[@class='template']|//div[@class='wh_ad']|//div[@class='step_num']|//comment()");
		foreach ($nodes as $node) {
			if ($node)
				$node->parentNode->removeChild($node);
		}
		...
		// handle no wiki
		$nodes = $xpath->query("//div[@class='nowiki']");
		foreach ($nodes as $node) {
			$val = $node->nodeValue;
		...
		#$this->debug("input-beforeimages.txt", $doc->saveHTML());
		$this->handleImages($doc, $xpath, $oldtext);
		$this->debug("input-afterimages.txt", $doc->saveHTML());
