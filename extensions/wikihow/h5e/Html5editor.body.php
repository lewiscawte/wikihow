	...
	function handleImages(&$doc, &$xpath, $oldtext) {
        ...
        foreach ($nodes as $node) {
			...
			if (isset($oldimages[$name])) {
        		...
        		$newnode = $doc->createTextNode(trim($tag));
            	$node->parentNode->replaceChild($newnode, $node);
				continue;
			}
...
DONE;
        ...
        foreach ($nodes as $node) {
			...
			foreach ($node->childNodes as $c) {
				...
				if ($c->nodeName == '#text')  {
					...
					$newnode->appendChild($doc->createTextNode(trim($c->nodeValue)));
				} else {
        ...
        foreach ($nodes as $node) {
            ...
            $newnode = $doc->createElement("div");
            $text = $doc->createTextNode(trim($matches[0]) . "\n");
			$newnode->appendChild($text);
			// for video descriptions
			foreach ($node->childNodes as $n) {
				if ($n->nodeValue && trim($n->nodeValue) != "") {
					$x = $n->cloneNode(true);
					$r = $newnode->appendChild($n);
				}
			}
            $node->parentNode->replaceChild	($newnode, $node);
    ...
    function execute ($par) {
		...
		if ($a->doEdit($newtext, "updating from html5 editor")) {
