...
class LSearch extends SpecialPage {
	...
	function logSearch ($q, $host_id, $cache, $error, $curl_err, $gm_tm_count, $gm_ts_count, $username, $userid, $rank, $num_results) {
		$dbw	= wfGetDB(DB_MASTER);
		$dbw->insert('gmini', 	
			array('gm_query' 			=> strtolower($q), 
					'gm_host_id' 		=> $host_id, 
					'gm_cache' 			=> $cache, 
					'gm_error' 			=> $error, 	
	...
	function wfGoogleMiniStarElement($parser, $name, $attrs) {
		if ($name == "RES") {
			$this->mEn = $attrs['EN'];
		}	
		if ($name == "R") {
	...
	function wfGoogleMiniEndElement($parser, $name) {
		if ($name == "R") {
			$this->mResults[] = $this->mCurrentNode;
			$this->mCurrentNode = null;
		}
	}
	function wfGoogleMiniDefaultHandler($parser, $data)  {
		if ($this->mCurrentTag) {
	...
	function googleSearchResultTitles($q, $start = 0, $limit = 30, $minrank = 0) {
		$this->googlSearchResults($q, $start, $limit);
		$results = array();	
	...
	function googlSearchResults($q, $start, $limit = 30) {
		global $wgGoogleMiniHost, $wgOut, $wgRequest, $wgUser,  $wgMemc, $wgBogusQueries;

		if (
		//		$wgUser->getID() == 0 && 
			$wgRequest->getVal('rss') == 1) {
				require_once('extensions/wikihow/GoogleAjaxSearch.body.php');
			// use limit and start parameters to allow for paging
			if ($wgRequest->getVal('chrome') == '1') {
				$results = GoogleAjaxSearch::scrapeGoogle($q, $wgRequest->getVal('limit'), 'wikkihow.com');
			} else {
				$results = GoogleAjaxSearch::scrapeGoogle($q, 16);
			}
			$wgOut->disable();		
			$pad = "           ";
...
echo "</RES>
		...
		$key = "GoogleMini:" . str_replace(" ", "-", $q)."-".$start."-".$limit;
		$dbw = wfGetDB(DB_MASTER);
		$set_cache = true;
...
#echo $url;
			...
			if (curl_errno($ch)) {
				$wgOut->addHTML(	
						//"Error received: ". print_r(curl_error($ch), true) . " for urls $url<br/>\n		
				...
				LSearch::logSearch($q, $host_id, 0, 1, curl_errno($ch), $gm_tm_count, 0, $wgUser->getName(), $wgUser->getID(), 0, 0);
		 		curl_close($ch);
				return null; 
			} else {
		 		curl_close($ch);
		...
		LSearch::logSearch ($q, $host_id, $cache, 0, 0, $gm_tm_count, $tm, $wgUser->getName(), $wgUser->getId(), $rank, $num_results);
		
		if ($set_cache) {
			$wgMemc->set($key, $contents, time() + 3600);
			#echo "setting cache for $q<br/>";
		}
		preg_match_all("/<R N=[^>]*>.*<\/R>/", $contents, $matches);
		$xml_parser = xml_parser_create();
		xml_set_element_handler($xml_parser, array($this, "wfGoogleMiniStarElement"), array($this, "wfGoogleMiniEndElement"));
		xml_set_default_handler($xml_parser, array($this, "wfGoogleMiniDefaultHandler"));
	...
	function execute($par = '') {	
		...
		$contents = $this->googlSearchResults($q, $start);
	
		if ($contents == null) return; 
	
		if ($wgRequest->getVal('raw') == true) {
			if ($this->mResults == null) { exit; } 
