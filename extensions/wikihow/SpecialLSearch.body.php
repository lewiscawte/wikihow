...
echo "</RES>
		...
		if ($results === null) {
			...
			foreach ($titles as $title) {
				...
				if ($title['is_category']) {
					$title['has_supplement'] = 1;
					$title['popularity'] = 1001;
					$title['img_thumb_100'] = 'http://pad1.whstatic.com/images/a/a2/Book_266.png';
				}
				$results[] = $title;
			}
	...
	public function execute($par = '') {
		...
		$html = $tmpl->execute('search-results.tmpl.php');

		$wgOut->addStyle(wfGetPad('/skins/WikiHow/searchresults.css?' . WH_SITEREV));
		$wgOut->addHTML($html);
	}
