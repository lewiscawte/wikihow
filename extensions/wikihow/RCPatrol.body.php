...
#echo $reverse; print_r($_GET);
	...
	function getHelpTab(){
		$tab = "<tr class='rc_submenu' id='rc_help'><td>" . wfMsg('rcpatrolhelp_top') . wfMsg('rcpatrolhelp_bottom') . "</td></tr>";
		return $tab;
	}
	...
	function getActiveWidget() {
		...
		//line below no longer needed with weather icon on widget
	...
	function setActiveWidget() {
		global $wgUser;
		$html = "<div id='rcactivewidget'>";
		$html .= "<a class='rc_help' title='" . wfMsg("rc_help") . "' href='/" . wfMsg("rcchange-patrol-article") . "'><img src='" . wfGetPad("/skins/WikiHow/images/icon_help_tan.jpg") . "'></a>";
		$html .= "<h3>" . wfMsg('rcpatrolstats_currentstats') . "<span class='weather'></span></h3>";
		$html .= "<table class='rcleaders'>";
		$html .= $this->getActiveWidget() . "</table><center>" . wfMsg('rcpatrolstats_activeupdate') . "</center></div>";
        $skin = $wgUser->getSkin();
        $skin->addWidget($html);
	}
	...
	function getLeaderboard() {
		...
		$data = Leaderboard::getRCQuickEdits($starttimestamp);
		arsort($data);
		...
		foreach($data as $key => $value) {
			...
			if (($value > 0) && ($key != '')) {
				...
				$html .= "<tr $class>
					<td class='rcleaderimg'>" . $img . "</td>
					<td class='leader_user'>" . $sk->makeLinkObj($u->getUserPage(), $u->getName()) . "</td>
					<td class='patrolcount'><a href='/Special:Leaderboard/$target?action=articlelist&period=$period&lb_name=".$u->getName() ."' >$value</a> </td>
				</tr> ";
				$data[$key] = $value * -1;
				$index++;
			}
		...
		//this is the old code for top patroller, above is top rc editors
		/*$dbr = wfGetDB(DB_SLAVE);
		$res = $dbr->query ("select log_user, count(*) as C
...
class RCActiveWidget extends SpecialPage {
    ...
    function __construct() {
        SpecialPage::SpecialPage( 'RCActiveWidget' );
    }
