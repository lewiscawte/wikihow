...
#print_r($users); print_r($result); exit;
	...
	function getButtons($result, $rev) {
		...
		$s .= ThumbsUp::getThumbsUpButton($result);
		$s .= "</td></tr>";
...
class RandomEdit extends SpecialPage {

	function __construct() {
		SpecialPage::SpecialPage( 'RandomEdit' );
	}
	...
	function getDiffToMeasure($r) {
		...
		$not_hi_id  = $dbr->selectField('revision', 'rev_id', 
			array('rev_page'=>$r->mTitle->getArticleID(), 
					'rev_user_text != ' . $dbr->addQuotes($r->mUserText),
				'rev_id > ' . $r->mId
				)
		...
		if (!$not_hi_id) {
			...
			$revhi = $dbr->selectField('revision', 'rev_id', 
				array('rev_page'=>$r->mTitle->getArticleID(), 'rev_id <  ' . $not_hi_id),
				"RandomEdit::getDiffToMeasure",
				array("ORDER BY"=>"rev_id desc", "LIMIT"=>1)
				);
		}
	...
	function getPoints($r, $d, $de) {
		...
		if (preg_match("@Reverted@", $r->mCommment)) {
			$wgOut->addHTML("<li>No points : reverted edit.</li></ul>");
			return;
		}

		$wordpoints = min(round($addedwords / 100), 5);
	...
	function execute($par)  {
		...
		if ($target) {
			if (preg_match("@[^0-9]@", $target)) {
				// get a random revision from a specific title
				$t = Title::newFromURL($target);
				$r = self::getRandomEdit($t); 
			} else {
				// get a specific revision
				$r = Revision::newFromID($target);		
			}
		} else {
			// get a completely random revision from a random artile
			$r = self::getRandomEdit(); 
		}
