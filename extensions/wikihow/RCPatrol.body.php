...
class RCPatrol extends SpecialPage {
	...
	function getNextArticleToPatrolInner($rcid = null) {
		...
		$rc_user_filter		= $wgRequest->getVal('rc_user_filter');
	...
	function getButtons($result, $rev) {
		...
		$s .= "<div id='rc_subtabs'>
            ...
            <div class='tableft' id='rctab_user'>
                <div class='tabright'>
                    <a href='#'><div class='arrow'></div>By User</a>
                </div>
            </div>
...
#echo $reverse; print_r($_GET);
	...
	function getUserTab() {
        $tab = "<tr class='rc_submenu' id='rc_user'><td>
            <div id='controls' style='text-align:center'>
				Username: <input type='text' name='rc_user_filter' id='rc_user_filter' size='30' onchange='changeUserFilter();'/>	
				<input type='button' value='Go' onclick='changeUser(true);'/>
				|
				<a href='#' onclick='changeUser(false);'>Off</a>
            </div></td></tr>";
        return $tab;
	}
