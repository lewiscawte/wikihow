...
class YTThumb extends UnlistedSpecialPage {
	...
	function processUpload() {
		...
		// take out the ones that were supposed to be removed
		$remove = split(",", $wgRequest->getVal("remove"));
		foreach ($remove as $r) {
			$r = trim($r);
			if ($r == "") {
				continue;
			}
			$r = preg_replace("@^/Image:@", "", $r);
			$img = Title::newFromURL(urldecode($r));
			$steps = preg_replace("@\[\[Image:{$img->getFullText()}[^\]]*\]\]@", "", $steps);
		}
	
		$newtext = $wgParser->replaceSection( $text, $index, $steps );
		
		// add sources and citations 
		$sindex = 0;
		$sources = null;
		while ($s = Article::getSection($newtext, $sindex)) {
		...
		if ($sources && !preg_match("@{$vidid}@", $sources)) {
			$sources .= "\n" . $ref;
			$newtext = $wgParser->replaceSection( $newtext, $sindex, $sources);
		} else if (!$sources) {
			$newtext .= "\n\n== " . wfMsg('sources')  . " ==\n" . $ref; 
		}
		wfDebug("NEWTEXT: " . $newtext); 
    ...
    function execute ($par) {
		...
		if (preg_match($re, $author)) {
			$wgOut->addHTML("Unfortunately, we cannot add thumbnails from the author {$author}'s videos");
			return;
		}
