...
return "";
	...
	public static function getNextToPatrol($type) {
		...
		if ($previous) {
			$opts[] = "  qc_id NOT IN ({$previous})"; 
		}
			
		$result = $dbw->selectRow('qc', array('*'), $opts);
...
#echo $dbw->lastQuery(); 
	...
	public static function vote($qcid, $vote) {
		...
		$row = $dbw->selectRow('qc', array('qc_yes_votes', 'qc_no_votes', 'qc_yes_votes_req', 'qc_no_votes_req'),array('qc_id'=>$qcid));
...
class QCRuleIntroImage extends QCRuleTextChange {
	...
	function getNextToPatrolHTML() {
		...
		$text = $r->getText(); 
		$intro = Article::getSection($text, 0); 
        $html .= "<h2>" . wfMsg('howto', $t->getText()) . "</h2>";

		preg_match("@\[\[Image:[^\]]*\]\]@", $text, $matches); 
		if (sizeof($matches) > 0) {
        	$html .= "<center>" . $wgOut->parse($matches[0]) . "</center><br clear='all'/>";
		} else {
...
class QC extends SpecialPage {
	...
	function execute ($par) {
		...
		if ( !in_array( 'qc', $wgUser->getRights() ) ) {
		...
		// set up the form
		$wgOut->addHTML("<form name='qcvote_form' method='POST' action='/Special:QC/{$target}'>
			<input type='hidden' name='qc_vote' id='qc_vote' value=''/>
			<input type='hidden' name='qc_skip' id='qc_skip' value='0'/>
		");

		// grab the next check
		$c = QCRule::getNextToPatrol($target); 
		if ($c)  {
			$wgOut->setPageTitle($c->getPrompt());
			$html = $c->getNextToPatrolHTML();
			// UI work needed here
			$wgOut->addHTML("<center><input type='button' value='Yes' onclick='return qcVote(true);'> | 
						<input type='button' value='No' onclick='return qcVote(false);'/> | 
						<input type='button' value='Skip' onclick='return qcSkip();'/>
						</center><br/><br/>");
			$wgOut->addHTML($html);
		} else {
			$wgOut->addHTML(wfMsg('qc_congrats'));
		}
		$wgOut->addHTML("</form>");
	...
	/*
		...
		$dbr = wfGetDB(DB_SLAVE); 
		$res = $dbr->select('qc', array('qc_id', 'qc_key', 'qc_action', 'qc_page', 'qc_timestamp', 'qc_user', 'qc_user_text', 'qc_rev_id', 'qc_old_rev_id', 'qc_patrolled', 'qc_yes_votes_req', 'qc_no_votes_req', 'qc_yes_votes', 'qc_no_votes'), array('qc_patrolled'=>0)); 
		$wgOut->addHTML("<table width='100%'>");
		while ($row = $dbr->fetchObject($res)) {
			$t = Title::newFromID($row->qc_page);
			$u = User::newFromName($row->qc_user_text);
			if ($u) {
				$wgOut->addHTML("
				<tr><td>{$row->qc_id}<td><td>{$row->qc_key}:{$row->qc_action}</td>
				<td>{$row->qc_timestamp}</td>
				<td><a href='{$u->getUserPage()->getFullURL()}' target='new'>{$u->getName()}</td>
				<td><a href='{$t->getFullURL()}' target='new'>{$t->getFullText()}</td>
				<td><a href='{$t->getFullUrl()}?diff={$row->qc_rev_id}&oldid={$row->qc_old_rev_id}' target='new'>Diff</a></td>		
				</tr>");
			} else {
				$wgOut->addHTML("
				<tr><td>{$row->qc_id}<td><td>{$row->qc_key}:{$row->qc_action}</td>
				<td>{$row->qc_timestamp}</td>
				<td><a href='' target='new'>{$row->qc_user_text}</td>
				<td><a href='{$t->getFullURL()}' target='new'>{$t->getFullText()}</td>
				<td><a href='{$t->getFullUrl()}?diff={$row->qc_rev_id}&oldid={$row->qc_old_rev_id}' target='new'>Diff</a></td>		
				</tr>");
			}
