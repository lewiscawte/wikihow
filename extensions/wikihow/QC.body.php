...
class QC extends SpecialPage {
	...
	function getVoteBlock($qc_id) {
		$dbr = wfGetDB(DB_SLAVE);
		$res = $dbr->select('qc', array('qc_yes_votes_req','qc_no_votes_req','qc_key'), array('qc_id' => $qc_id), 'QC::getVoteBlock');
	...
	function getYesNoVotes($qc_id, $req_y, $req_n, $qc_key) {
		...
		$dbr = wfGetDB(DB_SLAVE);
		$res = $dbr->select('qc_vote', array('qcv_user','qcv_vote'), array('qcv_qcid' => $qc_id), 'QC::getVoteBlock', array('ORDER BY' => 'qcv_vote DESC'));
