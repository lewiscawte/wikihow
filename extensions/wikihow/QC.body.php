...
abstract class QCRule {
	...
	public static function getNextToPatrol($type) {
		...
		$sql .= " ORDER BY qc_id DESC LIMIT 1";
		$res = $dbw->query($sql);	
	...
	function getChangedBy($action_str,$r,$t,$pic_height='',$pic_width='') {
		$u = User::newFromName($this->mResult->qc_user_text);		
		
/*		if ($pic_height != '') {
			//for images w/ an inset thumbs up button and changedby
			$thumbresult['new'] = $r->getID();
			$thumbresult['old'] = $r->getPrevious()->getID();
			$thumbresult['title'] = $t;
			
			$thumbs = ThumbsUp::getThumbsUpButton($thumbresult);
			$style = " style='margin-right:".((639-$pic_width) / 2)."px;margin-top:-".($pic_height-2)."px;'";
			$html = "<div id='qc_changedby' class='qc_changedby_inset'{$style}>{$action_str}";
		}
		else {
*/
...
//		}
		...
		if ($u) {		
			...
			$html .= $thumbs . "<span class='tooltip_span'>Hi, I'm {$display}</span>";
		} else {
...
class QCRuleIntroImage extends QCRuleTextChange {
	...
	function getNextToPatrolHTML() {
		...
		$changedby = self::getChangedBy("Image added by: ",$r,$t,$pic->height,$pic->width);
...
*/						
			...
			$html .= "Hmm..., it has likely already been removed. Report this to travis: Title: {$t->getText()}";
		} 	
...
#$test = $this->hasText($part, $newtext); echo var_dump($test); exit;
	...
	function getNextToPatrolHTML() {
		...
		$html = self::getHeader($t); 
		$changedby = self::getChangedBy("Video added by: ",$r,$t);
...
class QCRCPatrol extends QCRule {
	...
	function getNextToPatrolHTML() {
		...
		// interesting
		$html = self::getHeader($t); 		
		$changedby = self::getChangedBy("Edits patrolled by: ",$r,$t);
...
class QCRuleTemplateChange extends QCRuleTextChange {
	...
	function getNextToPatrolHTML() {
		...
		$changedby = self::getChangedBy("Template " . $this->mResult->qc_action . " by: ",$r,$t);
