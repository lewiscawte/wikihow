...
abstract class QCRule {
	...
	function getChangedBy($action_str) {
		...
		if ($u) {
			...
			$html .= "<a target='new' href='{$u->getUserPage()->getFullURL()}'>{$display}</a>";
		} else {
...
$wgOut->addHTML("Sorry, we've found an issue in our recent 'Quality Guardian' tool.  We've taken this tool temporarily while we fix the problem.  The tool should be available again on Monday.  Thanks- Engineering team.");
return;
