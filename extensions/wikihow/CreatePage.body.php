...
class CreatePage extends SpecialPage {
	...
	function execute($par) {
		...
		$sk = $wgUser->getSkin();
		$this->setHeaders();
		$wgOut->addHTML("<style type='text/css' media='all'>/*<![CDATA[*/ @import '" . wfGetPad('/extensions/min/f/extensions/wikihow/createpage.css?') . WH_SITEREV . "'; /*]]>*/</style> ");
		$wgOut->addHTML("<script type='text/javascript' src='" . wfGetPad('/extensions/min/f/extensions/wikihow/createpage.js?') . WH_SITEREV . "'></script>");
	...
	function outputCreatePageForm() {
		...
		<script type=\"text/javascript\" src=\"" . wfGetPad('/skins/common/ac.js?') . WH_SITEREV . "\"></script>
		"
...
class SuggestionSearch extends UnlistedSpecialPage {
	...
	function matchKeyTitles($text, $limit = 10) {
		...
		$cacheKey = 'match_suggest_titles__' . $limit . ':' .$text;
		...
		if ($wgRequest->wasPosted() && $wgRequest->getVal('q') != null) {
			...
			for ($i = 0; $i < count($matches); $i++) {
				...
				if ($t)
					...
					$wgOut->addHTML("<td><!--id {$matches[$i][1]} --><input type='checkbox' name=\"{$name}\"/>&nbsp;<a href='{$t->getEditURL()}' class='new'>{$t->getFullText()}</a></td>");
				if ($i % 3 == 2) $wgOut->addHTML("</tr><tr>");
			}
