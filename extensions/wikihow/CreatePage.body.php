...
class CreatePage extends SpecialPage {
	...
	function execute($par) {
		...
		$sk = $wgUser->getSkin();
		$this->setHeaders();
		$wgOut->addHTML("<style type='text/css' media='all'>/*<![CDATA[*/ @import '" . wfGetPad('/extensions/wikihow/createpage.css?') . WH_SITEREV . "'; /*]]>*/</style> ");
		$wgOut->addHTML("<script type='text/javascript' src='" . wfGetPad('/extensions/wikihow/createpage.js?') . WH_SITEREV . "'></script>");
...
class SuggestionSearch extends UnlistedSpecialPage {
	...
	function matchKeyTitles($text, $limit = 10) {
		...
		$cacheKey = 'match_suggest_titles:' . $limit . ':' .$text;
