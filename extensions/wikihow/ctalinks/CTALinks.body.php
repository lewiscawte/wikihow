...
class CTALinks {
	...
	function getGoogleConversionScript() {
		$script = "";
		if (CTALinks::isConversionPageTarget()) {
			if (stripos($_SERVER['HTTP_REFERER'], "google") !== FALSE) {
				$script = wfMsgForContent('cta_search_conversion');
			} else {
				$script = wfMsgForContent('cta_nonsearch_conversion');
			}
		}
		return $script;
	}
	...
	function getGoogleControlScript() {
		$script = "";
		if (CTALinks::isArticlePageTarget()) {
			if (stripos($_SERVER['HTTP_REFERER'], "google") !== FALSE) {
				$script = wfMsgForContent('cta_search_control');
			} else {
				$script = wfMsgForContent('cta_nonsearch_control');
			}
		}
		return $script;
	}
	...
	function getGoogleControlTrackingScript() {
		...
		if (CTALinks::isArticlePageTarget()) {
			if (stripos($_SERVER['HTTP_REFERER'], "google") !== FALSE) {
				$script = wfMsgForContent('cta_search_control_tracking');
			} else {
				$script = wfMsgForContent('cta_nonsearch_control_tracking');
			}
		}
		return $script;
	}
	...
	function isArticlePageTarget() {
                ...
                return !($isLoggedIn || $createNewArticle || $wgTitle->getNamespace() != NS_MAIN || $wgTitle->getFullText() == wfMsg('mainpage') || $wgRequest->getVal('action') != '');
	}
