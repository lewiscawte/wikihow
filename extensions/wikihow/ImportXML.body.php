...
#echo "got T +$t+\n";
				...
				$title = trim(preg_replace("@^How to@im", "", $title));
				$articles[$title] = $text;
			}
		}
#print_r($articles); exit;
    ...
    function execute ($par) {
		...
		if ($wgRequest->wasPosted() && $wgRequest->getVal('xml')) {
			$dbw = wfGetDB(DB_MASTER);
			$articles = self::parseXML($wgRequest->getVal('xml'));
		...
		$wgOut->addHTML("
			<form method='post' action='/Special:ImportXML'>
		...
		while ($row = $dbr->fetchObject($res)) {
			...
			$wgOut->addHTML("<tr id='row_{$row->ia_id}' class='{$class}'><td>{$row->ia_id}</td><td>{$t->getText()}</td><td>{$row->ia_timestamp}</td>
				...
				<td><input style='height: 24px; width: 24px;' type='image' src='/extensions/wikihow/rcwDelete.png' onclick=\"delete_xml({$row->ia_id}, '{$row->ia_title}');\"></td>
		...
		<form action='/Special:ImportXML' method='post' accept-charset='UTF-8'>
				...
				if ($x == "steps") {
					if (preg_match("@(^===[^=]*===)@im", $beef))  {
						...
						while (sizeof($subs) > 0) {
							...
							$tnode->appendChild($dom->createTextNode(self::cleanupText($x)));
