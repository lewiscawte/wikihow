...
class NFDRuleTemplateChange {
	...
	function markPreviousAsPatrolled() {
		...
		$dbw->update("nfd", array("nfd_patrolled"=>1), array("nfd_page"=> $this->mArticle->getID()));
	}
	...
	function removeNFDEntry () {
		...
		if($row){
			...
			$dbw->update('nfd', array('nfd_patrolled' => '1'), array('nfd_id' => $row->nfd_id));
		}
	}
	...
	public static function vote($nfdid, $vote) {
		...
		// check, do we have to mark it as patrolled, or roll the change back?
		if ($vote) {
			if ($row->nfd_admin_delete_votes >= NFDRuleTemplateChange::getAdminDeleteVotesRequired() && $row->nfd_delete_votes >= NFDRuleTemplateChange::getDeleteVotesRequired()) {
				self::markNFDPatrolled($nfdid);
				$c = new NFDRuleTemplateChange();
	...
	function setFullTemplate($part, $newText){
		$matches = array();
		$count = preg_match('/{{nfd.*[^{{]}}/', $newText, $matches);
		if(count($matches) > 0){
			return $matches[0];
		}
		else{
	...
	function flagAction() {
		...
		if ($this->textRemoved($part, $oldtext, $newtext)) {	
			...
			$ret = true;
			$this->mAction = "added";
			$this->mReason = $this->setFullTemplate($part, $newtext);
		}
	...
	static function getDeleteVotesRequired() {
	...
	function getNextToPatrolHTML() {
		...
		if (!$t) {
			...
			return "<!--{$this->mResult->nfd_page}-->error creating title, oops, please <a href='#' onclick='window.location.reload()'>refresh</a>";
		}
	...
	function deleteArticle($nfdid, $nfdReason) {
		...
		if(!$foundDup){
			...
			//log the delete in the deletion table
			$log = new LogPage( 'delete', false );
	...
	function keepArticle($nfdid) {
		...
		$a = new Article($t);
		$editSuccess = $a->doEdit($text, wfMsg('nfd_keep_summary_template', $this->mTemplate));
		...
		if($editSuccess){
			...
			$comment = wfMsgHtml('nfd_keep_message', $dateStr, $votes['keepUsers'], $votes['deleteUsers']);
			$formattedComment = wfMsg('postcomment_formatted_comment', $dateStr, "NFD-Voter-Tool", "NFD Voter Tool", $comment);
...
class NFDGuardian extends SpecialPage {
	...
	function execute ($par) {
		...
		/** 
	   	 ...
	   	 */
		$wgOut->addScript(HtmlSnips::makeUrlTags('js', array('nfdGuardian.js'), 'extensions/wikihow/nfd', true));
		$wgOut->addScript(HtmlSnips::makeUrlTags('js', array('clientscript.js'), 'skins/common', true));
		$wgOut->addScript(HtmlSnips::makeUrlTags('css', array('nfdGuardian.css'), 'extensions/wikihow/nfd', true));
	...
	function getDeleteKeepVotes($nfd_id, $act_d, $act_k, $act_d_a, $act_k_a, $nfd_page) {
		...
		$req_d = NFDRuleTemplateChange::getDeleteVotesRequired();
		...
		for ($i=0; $i<($req_k); $i++) {
			$html .= self::getAvatar($keep[$i], $foundAdmin, $i);
		}
		...
		for ($i=0; $i<($req_d); $i++) {
			$html .= self::getAvatar($delete[$i], $foundAdmin, $i);
		}
	...
	function getAvatar($user_id, &$foundAdmin, $i) {
		if ($i < 2 && $user_id) {
			$u = new User();
			$u->setID($user_id);
