...
class NFDGuardian extends SpecialPage {
	...
	function getUnfinishedCount(&$dbr){
		return $dbr->selectField('nfd', 'count(*)', array('nfd_patrolled' => 0, 'nfd_status' => NFDGuardian::NFD_AVAILABLE));
	}
	...
	public static function importNFDArticles(){
		...
		foreach($results as $result){
			$hasEntry = $dbr->selectField('nfd', 'count(*)', array('nfd_page'=> $result->page_id, 'nfd_patrolled'=>0, 'nfd_status' => NFDGuardian::NFD_AVAILABLE)) > 0;
			if(!$hasEntry){
