...
class NFDRuleTemplateChange {
	...
	function processCurrentRevision($addToDb = true){
		...
		if(preg_match("@{$this->getPart()}@i", $text)){
			if($addToDb){
			...
			NFDRuleTemplateChange::markPreviousAsPatrolled($this->mArticle->getID());
			return false;
		}
	}
...
class NFDGuardian extends SpecialPage {
	...
	function execute ($par) {
		...
		// temporary for weekend
		$wgOut->addHTML('We are fixing some important bugs in the NFD Guardian.  We plan to bring this tool back on Monday July 11th.');
		return;
	...
	public static function checkArticlesInNfdTable(){
		...
		$results = array();
		$res = $dbr->select('nfd', array('nfd_id', 'nfd_page'), array('nfd_patrolled' => '0'));
		while($result = $dbr->fetchObject($res)){
			$results[] = $result;
		}
		...
		foreach($results as $result){
			...
			if($t){
				...
				if($a->isRedirect()){
					...
					NFDRuleTemplateChange::markPreviousAsPatrolled($result->nfd_page);
					$count++;
				}
				else{
