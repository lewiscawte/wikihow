...
class NFDRuleTemplateChange {
	...
	function textRemoved($part, $oldtext, $newtext) {
		if (preg_match("@{$part}@i", $oldtext) && !preg_match("@{$part}@i", $newtext)) {
			return true;
		}
		return false;
	}
	...
	function textAdded($part, $oldtext, $newtext) {
		if (!preg_match("@{$part}@i", $oldtext) && preg_match("@{$part}@i", $newtext)) {
			...
			/*if(NFDGuardian::isInUse($newtext))
				return false;
			else*/ if($nfdReason['type'] != "dup")
				return true;
			else
				return false;
		}
...
class NFDGuardian extends SpecialPage {
	...
	const NFD_WAITING = 604800; //60*60*24*7 = 7 days
	...
	static function isInUse($articleContent){
		return preg_match("@{{inuse@i", $articleContent);
	}
	...
	public static function checkArticlesInNfdTable(){
		...
		foreach($results as $result){
			...
			if($t){
				...
				/*else if(NFDGuardian::isInUse($a->getContent())){
					NFDRuleTemplateChange::markPreviousAsInactive($result->nfd_page);
					echo "Removing Inuse: " . $t->getText() . "\n";
					$count++;
				}*/
				else{
