...
//keep hidden input list of nfd choices
function updateChoices() {
	var choices = [];
	$("#nfd_options input:checked").each(function() {
...
function loadResult(result) {
	...
	if (result['done']) {
		$("#bodycontents").before("<div id='nfdcontents'>"+result['msg']+"</div>");
	}
	else {
		$("#bodycontents").before("<div id='nfdcontents'>"+result['html']+"</div>");
	}
	...
	$('#nfd_delete').click( function(e) {
		e.preventDefault();
		jQuery('#nfd_delete_confirm').dialog({
		   width: 450,
		   modal: true,
...
function getEditor(){
	...
	$.get('/Special:NFDGuardian', {
		...
		function (result) {
			...
			$('#articleEdit').show();
			$('.waiting').hide();
			document.getElementById('articleEdit').innerHTML = result['form'];
			//replace with the actual text we want in the box (doesn't contain the nfd template)
			$('#articleEdit #wpTextbox1').val(result['newContent']);
...
function getHistory(url){
	...
	$.get(url,
		function (result) {
			...
			jQuery('#history_form').submit(function(e) {
				e.preventDefault();
				//just a preview?
				...
				$.get('/Special:NFDGuardian', {
					diff: $(".history_table input[name!='oldid']:checked").val(),
					articleId: nfd_page,
					oldid: $(".history_table input[name='oldid']:checked").val()
					},
					function (result) {
						$('#article_tabs a').removeClass('on');
						$('#tab_article').addClass('on');
...
function getVoteBlock() {
	...
	$.get('/Special:NFDGuardian', {
		...
		function (result) {
			$('#nfd_voteblock,#nfd_votearrow,#nfd_voteblock_top,#nfd_voteblock_bottom').remove();

			vote_block = "<div id='nfd_votearrow'></div>" +
						"<div id='nfd_voteblock_top'></div>" +
						"<div id='nfd_voteblock'>" + result + "</div>" +
						"<div id='nfd_voteblock_bottom'></div>";

			$('#top_links').after(vote_block);

			//animate in
			$('#nfd_voteblock,#nfd_votearrow,#nfd_voteblock_top,#nfd_voteblock_bottom').animate({
				"height": "toggle",
				"opacity": "toggle"
				}, {duration: 800});
