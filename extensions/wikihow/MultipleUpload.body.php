...
class MultipleUpload extends SpecialPage {
    ...
    function __construct() {
		global $wgHooks;
		$wgHooks['UploadComplete'][] = array("MultipleUploadForm::showSuccess");
        SpecialPage::SpecialPage( 'MultipleUpload' );
    }
...
class MultipleUploadForm extends UploadForm {
	...
	function showSuccess($uploadForm) {
		...
		$sk = $wgUser->getSkin();
		$text = wfMsgWikiHtml( 'fileuploaded', $uploadForm->mLocalFile->getTitle()->getText(), "[[" . $uploadForm->mLocalFile->getTitle()->getFullText() . "]]");
		$wgOut->addHTML( $text );
		return true;
	}
