...
class FormatAppWidget extends DashboardWidget {
	...
	public function getLastContributor(&$dbr){
		...
		if($sql != "")
			$res = $dbr->select('logging', array('*'), array('log_type' => 'EF_format', $sql), 'FormatAppWidget::getLastContributor', array("ORDER BY"=>"log_timestamp DESC", "LIMIT"=>1));
		else
	...
	public function getTopContributor(&$dbr){
		...
		if($sql != "")
			$res = $dbr->select('logging', array('*', 'count(*) as C', 'MAX(log_timestamp) as log_recent'), array('log_type' => 'EF_format', 'log_timestamp >= "' . $starttimestamp . '"', $sql), 'FormatAppWidget::getTopContributor', array("GROUP BY" => 'log_user', "ORDER BY"=>"C DESC", "LIMIT"=>1));
		else
			$res = $dbr->select('logging', array('*', 'count(*) as C', 'MAX(log_timestamp) as log_recent'), array('log_type' => 'EF_format', 'log_timestamp >= "' . $starttimestamp . '"'), 'FormatAppWidget::getTopContributor', array("GROUP BY" => 'log_user', "ORDER BY"=>"C DESC", "LIMIT"=>1));
		$row = $dbr->fetchObject($res);
		$res->free();
