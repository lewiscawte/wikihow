...
class StubAppWidget extends DashboardWidget {
	...
	public function getLastContributor(&$dbr){
		$res = $dbr->select('logging', array('*'), array('log_type' => 'editfinder'), 'StubAppWidget::getLastContributor', array("ORDER BY"=>"log_timestamp DESC", "LIMIT"=>1));
	...
	public function getTopContributor(&$dbr){
		...
		$res = $dbr->select('logging', array('*', 'count(*) as C', 'MAX(log_timestamp) as log_recent'), array('log_type' => 'editfinder', 'log_timestamp > "' . $starttimestamp . '"'), 'StubAppWidget::getLastContributor', array("GROUP BY" => 'log_user', "ORDER BY"=>"C DESC", "LIMIT"=>1));
		$row = $dbr->fetchObject($res);
	...
	public function getCountDescription(){
		if($this->getBaseline() == 0)
			return wfMsg('cd-stub-countdescription');
		else
			return wfMsg('cd-stub-countdescription-adjusted');
	}

	public function  getUserCountDescription(){
		return wfMsg('cd-stub-usercount');
	}
