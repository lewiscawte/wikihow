		...
		$(".comdash-more").click(function(){
			...
			$.ajax({
				...
				success: function(data) {
					...
					height = $(leader).height() - 5; //not sure why we need 5, but it doesn't match up without
		...
		$("#cd-user-more").click(function(){
			...
			$.ajax({
				...
				success: function(data) {
					//alert(data);
					json = $.parseJSON(data);
					html = "<table><tr><th class='cd-user-start'>Act</th><th class='cd-user-widgetname'></th><th>You</th><th class='cd-user-average'>Avg wikiHow Contributor</th></tr>";
