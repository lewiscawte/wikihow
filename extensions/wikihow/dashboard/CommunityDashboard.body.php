...
class CommunityDashboard extends UnlistedSpecialPage {
	...
	public function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage('CommunityDashboard');
	}
	...
	private function makeUrlTags($type, $localFiles = array()) {
		...
		if (!COMDASH_DEBUG) {
			$url = wfGetPad('/extensions/min/f/' . join(',', $files) . '&b=extensions/wikihow/dashboard&' . WH_SITEREV);
			$ret = sprintf($fmt, $url);
		} else {
			$ret = '';
			foreach ($files as $file) {
				$ret .= sprintf($fmt, '/extensions/wikihow/dashboard/' . $file);
			}
		}
	...
	private function displayContainer() {
		...
		if ($wgUser->getID() > 0) {
			...
			$imageLink = "/Special:IntroImageAdder";
		}
		else{
			$imageLink = "/Special:Userlogin?returnto=Special:IntroImageAdder";
		}
