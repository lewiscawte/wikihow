if (WH.dashboard && !WH.dashboard.DashboardWidget) {
	WH.dashboard.DashboardWidget = function () {
		...
		this.getAvatarURL = function(img) {
			var type, param;
			var m = img.match(/([^:]*)(:(.*))/);
			if (m) {
				type = m[1];
		...
		this.getUserLink = function(username) {
			if(username == "Anonymous")
				return '<a href="/wikiHow:Anonymous:" title="wikiHow:Anonymous">' + username + '</a>';
		...
		this.getAvatarLink = function(img) {
			return '<img src="' + this.getAvatarURL(img) + '" />';
		};
