...
class Leaderboard extends SpecialPage {
	...
	function getTabs($section, $tab) {
			...
			if ($section == 'Writing') {
				...
				$tab5 = $tab=='stubs_repaired' ? "class='on'" : "";

				$tabs = " <ul> <li><a href='/Special:Leaderboard/total_edits' $tab1 >All Edits</a></li> <li><a href='/Special:Leaderboard/articles_categorized' $tab2 >Categorization</a></li> <li><a href='/Special:Leaderboard/images_added' $tab3 >Images Added</a></li> <li><a href='/Special:Leaderboard/videos_reviewed' $tab4 >Videos Reviewed</a></li> <li><a href='/Special:Leaderboard/stubs_repaired' $tab5 >Stubs</a></li></ul>";
				return $tabs;
			}
		return '';
	}
...
*/
	...
	function getArticlesRepaired($starttimestamp, $templatetype, $lb_user = '', $getArticles = false) {
		...
		if ($getArticles) {
			$key = "leaderboard:stubs_repaired:$starttimestamp:$lb_user";
		} else {
			$key = "leaderboard:stubs_repaired:$starttimestamp";
		}
		...
		$sql = "SELECT log_user, count(*) as C ".
			"FROM logging ".
			"WHERE log_type='editfinder' and log_timestamp >= '$starttimestamp' ".
			"GROUP BY log_user ORDER BY C DESC LIMIT 30";
		$res = $dbr->query($sql);
...
*/
		...
		switch( $target ) {
			...
			case('stubs_repaired'):
				$section = 'Other';
				$learnlink = '/wikiHow:LB-Stubs-Repaired';
				$columnHeader = 'Stubs Repaired';
