...
class Leaderboard extends SpecialPage {
	...
	function getTabs($section, $tab) {
			...
			if ($section == 'Writing') {
				...
				$tab5 = $tab=='repair_format' ? "class='on'" : "";

				$tabs = " <ul> <li><a href='/Special:Leaderboard/total_edits' $tab1 >All Edits</a></li> <li><a href='/Special:Leaderboard/articles_categorized' $tab2 >Categorization</a></li> <li><a href='/Special:Leaderboard/images_added' $tab3 >Images Added</a></li> <li><a href='/Special:Leaderboard/videos_reviewed' $tab4 >Videos Reviewed</a></li> <li><a href='/Special:Leaderboard/repair_format' $tab5 >Formatting</a></li></ul>";
				return $tabs;
			}
		return '';
	}
...
*/
	...
	function getQCPatrols($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		$sql = "SELECT user_name,count(*) as C
			FROM qc_vote left join wiki_shared.user ON qcv_user=user_id
			 WHERE qc_timestamp >= '$starttimestamp'
			GROUP BY user_name";
		$res = $dbr->query($sql);
	...
	function getArticlesRepaired($starttimestamp, $templatetype, $lb_user = '', $getArticles = false) {
		...
		if ($getArticles) {
			$key = "leaderboard:repair_format:$starttimestamp:$lb_user";
		} else {
			$key = "leaderboard:repair_format:$starttimestamp";
		}
		...
		$sql = "SELECT log_user, count(*) as C ".
			"FROM logging ".
			"WHERE log_type='EF_$templatetype' and log_timestamp >= '$starttimestamp' ".
			"GROUP BY log_user ORDER BY C DESC LIMIT 30";
		$res = $dbr->query($sql);
