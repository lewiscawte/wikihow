...
class Leaderboard extends SpecialPage {
	...
	function getTabs($section, $tab) {
			...
			if ($section == 'Writing') {
				...
				$tabs = " <ul> <li><a href='/Special:Leaderboard/articles_nabed' $tab1 >Articles NABed</a></li> <li><a href='/Special:Leaderboard/risingstars_nabed' $tab2 >RS NABed</a></li> <li><a href='/Special:Leaderboard/rc_edits' $tab3 >Edits Patrolled</a></li> <li><a href='/Special:Leaderboard/rc_quick_edits' $tab4 >RC Quick Edits</a></li> </ul>";
				return $tabs;
			} else if ($section == "Other") {
				$tab1 = $tab=='total_edits' ? "class='on'" : "";
				$tab2 = $tab=='articles_categorized' ? "class='on'" : "";
				$tab3 = $tab=='images_added' ? "class='on'" : "";
				$tab4 = $tab=='videos_reviewed' ? "class='on'" : "";

				$tabs = " <ul> <li><a href='/Special:Leaderboard/total_edits' $tab1 >Total Edits</a></li> <li><a href='/Special:Leaderboard/articles_categorized' $tab2 >Categorization</a></li> <li><a href='/Special:Leaderboard/images_added' $tab3 >Images Added</a></li> <li><a href='/Special:Leaderboard/videos_reviewed' $tab4 >Videos Added</a></li> </ul>";
				return $tabs;
			}
		return '';
	}
	...
	function getArticlesWritten($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		$sql = 
			...
			WHERE rc_cur_id=page_id AND rc_timestamp >= '".$starttimestamp."' AND $condstext";
		$res = $dbr->query($sql);
	...
	function getRequestedTopics($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		if ($getArticles) {
			$sql = "SELECT page_title, fe_user_text ".
 					...
 					"WHERE st_isrequest = 1 AND fe_timestamp >= '$starttimestamp' AND fe_user_text = '$lb_user' ";
		} else {
			$sql = "SELECT page_title, fe_user_text ".
 					"FROM firstedit left join page on fe_page = page_id left join suggested_titles on page_title= st_title " .
 					"WHERE st_isrequest = 1 AND fe_timestamp >= '$starttimestamp' ";
		}
...
*/
       $sql = "SELECT log_user, count(*) as C , user_name
			...
			WHERE log_type='nap' and log_timestamp >= '$starttimestamp' 
	...
	function getRCEdits($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		if ($getArticles) {
			...
			$sql = "SELECT log_user, count(*) as C ".
				"FROM logging ".
				"WHERE log_type='patrol' and log_timestamp >= '$starttimestamp' ".
				"GROUP BY log_user ORDER BY C DESC LIMIT 30;";
		}
	...
	function getRCQuickEdits($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		$sql = "SELECT rc_user_text,rc_title ".
			"FROM recentchanges ".
			"WHERE rc_comment like 'Quick edit while patrolling' and rc_timestamp >= '$starttimestamp'". 
	...
	function getTotalEdits($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		$sql = "SELECT rev_user_text,page_title,page_namespace ".
			"FROM revision,page ".
			"WHERE rev_page=page_id and page_namespace NOT IN (2, 3, 18) and rev_timestamp >= '$starttimestamp' ".
	...
	function getArticlesCategorized($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		$sql = "SELECT rc_user_text,rc_title ".
			"FROM recentchanges ".
			"WHERE rc_comment like 'categorization' and rc_timestamp >= '$starttimestamp' ". 
			"GROUP BY rc_user_text,rc_title" ;
		$res = $dbr->query($sql);
	...
	function getImagesAdded($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		$sql = "SELECT img_user_text,img_name FROM image ". 
			"WHERE img_timestamp >= '$starttimestamp'";
		$res = $dbr->query($sql);
	...
	function getVideosReviewed($starttimestamp, $lb_user = '', $getArticles = false) {
		...
		$sql = "SELECT rc_user_text,rc_title ".
			"FROM recentchanges ".
			"WHERE rc_comment like '%adding video%' and rc_timestamp >= '$starttimestamp' ".
			"GROUP BY rc_user_text,rc_title" ;
		$res = $dbr->query($sql);
	...
	function showArticles($page, $starttimestamp, $user) {
		...
		$wgOut->addHTML("  <style type='text/css' media='all'>/*<![CDATA[*/ @import '/extensions/wikihow/Leaderboard.css'; /*]]>*/</style>
			<script type='text/javascript' src='/extensions/wikihow/Leaderboard.js'></script>");
...
*/
		...
		$wgOut->addHTML("  <style type='text/css' media='all'>/*<![CDATA[*/ @import '/extensions/wikihow/Leaderboard.css'; /*]]>*/</style>
			<script type='text/javascript' src='/extensions/wikihow/Leaderboard.js'></script>");
		$wgOut->addHTML("  <style type='text/css' media='all'>/*<![CDATA[*/ @import '/extensions/wikihow/winpop.css'; /*]]>*/</style>
		...
		$wgOut->addHTML("<div id='Leaderboard'>
            <p class='leader_head'>Leaders: $columnHeader</p> <span class='leader_learn'><img src='/skins/WikiHow/images/icon_help.jpg'><a href='$learnlink'>Learn about this activity</a></span>
		");
