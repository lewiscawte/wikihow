...
class AuthorEmailNotification extends SpecialPage {
	 ...
	 *
	 **************************************/
	function notifyRisingStar($articlename, $username, $nabName) {
		$dbw = &wfGetDB(DB_MASTER);
		...
		$nabName = "<a href='User_talk:$nabName$track_talk'>$nabName</a>";
	 ...
	 *
	 **************************************/
	function notifyMod(&$article, &$user, $text, $summary, $minoredit, &$watchthis, $sectionanchor, &$flags, $revision) {
		global $wgUser;

		$author = $article->getContributors(1);
		...
		$res = $dbw->select(
					...
					array ('en_page='.$articleTitle->getArticleID()),
					"AuthorEmails");
		...
		if ($row = $dbw->fetchObject($res)) {
			...
			// debug: uncomment to override email/day limit
			//echo "diff: " . $diff . ", en_watch " . $row->en_watch . ", articleId " . $articleTitle->getArticleID() . "<br>"; $diff = 86400 * 10;
			$recipientUser = User::newFromID( $row->en_user );
			if (($row->en_watch == 1) && ($diff > 86400) ) {
				$ret = $dbw->update('email_notifications', 
						array('en_watch_email=\'' . wfTimestampNow() . '\'', 'en_last_emailsent=\'' . wfTimestampNow() . '\''), 
						array('en_page='.$articleTitle->getArticleID(), 'en_user' => $recipientUser->getID() ) );
	
				// Only create a mod email if there is an actual revision
				if (!is_null($revision)) {
					$editUser = $user->getName();
					AuthorEmailNotification::sendModEmail(&$articleTitle, &$recipientUser, &$revision, &$editUser);
				}
			}
		} else {
			wfDebug("AEN DEBUG: notifyMod [USER] [PAGE] ".$articleTitle->getArticleID()." was modified but notification email not sent. \n");
		}
		return true;
	}
	...
	function populateTrackingLinks($editType, &$titleLink, &$editLink, &$diffLink, &$articleTitle, &$revision) {
		...
		$editLink = "<a href='".$articleTitle->getFullURL('action=edit&utm_term=article_edit' . $track_title)."'>edit article</a>";
	...
	function sendModEmail(&$articleTitle, &$recipientUser, &$revision, &$editUser) {
		...
		if (stripos($comment, "Added image:") !== FALSE || stripos($comment, "Added Image using ImageAdder Tool") !== FALSE) {
			...
			$subject = wfMsg('aen_mod_subject_image', $articleTitle->getText());
			$body = wfMsg('aen_mod_body_image', $recipientUser->getName(), $titleLink, $editUser, $editLink );
			...
			$subject = wfMsg('aen_mod_subject_video', $articleTitle->getText());
			$body = wfMsg('aen_mod_body_video', $recipientUser->getName(), $titleLink, $editUser, $editLink );
			...
			$subject = wfMsg('Aen_mod_subject_categorization', $articleTitle->getText());
			$body = wfMsg('aen_mod_body_categorization', $recipientUser->getName(), $titleLink, $editUser, $diffLink, $editLink );
		} else {
			AuthorEmailNotification::populateTrackingLinks('default', $titleLink, $editLink, $diffLink, $articleTitle, $revision);
			$subject = wfMsg('aen_mod_subject_edit', $articleTitle->getText());
			$body = wfMsg('aen_mod_body_edit', $recipientUser->getName(), $titleLink, $editUser, $diffLink, $editLink );
		}		
		AuthorEmailNotification::notify($recipientUser, $from_name, $subject, $body);
		wfDebug("AEN DEBUG email notification: " . $subject . "\n\n" . $body . "\n\n");

		/* Start of some diff matching if we want to match raw edits
		$d = new DifferenceEngine($article->getTitle(),$revision->getId(), 'prev');
		echo "<pre>" . print_r($d->getDiffBody()) . "<p>";
		preg_match_all('/<span class=\"diffchange diffchange-inline\">([^<]*)<\/span>/i', $d->getDiffBody(), $matches, PREG_SET_ORDER);
		echo print_r($matches) . "<p>";
		foreach ($matches as $match) {
			if (preg_match('/\[\[Image:.*\]\]/i', $match[1]) > 0) {
				echo "Image<p>";
			} else if (preg_match('/\[\[Category:.*\]\]/i', $match[1]) > 0) {
				echo "Category<p>";
			} else if (preg_match('/{{Video:.*}}/i', $match[1]) > 0) {
				echo "Video<p>";
			} 					
		} */
	}
