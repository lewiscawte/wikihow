...
class EmailLink extends SpecialPage {
    ...
    function execute ($par) {
		...
		if (!$wgRequest->wasPosted() || !$pass_captcha) {
			...
			//$wgOut->addHTML ("<b>E-mail this link to a friend.</b>");
...
<TD>&nbsp;</TD>
<TD colspan=\"2\"><br/>
<input type='submit' name=\"wpLoginattempt\" value=\"".wfMsg('submit')."\" class=\"btn\"
...
");
			...
			if ($titleObj->getNamespace() != NS_MAIN && $titleObj->getNamespace() != NS_REQUEST && $titleObj->getNamespace() != NS_PROJECT) {
				$wgOut->errorPage('emaillink', 'emaillink_invalidpage');
				return;
			}
...
----------------
	...
	";
				...
				//$body = wfMsg('friend-sends-article-email-africa-body', $message);
				$body = "$message";
			} else {
				$body = "$message
...
----------------
	...
	foreach ($r_array as $address) {
		...
		if (!userMailer($to, $from, $subject, $body, false)) {
