...
function loadResult(result) {
	if (result['error'] != undefined) {
		...
		$("#spch-title h1").html(result['title']);
		$("#spch-title").show();
...
function disableTopButtons() {
	...
	$('#spch-skip a').addClass('clickfail');
	$('#spch-skip-arrow').css('background-position','-165px -13px');
}

function enableTopButtons() {
	//disable edit/skip choices
	$('#spch-yes').removeClass('clickfail');	
	$('#spch-skip a').removeClass('clickfail');
	$('#spch-skip-arrow').css('background-position','-165px -13px');
}
