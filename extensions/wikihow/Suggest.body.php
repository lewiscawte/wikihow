...
*/
    ...
    function execute($par){
		...
		if($index % 6 <= 5){
			while($index % 6 != 0){
				if($index % 6 == 5){
					$theHTML .= '<td><a onmouseout="button_unswap(this);" onmouseover="button_swap(this);" style="float: right; background-position: 0% 0pt;" class="button button100" onclick="document.suggest_cats.submit();">' . wfMsg('save') . '</a></td>';
			...
			$theHTML .= "</tr></table><br/><br/>" . '<a onmouseout="button_unswap(this);" onmouseover="button_swap(this);" style="float: right; background-position: 0% 0pt;" class="button button100" onclick="document.suggest_cats.submit();">' . wfMsg('save') . '</a></form>';
		
		//selected all?
		$s = $select_count == $actual_count ? "checked='checked'" : "";
		//add checkbox at the top
		$theHTML = "<input type='checkbox' id='check_all_cats' ".$s." /> <label for='check_all_cats'>All categories</label>".$theHTML;
