    ...
    function execute($par){
		...
		if ($wgRequest->wasPosted()) {
			...
			if ($wgRequest->getVal('type') == '') {
				...
				header('location:/Special:EditFinder?type='.$wgRequest->getVal('type'));
			}
		}
		...
		// get top categories
		$wgOut->addHTML("<form method='post' action='/Special:SuggestCategories' id='suggest_cats' name='suggest_cats'><input type='hidden' name='cats' value='" . $hiddencats . "'/>");
		...
		foreach ($catmap  as $cat) {
			...
			if ($safekey == 'cars-and-other-vehicles') {
				$checkkey = 'cars-&-other-vehicles';
			}
			else {
				$checkkey = $safekey;
			}
			$c = $cats && in_array($checkkey, $cats) ? "chosen" : "not_chosen";
			$s = $cats && in_array($checkkey, $cats) ? "checked='checked'" : "";
