...
class ImageHelper extends UnlistedSpecialPage {
	 ...
	 *
	 */
	function getImageSlideStrip(&$title){
		...
		$result = $wgMemc->get(wfMemcKey("ih_iss_" . $imageName));
		if ($result) {
			return $result;
		}

		if(in_array($imageName, $imageExceptions)){
			$wgMemc->set(wfMemcKey("ih_iss_" . $imageName), "");
			return "";
		}
		...
		foreach($results as $row){
			...
			if($row['il_to'] != $imageName && !in_array($row['il_to'], $imageExceptions)){
				...
				if ($image && $image->getArticleID() > 0) {
					...
					if ($file && isset($file)) {
						$thumb = $file->getThumbnail(88, -1, true, true);
						if ($thumb->getHeight() >= 40 && $thumb->getHeight() <= 77) {
		...
		if($count > 2){
			...
			$html .= $tmpl->execute('imageStack');
		}
		$wgMemc->set(wfMemcKey("ih_iss_" . $imageName), $html);
		return $html;
	}
	...
	function getImageStrip(&$title){
		...
		$imageName = $title->getDBkey();
		$result = $wgMemc->get(wfMemcKey("ih_sb_" . $imageName));
		if ($result) {
			return $result;
		}

		if(in_array($imageName, $imageExceptions)){
			$wgMemc->set(wfMemcKey("ih_sb_" . $imageName), "");
			return "";
		}
		...
		foreach($results as $row){
			...
			if($row['il_to'] != $imageName && !in_array($row['il_to'], $imageExceptions)){
				...
				if ($image && $image->getArticleID() > 0) {
					...
					if ($file && isset($file)) {
						$thumb = $file->getThumbnail(88, -1, true, true);
						if (!in_array($thumb->url, $thumbUrl) && $thumb->getHeight() >= 40 && $thumb->getHeight() <= 77) {
							$fileUrl[] = ImageHelper::getLargeThumbnail($file);
							$imageUrl[] = $image->getFullURL();
							$thumbUrl[] = $thumb->url;
							$imageTitle[] = $row['il_to'];
							$imageWidth[] = $thumb->getWidth();
		...
		if($count > 2){
			...
			$tmpl->set_vars(array(
				...
				'imageHeight' => $imageHeight, 
				'imgStrip' => true,
				'js' => HtmlSnips::makeUrlTags('js', array('slimbox2.js'), 'extensions/wikihow/gallery/slimbox-2.04/js', true),
				'css' => HtmlSnips::makeUrlTags('css', array('slimbox2.css'), 'extensions/wikihow/gallery/slimbox-2.04/css', true)
			));

			$html .= $tmpl->execute('slimbox.tmpl.php');
		}
		$wgMemc->set(wfMemcKey("ih_sb_" . $imageName), $html);
		return $html;
	}

	static function getLargeThumbnail(&$file) {
		$thumb = $file->getThumbNail(600, -1, false, true);
		return $thumb ? $thumb->getUrl() : $file->getUrl();
	}
