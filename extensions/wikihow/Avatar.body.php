...
class Avatar extends UnlistedSpecialPage {
	...
	function displayManagement() {
      ...
      if ( !in_array( 'sysop', $wgUser->getGroups() ) ) {
		...
		$wgOut->addHTML("
<script language='javascript' src='/extensions/wikihow/avatar.js'></script>
<link rel='stylesheet' media='all' href='/extensions/wikihow/cropper/cropper.css' type='text/css' />
<link rel='stylesheet' media='all' href='/extensions/wikihow/avatar.css' type='text/css' />
...
wfDebug("VOOO sql2 ".$sql."\n");
	...
	function getAvatarURL($name) {
		...
		if (file_exists($cropout))
			return  "/images/avatarOut/".$u->getID() .".jpg";
	...
	function getPicture($name, $raw = false, $fromCDN = false) {
		...
		$ret = "";
		if (!$raw) {
			$ret = "<link rel='stylesheet' media='all' href='/extensions/wikihow/avatar.css' type='text/css' /><script language='javascript' src='/extensions/wikihow/avatar.js'></script>\n";
		}
		...
		if (($wgUser->getID() == $u->getID()) && ($wgUser->getID() > 0) && ($wgTitle->getNamespace() == NS_USER)) {
			...
			if (file_exists($crop_out)) {
				$ret .= "
<img id='avatarULimg' src='/images/avatarOut/".$u->getID() .".jpg?".rand()."' height='80px' width='80px' /><br/>
...
<div id='avatarULaction'></div>
			...
			if (file_exists($crop_out)) {
				if ($raw) {
					$imgUrl = "/images/avatarOut/".$u->getID() .".jpg";
					if ($fromCDN) $imgUrl = wfGetPad($imgUrl);
					$ret .= "<img src='" . $imgUrl . "' />";
				} else {
					$ret .= "<div id='avatarID' class='avatar'>";
					$ret .= "<img id='avatarULimg' src='/images/avatarOut/".$u->getID() .".jpg' height='80px' width='80px' /></div>";
				}
			} else {
...
*/
    ...
    <script language='javascript' src='/extensions/wikihow/cropper/lib/prototype.js'></script>
    <script language='javascript' src='/extensions/wikihow/cropper/lib/scriptaculous.js?load=builder,dragdrop'></script>
    <script language='javascript' src='/extensions/wikihow/cropper/cropper.js'></script>
    <link rel='stylesheet' media='all' href='/extensions/wikihow/cropper/cropper.css' type='text/css' />";
...
<div id='avatarModalPage'>
   ...
   <div class='avatarModalContainer' id='avatarModalContainer'>
   		<img height='10' width='679' src='/skins/WikiHow/images/article_top.png' alt=''/>
		<div class='avatarModalContent'>
      <div class='avatarModalTitle'><span style='float:right;'><a onclick=\"closeButton();\"><img src='/extensions/wikihow/winpop_x.gif' width='21' height='21' alt='close window' /></a></span>". wfMsg('avatar-instructions',$wgUser->getName())."</div>
      <div class='avatarModalBody'>
         <div id='avatarUpload' >
	  ...
	  <img width='679' src='/skins/WikiHow/images/article_bottom_wh.png' alt=''/>
   </div>
</div>
<script type='text/javascript'>
...
Event.observe(window, 'load', initNonModal);
	...
	function crop() {
		...
		$doit = "convert -crop " . $width . "x" . $height . "+$x1+$y1 " .  $crop_in . " +repage -strip " . $crop_out;
		system($doit, $ret);
		if (!$ret) {
			if ($width > $imagesize) {
				$doit = "convert " . $crop_out . " -resize ".$imagesize."x".$imagesize." " . $crop_out;
				system($doit, $ret);
  				if ($ret) {
					wfDebug('trace 1: '. $ret.' from: '.$doit);
					#$wgOut->addHTML('trace 1: '. $ret.' from: '.$doit);
					#return false;
				}
			}
		} else {
      ...
      $dbw = wfGetDB(DB_MASTER);
		...
		if ($type == 'upload') {
			...
			if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
				...
				#unlink($target_path2);
				while ($count < 3){
					$doit = "convert " . $target_path . " " . $target_path2;
					system($doit, $ret);
				...
				if ($ratio != 1) {
					...
					$doit = "convert " . $target_path2 . " -resize ".$newwidth."x".$newheight." " . $target_path2;
					system($doit);
