...
<? foreach($results as $i => $result): ?>
	<div class="result">
		<? if (!empty($result['img_thumb_100'])): ?>
			...
			<? else: ?>			
				<div class='result_thumb cat_thumb'><img src="<?= $result['img_thumb_100'] ?>" /></div>
			<? endif; ?>
		<? endif; ?>

		<? if ($result['has_supplement']): ?>
			<? if (!$result['is_category']): ?>
				<a href="<?= $result['url'] ?>" class="result_link"><?= $result['title_match'] ?></a>
			<? else: ?>
				<a href="<?= $result['url'] ?>" class="result_link"><?= wfMsg('lsearch_article_category', $result['title_match']) ?></a>
			<? endif; ?>
