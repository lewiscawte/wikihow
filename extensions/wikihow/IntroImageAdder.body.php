...
class IntroImageAdder extends UnlistedSpecialPage {
	...
	function addIntroImage($v) {
		...
		if (!preg_match('/\[\[Image:(.*?)\]\]/', $intro)) {
			...
			//grab templates from article
			preg_match_all('@\{\{[^}]*\}\}@', $r->getText(), $templates);
			//put it all together
			$text = implode(" ",$templates[0]) . $imgtag . preg_replace('@\{\{[^}]*\}\}@', '', $r->getText());
...
<input type='checkbox' id='confirmModalFlag' >   ".wfMsg('iia_confirmation_dialog_flagmsg')."
	...
	function getArticleCount(&$dbr){
		...
		$total = $dbr->selectField('imageadder', 'count(*)',  array ("imageadder_last_viewed < '$timediff'","imageadder_page != 5791", "imageadder_skip < 4", "imageadder_hasimage = '0'"));
	...
	<script type='text/javascript' src='" . wfGetPad('/extensions/wikihow/standings.js?rev=') . WH_SITEREV . "'></script>
