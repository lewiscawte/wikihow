...
class Newarticleboost extends SpecialPage {
	...
	function getNABList($newbie_only = false) {
		...
		$sk = $wgUser->getSkin();
		$old = wfTimestamp(TS_MW, time() - 1 * 60);
		...
		$sql = "SELECT page_namespace, page_title, nap_timestamp , st_title, nap_page
				...
				WHERE 	page_id=nap_page AND page_is_redirect=0 AND nap_patrolled=0 AND nap_timestamp < '$old' AND tl_title is NULL  {$newbie_opt}
				GROUP BY page_title
    ...
    function execute ($par) {
		...
		if ($target == '') {
			if ($can_newbie && $wgRequest->getVal('flushnewbie') == 1) {
				...
				$dbw->update('newarticlepatrol', array('nap_newbie'=>0), array('nap_newbie'=>1));
			}
			$btn_class = "style='float: right; margin-bottom: 10px;' class='button white_button_150 submit_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);'";
			if ($can_newbie) {
				if ($do_newbie) {
			...
			if ($can_newbie) {
				$btn_class = "style='float: right; margin-top: 10px;' class='button white_button_150 submit_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);'";
				if ($do_newbie) {
					$wgOut->addHTML("<a $btn_class onclick='checkNewbieFlush();'>Flush Newbie Queue</a>");
				}
			}
		} else {
...
#print_r($_post); print_r($templatesArray); exit;
				...
				$sql = "SELECT nap_page 
						...
						WHERE nap_page <  $aid AND nap_patrolled=0 AND (nap_user_co=0 || nap_timestamp_co < '$old') AND nap_page=page_id 
							AND tl_title is null AND page_is_redirect=0 {$newbie_opt}
						ORDER BY nap_page desc
						 LIMIT 1;";
				$res = $dbr->query($sql);
