...
class Newarticleboost extends SpecialPage {
	...
	function getHighestNAB(&$dbr, $period='7 days ago'){
		...
		$sql = "SELECT *, count(nap_user_ci) as nap_count FROM `newarticlepatrol` WHERE nap_patrolled = 1 AND nap_newbie = 0 AND nap_user_ci != 0 AND nap_timestamp_ci > '" . $starttimestamp . "' GROUP BY nap_user_ci ORDER BY nap_count DESC";
		$res = $dbr->query($sql);
		$row = $dbr->fetchObject($res);
...
END
	...
	function notifyUserOfRisingStar($t, $name) {
		...
		//
		AuthorEmailNotification::notifyRisingStar($t->getText(), $name, $real_name);
	}	
}
