...
class Newarticleboost extends SpecialPage {
	...
	public function getNABCount(&$dbr) {
		...
		$sql = "SELECT count(*) as C
				  ...
				  WHERE page_id=nap_page AND page_is_redirect=0 
				    AND nap_patrolled=0 AND nap_timestamp < '$old'
					AND tl_title IS NULL AND nap_newbie = 0";
		$res = $dbr->query($sql);
	...
	public function getLastNAB(&$dbr) {
		...
		$res = $dbr->select('newarticlepatrol',
			...
			array('nap_patrolled' => 1, 'nap_newbie' => 0),
			__METHOD__,
	...
	/**
	 * Flush the queue of newbie NAB articles to do.
	 */
	private function flushNewbieQueue(&$dbw) {
		global $wgRequest, $wgOut;
		$opts = '';
		$limit = $wgRequest->getVal('flushlimit', 0);
		$limit = intval($limit);
		if ($limit) {
			$opts = "ORDER BY nap_page LIMIT " . $limit;
		}
	...
	private function displayNABList(&$dbw, &$dbr) {
		...
		if ($this->can_newbie && $wgRequest->getVal('flushnewbie', 0) == 1) {
			$this->flushNewbieQueue($dbw);
		}
		...
		// they can do newbie and want to do it
		$newbie_opt = "";
		if ($this->do_newbie)
		...
		$sql = "SELECT page_namespace, page_title, nap_timestamp, st_title, nap_page
				  ...
				  WHERE page_id = nap_page AND page_is_redirect = 0
				    AND nap_patrolled=0 AND nap_timestamp < '$old'
				    AND tl_title is NULL {$newbie_opt} {$cat}
				  GROUP BY page_title
				  ORDER BY nap_page desc
				  LIMIT $offset, $limit";
		$res = $dbr->query($sql);
		$wgOut->addHTML("<table width='100%' class='nablist'><tr class='toprow'><td>#</td><td>Article</td><td title='Was this article from the Suggested Title list?'>ST?</td><td style='width:140px;'>Created</td></tr>");
		$index = 0;
		while ($row = $dbr->fetchObject($res)) {
			$index++;
			$title = Title::makeTitle($row->page_namespace, $row->page_title);
			$s = SpecialPage::getTitleFor( 'Newarticleboost', $title->getPrefixedText() );
			$wgOut->addHTML("<tr><td>{$index}.<!--{$row->nap_page}--></td><td class='link''>" . $this->skin->makeLinkObj($s, $title->getText(), $this->do_newbie ? "newbie=1" : null ) . "</td><td class='sugg'>");
	 ...
	 */
	private static function showEditForm($title) {
		...
		if ($wgRequest->getVal('nap_submit', null) != null) {
				...
				&& $wgUser->isAllowed('move'))
			{
				...
				if( $oldTitleTalkPage->exists() ) {
					...
					$error = $oldTitleTalkPage->moveTo( $newTitleTalkPage );
				}
	...
	private function getNextUnpatrolledArticle(&$dbw, $aid, $recurse=true) {
		...
		$sql = "SELECT page_title, nap_page
				  ...
				  WHERE nap_page < $aid
					AND nap_patrolled = 0 
					AND (nap_user_co = 0 OR nap_timestamp_co < '$old')
					AND nap_page = page_id
	...
	private function displayNABConsole(&$dbw, &$dbr, $target) {
		...
		if (!$title || !$title->exists()) {
			...
			$wgOut->addHTML("Error: Article &quot;{$articleName}&quot; not found. Return to <a href='/Special:Newarticleboost'>New Article Boost</a> instead.");
			return;
		}

		$rev = Revision::newFromTitle($title);
		if (!$rev) {
			$wgOut->addHTML("Error: No revision for &quot;{$title->getFullText()}&quot;. Return to <a href='/Special:Newarticleboost'>New Article Boost</a> instead.");
			return;
		}
		$in_nab = $dbr->selectField('newarticlepatrol', 'count(*)', array('nap_page'=>$title->getArticleID())) > 0;
			$newbs  = $dbr->selectField("newarticlepatrol", "nap_newbie", "nap_page=" . $title->getArticleId());
		if (!$in_nab || ($newbs && !$this->can_newbie)) {
		...
		$wgOut->addHTML("<div class='nap_header'>$expandSpan " . wfMsg('nap_similarresults') . "</div>");
		$wgOut->addHTML("<div class='nap_body'>");
		$count = 0;
		$l = new LSearch();
		...
		$wgOut->addHTML("<div class='nap_section'>");
		$wgOut->addHTML("<div class='nap_header'>$expandSpan " . wfMsg('nap_copyrightchecker') . "</div>");
		$wgOut->addHTML("<div class='nap_body'>");
		...
		/// ARTICLE PREVIEW
		$editUrl = $this->me->getFullURL() . "?type=editform&target=" . urlencode($title->getFullText()) . "&fromnab=1";
		$wgOut->addHTML("<div class='nap_section'>");
		$wgOut->addHTML("<a name='article'></a>");
		$wgOut->addHTML("<div class='nap_header'>$expandSpan " . wfMsg('nap_articlepreview')
		...
		$wgOut->addHTML("<div class='nap_header'>$expandSpan " . wfMsg('nap_discussion')
		...
		$wgOut->addHTML("<div class='nap_header'>$expandSpan " . wfMsg('nap_userinfo')
...
END
	...
	public function execute($par) {
		...
		$wgOut->addHTML('<style type="text/css" media="all">/*<![CDATA[*/ @import "' . wfGetPad('/extensions/min/f/extensions/wikihow/newarticlepatrol.css&' . self::REVISION) . '"; /*]]>*/</style>');
		$wgOut->addHTML('<script type="text/javascript">
			var gAutoSummaryText = "' . wfMsg('nap_autosummary') . '";
			var gChangesLost = "'.wfMsg('all-changes-lost').'";
			</script>');
		$wgOut->addHTML('<script type="text/javascript" src="' . wfGetPad('/extensions/min/?f=/skins/common/clientscript.js,/extensions/wikihow/newarticlepatrol.js&' . self::REVISION) . '"></script>');
...
class NABStatus extends SpecialPage {
	...
	public function execute($par) {
		...
		$wgOut->addScript('<style type="text/css" media="all">/*<![CDATA[*/ @import "' . wfGetPad('/extensions/min/f/extensions/wikihow/newarticlepatrol.css&' . Newarticleboost::REVISION) . '"; /*]]>*/</style>');
		...
		$newarticles = $dbr->selectField(array('newarticlepatrol'),
			array('count(*)'),
			array("nap_timestamp > '$old'"),
			__METHOD__);
		$na_boosted = $dbr->selectField(array('newarticlepatrol'),
			array('count(*)'),
			array("nap_timestamp > '$old'", "nap_patrolled"=>1),
			__METHOD__);
...
class Copyrightchecker extends UnlistedSpecialPage {
	...
	public function execute($par) {
		...
		if (!$query) {
			...
			$wikitext = WikiHow::textify($wikitext);
