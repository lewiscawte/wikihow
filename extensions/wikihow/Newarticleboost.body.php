...
class Newarticleboost extends SpecialPage {
	...
	public function getNABList($newbie_only = false) {
		...
		$sql = "SELECT page_namespace, page_title, nap_timestamp , st_title, nap_page
				...
				LIMIT $offset, $limit";
		$res = $dbr->query($sql);
		$wgOut->addHTML("<table width='100%' class='nablist'><tr class='toprow'><td>#</td><td>Article</td><td title='Suggested Title?'>ST?</td><td style='width:140px;'>Created</td></tr>");
		$index = 0;
		while ($row = $dbr->fetchObject($res)) {
			$index++;
			$t = Title::makeTitle($row->page_namespace, $row->page_title);
			$s = SpecialPage::getTitleFor( 'Newarticleboost', $t->getPrefixedText() );
...
#print_r($_post); print_r($templatesArray); exit;
				...
				$nap = SpecialPage::getTitleFor( 'Newarticleboost', $t->getPrefixedText() );
			...
			$expand_span = '<span class="nap_expand">&#9660;</span>';

			/// SIMILAR RESULT
			$wgOut->addHTML("<div class='nap_section'>");
			$wgOut->addHTML("<div class='nap_header'>$expand_span " . wfMsg('nap_similarresults') . "</div>");
			...
			$wgOut->addHTML("<div class='nap_section'>");
			$wgOut->addHTML("<div class='nap_header'>$expand_span " . wfMsg('nap_copyrightchecker') . "</div>");
			$wgOut->addHTML("<div class='nap_body'>");
			...
			$wgOut->addHTML("<div class='nap_header'>$expand_span " . wfMsg('nap_articlepreview')
							. " - <a href=\"{$t->getFullURL()}\" target=\"new\">" . wfMsg('nap_articlelinktext')."</a> <img src='/skins/common/images/external.png'/>"
							. " - <a href=\"{$t->getEditURL()}\" target=\"new\">" . wfMsg('edit')."</a> <img src='/skins/common/images/external.png'/>"
							. " - <a href=\"{$t->getFullURL()}?action=history\" target=\"new\">" . wfMsg('history')."</a> <img src='/skins/common/images/external.png'/>"
							. " - <a href=\"{$t->getTalkPage()->getFullURL()}?action=history\" target=\"new\">" . wfMsg('discuss')."</a> <img src='/skins/common/images/external.png'/>"
						. "</div>");
			$wgOut->addHTML("<div class='nap_body'>");
			...
			$wgOut->addHTML("<div class='nap_header'>$expand_span " . wfMsg('nap_discussion')
							. " - <a href=\"{$tp->getFullURL()}\" target=\"new\">" . wfMsg('nap_articlelinktext')."</a> <img src='/skins/common/images/external.png'/>"
						. "</div>");
			$wgOut->addHTML("<div class='nap_body'>");
			...
			$wgOut->addHTML("<div class='nap_header'>$expand_span " . wfMsg('nap_userinfo')
							. " - <a href=\"{$user_talk->getFullURL()}\" target=\"new\">" . wfMsg('nap_articlelinktext')."</a> <img src='/skins/common/images/external.png'/>"
				. "</div>");
			$wgOut->addHTML("<div class='nap_body'>");
...
END
	...
	public function notifyUserOfRisingStar($t, $name) {
