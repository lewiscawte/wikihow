...
class Newarticleboost extends SpecialPage {
    ...
    function execute ($par) {
		...
		$wgOut->addHTML('<script src="/extensions/wikihow/newarticlepatrol.js"></script>');
		...
		if ($target == '') {
			...
			$sql = "SELECT page_namespace, page_title, nap_timestamp , st_title
					...
					WHERE 	page_id=nap_page AND page_is_redirect=0 AND nap_patrolled=0 AND nap_timestamp < '$old' AND tl_title is NULL 
					GROUP BY page_title
			...
			while ($row = $dbr->fetchObject($res)) {
				...
				$wgOut->addHTML("<tr><td>{$index}.</td><td class='link''>" . $sk->makeLinkObj($s, $t->getText()) . "</td><td class='sugg'>");
				if ($row->st_title != null) {
					$wgOut->addHTML("<img src='/extensions/wikihow/CheckMark.png' height='16' width='16'>");
				}
				$wgOut->addHTML("</td><td>");
...
#print_r($_POST); print_r($templatesArray); exit;
				...
				$sql = "SELECT nap_page 
						...
						WHERE nap_page <  $aid AND nap_patrolled=0 AND (nap_user_co=0 || nap_timestamp_co < '$old') AND nap_page=page_id 
							AND tl_title is null AND page_is_redirect=0
						ORDER BY nap_page desc
						 LIMIT 1;";
				$res = $dbr->query($sql);
				...
				if (!$err) $wgOut->redirect($nap->getFullURL());
	        ...
	        $wgOut->addHTML("<div id='nap_copyrightresults'><center><img src='/extensions/wikihow/rotate.gif'></center></div>");
			...
			$wgOut->addHTML("<div class='nap_header'>" . wfMsg('nap_discussion') 
						...
						. "</div><div id='disc_page' style='display:none;'>");
			if ($tp->getArticleID() > 0) {
			...
			if (User::getAuthorStats($first_user) < 50) {
				if ($user_talk->getArticleId() == 0) {
                	...
                	$wgOut->addHTML("<div style='border: 1px solid #eee; max-height: 300px; overflow: scroll;'>" . $wgOut->parse($rp->getText()) . "</div>");
				}
			}
...
END
	...
	function notifyUserOfRisingStar($t, $name) {
		...
		//
		AuthorEmailNotification::notifyRisingStar($t->getText(), $name);
	}	
}
...
class Copyrightchecker extends UnlistedSpecialPage {
	...
	function execute($par) {
	    ...
	    if (!$query) {
	        ...
	        $text = strip_tags($wgOut->parse($text));
	        // Pick the first 6 words of 2 unique random sentences in the first half of the article to form a query
	        $queries = array();
	        $rnums = array();
	        $rnum = -1;
	        $lines = preg_split("/[\.\n]/im", trim($text));
...
#echo $text;
#print_r($lines);
	        $max = sizeof($lines) > 10 ? floor(sizeof($lines) / 2) : sizeof($lines);
	        for ($i = 0; $i < 2; $i++) {
	            $rnum = rand(0, $max);
	            while (isset($rnums[$rnum]) && sizeof($rnums) < $max) {
	                $rnum = rand(0, $max);
	                $rnums[$rnum] = 1;
	            }
	    #echo "$rnum ($max) <br/>\n";
	            $x = min($rnum, sizeof($lines));
	            $words = preg_split("/[ ]+/", trim($lines[$x]));
	            $queries[] = '"' . implode(" ", array_slice($words, 0, min(sizeof($words),10))) . '"';
...
#echo "putting a link in '{$t1->getText()}' to '{$t2->getText()}'\n\n";
		...
		for ($i = 0; $i <10; $i++) {
			...
			if (preg_match("@^==[ ]*" . wfMsg('relatedwikihows') . "@m", $s)) {
				...
				$s .= "\n*[[{$t2->getText()}|" . wfMsg('howto', $t2->getText()) . "]]\n";
				$text = $a->replaceSection($i, $s);
				$found = true; 
				break;
			} else if (preg_match("@^==[ ]*(" . wfMsg('thingsyoullneed') . "|" . wfMsg('sources') . ")@m", $s)) {
...
#echo $text . "\n\n";
		$a->doEdit($text, "Adding related wikihow from NAB");	
	}
