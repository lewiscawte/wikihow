...
EditFinder.prototype.init = function () {
	...
	//update name/type
	jQuery('#editfinder_stats h3').html('Your '+WH.lang['app-name']+': '+g_eftype+' Stats');
...
EditFinder.prototype.edit = function (id,title) {
	...
	jQuery.ajax({
		...
		success: function(data) {
			...
			//pre-fill summary
			jQuery('#wpSummary').val("Edit from "+WH.lang['app-name']+": "+g_eftype);
...
EditFinder.prototype.upTheStats = function() {
	$('#iia_stats_today_stubs_repaired, #iia_stats_week_stubs_repaired, #iia_stats_all_stubs_repaired').each(function(index, elem) {
			$(this).fadeOut();
