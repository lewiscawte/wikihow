...
class EditFinder extends UnlistedSpecialPage {
	...
	function getNext($skip_aid) {
		...
		// mark skipped 
		if (is_null($skip_aid) === false) {
			$t = Title::newFromText($skip_aid);
			$id = $t->getArticleID();
			$dbm->update('editfinder', array('editfinder_last_viewed'=>wfTimestampNow()), array('editfinder_id'=>$id));
		...
		if (empty($pageid) === false) {
			...
			//(unless they specified an article)
			if (($this->articleInUse($pageid)) and (!$aid)) {
				//mark it as viewed
