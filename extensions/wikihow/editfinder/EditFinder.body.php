...
class EditFinder extends UnlistedSpecialPage {
	...
	function execute ($par) {
		...
		if (!in_array('staff', $userGroups) and !in_array('sysop',$userGroups)) {
		...
		if ($wgRequest->getVal( 'fetchArticle' )) {
			...
			//log it
			$params = array($efType);            
			$log = new LogPage( 'editfinder-'.$efType, false ); // false - dont show in recentchanges
		...
		$stats = new EditFinderStandingsIndividual();
