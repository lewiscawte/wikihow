...
function getContent(url,divID,w,h, isSpecial, onloadFunc) {
    ...
    http_request.onreadystatechange = function() { getData(http_request,divID,winPopW,h, onloadFunc); };
...
function postForm(url, name, method) {
    ...
    http_request.onreadystatechange = function() { getData(http_request,'winpop_outer',winPopW,winPopH); };
	if (method === 'GET'){
		url += "?" + params;
	}
...
function getData(http_request,divID,w,h, onloadFunc) {
    if (http_request.readyState==4) {
        if (http_request.status == 200) {
			//alert("making height: " + h);
            var html = '' +
			'<img height="10" width="' + w + '" src="/skins/WikiHow/images/article_top.png" alt="" style="display:block;" class="top" />' +
			'<div id="winpop_content">' + 
            '<div id="winpop_header">' +
            '<img src="/extensions/wikihow/winpop_x.gif" width="21" height="21" alt="close window" id="winpop_x" onclick="closeModal();" />' +
            '<img src="/skins/WikiHow/images/wikihow.gif" id="winpop_logo" alt="wikiHow" />' + 
            '</div>' +
            '<div id="winpop_inner" style="height:'+h+'px;">' + http_request.responseText + '<br /></div>' + 
			'</div><!--end winpop_content-->' + 
			'<img width="' + w + '" src="/skins/WikiHow/images/article_bottom_wh.png" alt="" style="display:block;" class="bottom" />';
