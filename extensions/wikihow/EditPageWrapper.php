...
/***this might not be needed anymore
		...
		**/
		...
		// only 1 access key per page for this bad boy
...
//
		...
		$wgOut->addHTML("<script language=\"JavaScript\">
			...
			<script type=\"text/javascript\" src=\"" . wfGetPad('/extensions/min/f/skins/common/clientscript.js,/skins/common/ac.js,/extensions/wikihow/importvideo.js&rev=') . WH_SITEREV . "\"></script>
		...
		// alternate methods
		$alts = array();
		$parts = preg_split("@^(===[^=]*===)@m", $steps, 0, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
		if (!preg_match("@^===@", $parts[0])) {
			// some stuff before the alt methods
			// or just plain old steps with no alt methods
			$steps = array_shift($parts);
		}
		else {
			// Nothing but alternate methods in steps.
			// Clear out the alternate methods from the steps
			$steps = "";
		}
		while (sizeof($parts) > 0) {
			if (preg_match("@^===@", $parts[0])) {
				$alt_title = trim(str_replace("=", "", array_shift($parts)));
				$alts[$alt_title] = array_shift($parts);
			} else {
				// otherwise... WTF?
				$alts[] = array_shift($parts);
			}
		}
...
//		}
		...
		if ($wgUser->getID() > 0 &&( $wgLanguageCode=='en' || $wgLanguageCode== 'fr' || $wgLanguageCode=='es' || $wgLanguageCode == 'nl' || $wgLanguageCode== 'he' )) {
			...
			$categoryHTML = "
				<style type='text/css' media='all'>/*<![CDATA[*/ @import '" . wfGetPad('/extensions/min/f/extensions/wikihow/categoriespopup.css') . "'; /*]]>*/</style>
				<div id='categories'>
		...
		$vid_alt = "";
		$video_msg = "";
		$video_button ="<a id='gatVideoImportEdit' type='button' onclick=\"changeVideo('". urlencode($wgTitle->getDBKey()) . "'); return false;\" href='#' id='show_preview_button' class='button white_button_150' onmouseover='button_swap(this);' onmouseout='button_unswap(this);' >" . wfMsg('video_change') . "</a>";
		if ($wgUser->getID() == 0) {
...
enctype=\"application/x-www-form-urlencoded\"  onSubmit=\"return checkForm();\">		");
		...
		$langKeys = array('alt_method', 'alt_title_error', 'alt_title_default');
...
<style type='text/css' media='all'>/*<![CDATA[*/ @import '" . wfGetPad('/extensions/min/f/extensions/wikihow/expandtextarea.css,/extensions/wikihow/editpagewrapper.css,/extensions/wikihow/winpop.css,/extensions/wikihow/importvideo.css&rev=') . WH_SITEREV . "'; /*]]>*/</style>
	<div id='introduction'>
		...
		<a href='#' class='button white_button_200 add_image_button' onmouseout='button_unswap(this);' onmouseover='button_swap(this);' onclick='easyImageUpload.doEIUModal(\"intro\"); return false;'><img src='" . wfGetPad('/skins/WikiHow/images/upload_image.png') . "'/>".wfMsg('eiu-add-image-to-introduction')."</a><br />
	...
	<div id='ingredients' class='{$ingredients_vis}'>
		...
		<a href='#' class='button white_button_200 add_image_button' onmouseout='button_unswap(this);' onmouseover='button_swap(this);' onclick='easyImageUpload.doEIUModal(\"ingredients\"); return false;'><img src='" . wfGetPad('/skins/WikiHow/images/upload_image.png') . "'/>".wfMsg('eiu-add-image-to-ingredients')."</a><br />
	</div>
	...
	<div id='steps'>
		...
		<a href='#' class='button white_button_200 add_image_button' style='float: left;' onmouseout='button_unswap(this);' onmouseover='button_swap(this);' onclick='return add_alt_method();'><img src='" . wfGetPad('/skins/WikiHow/images/plus-small.png') . "'/>".wfMsg('add-alt-method') . "</a>

		<a href='#' class='button white_button_200 add_image_button' onmouseout='button_unswap(this);' onmouseover='button_swap(this);' onclick='easyImageUpload.doEIUModal(\"steps\", 0); return false;'><img src='" . wfGetPad('/skins/WikiHow/images/upload_image.png') . "'/>".wfMsg('eiu-add-image-to-steps')."</a><br /><br/>

	</div>");

		// alternate methods
		$alt_index = 0; 
		foreach($alts as $alt_title => $alt_step) {
			$wgOut->addHTML("
		<div id='alt_method_{$alt_index}' class='alt_method_edit'>
			<h3>" . wfMsg('alt_method') . ": <input type='text' name='alt_title_{$alt_index}' value='{$alt_title}'/></h3>
			<div class='article_inner'>
				<div class='taButtons'>
					<p><a onclick='javascript:expandtext(\"alt_text_{$alt_index}\");' class='button expand_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);'></a></p>
					<p><a onclick='javascript:compresstext(\"alt_text_{$alt_index}\");' class='button contract_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);'></a></p>
				</div>
				<textarea name='alt_text_{$alt_index}' rows='{$wgRequest->getVal('txtarea_alt_text_' + $alt_index, 12)}' cols='100' wrap='virtual' onKeyUp=\"addNumToSteps(event, this);\" tabindex='4' id='alt_text_{$alt_index}'>{$alt_step}</textarea>
			</div>
		</div>");
			$alt_index++;
		}
	...
	<div id='tips'>
		...
		<a href='#' class='button white_button_200 add_image_button' onmouseout='button_unswap(this);' onmouseover='button_swap(this);' onclick='easyImageUpload.doEIUModal(\"tips\"); return false;'><img src='" . wfGetPad('/skins/WikiHow/images/upload_image.png') . "'/>".wfMsg('eiu-add-image-to-tips')."</a><br />
	...
	<div id='warnings'>
		...
		<a href='#' class='button white_button_200 add_image_button' onmouseout='button_unswap(this);' onmouseover='button_swap(this);' onclick='easyImageUpload.doEIUModal(\"warnings\"); return false;'><img src='" . wfGetPad('/skins/WikiHow/images/upload_image.png') . "'/>".wfMsg('eiu-add-image-to-warnings')."</a><br />
	</div>
	...
	<div id='thingsyoullneed' class='{$things_vis}'>
		...
		<a href='#' class='button white_button_260 add_image_button' onmouseout='button_unswap(this);' onmouseover='button_swap(this);' onclick='easyImageUpload.doEIUModal(\"thingsyoullneed\"); return false;'><img src='" . wfGetPad('/skins/WikiHow/images/upload_image.png') . "'/>".wfMsg('eiu-add-image-to-thingsyoullneed')."</a><br />
	...
	<div id='relatedwikihows' class='{$related_vis}'>
		...
		<div class='article_inner'>
			<div id='related_buttons'>
				<a href='#'  class='button white_button_100' onmouseover='button_swap(this);' onmouseout='button_unswap(this);' onclick='moveRelated(true);return false;' >" . wfMsg('epw_move_up') . "</a>
				<a href='#' class='button white_button_100' onmouseover='button_swap(this);' onmouseout='button_unswap(this);' onclick='moveRelated(false);return false;'>" . wfMsg('epw_move_down') . "</a>
				<a href='#' class='button white_button_100' onmouseover='button_swap(this);' onmouseout='button_unswap(this);' onclick='removeRelated(); return false;'>" . wfMsg('epw_remove') . "</a>
			</div>
			...
			<a href='#' id='add_button' class='button white_button' style='float:left;' onmouseover='button_swap(this);' onmouseout='button_unswap(this);' onclick='add_related();return false;'>" . wfMsg('epw_add') . "</a>
			<br class='clearall'/>
		</div>
	</div>
...
<span class='editHelp'><img src='" . wfGetPad('/skins/WikiHow/images/icon_help.jpg') . "' /> {$edithelp}</span>
