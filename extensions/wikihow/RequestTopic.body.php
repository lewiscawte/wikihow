...
class RequestTopic extends SpecialPage {
    ...
    function execute ($par) {
		...
		global $wgScript, $wgParser, $wgFilterCallback;
		...
		if (!$wgRequest->wasPosted()) {
			...
			//$titles = Title::matchKeyTitles($topic);
			...
			if ($title->getArticleID() <= 0) {
				...
				//$wgParser->mStripState = array();
				//$wgParser->replaceInternalLinks ($details); //bullshit workaround
				$options = ParserOptions::newFromUser( $wgUser );
				$wgParser->parse($details, $title, $options);
	
			} else {
				// TODO: what to do here? give error / warning? append details? 
				// this question has already been asked, if you want to ask a slightly different
				// question, go here:
				//
			}
...
class ListRequestedTopics extends SpecialPage {
	...
	/*
		...
		$sql = "SELECT page_title, page_touched  from "
		. " page where page_namespace = " . NS_ARTICLE_REQUEST 
	...
	*/
	...
	//		if ($title && $title->getArticleID() > 0) {
			if ($title) {
				...
				//$wgOut->addHTML( "<td bgcolor=\"$bgcolor\" width=\"15%\"><font size=-2>$real_name</td></font></td>
			...
			if ($wgUser->isSysop()) {
				...
				if (!$found) 
				//$wgOut->addHTML(" - " .  $sk->makeLinkObj($request,wfMsg('edit_title'), "action=move") );
