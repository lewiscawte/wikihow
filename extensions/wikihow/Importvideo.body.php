...
class Importvideo extends SpecialPage {
    ...
    function execute ($par) {
		...
		$wgOut->addHTML("<script type='text/javascript' src='/extensions/wikihow/importvideo.js'> </script>	");
		$wgOut->addHTML('<style type="text/css" media="all">/*<![CDATA[*/ @import "/extensions/wikihow/importvideo.css"; /*]]>*/</style>');
...
*/
	...
	function updateVideoArticle($title, $text) {
		$a = new Article($title);	
	    if ($title->getArticleID() > 0) {
        	$a->updateArticle($text, wfMsg('importvideo_addingvideo_summary'), false, false);
        } else {
            $a->insertNewArticle($text, wfMsg('importvideo_addingvideo_summary'), false, false);
        }
	}
	function updateMainArticle($target) {
		global $wgOut, $wgRequest;
		...
		if ($update)
			$a->updateArticle($newtext,  wfMsg('importvideo_addingvideo_summary'), false, $watch);
		else 
			$a->insertNewArticle($newtext,  wfMsg('importvideo_addingvideo_summary'), false, $watch);
		...
		if ($wgRequest->getVal("popup") == "true") {
			...
			function onLoad() {
				...
				if (summary.value != '') 
					summary.value += ',  " . ($update ? wfMsg('importvideo_changingvideo_summary') :  wfMsg('importvideo_addingvideo_summary')) . "';
				else
					summary.value = '" . ($update ? wfMsg('importvideo_changingvideo_summary') :  wfMsg('importvideo_addingvideo_summary')). "';
				closeModal();
			}
