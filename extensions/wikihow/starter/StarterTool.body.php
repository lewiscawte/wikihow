...
class StarterTool extends UnlistedSpecialPage {
	...
	public static function getSentence() {
		...
		$sent .= '<input type="hidden" value="'.$the_title.'" id="test_title">';
	...
	function execute ($par) {
		...
		if ($wgRequest->getVal('edit')) {
			...
			$t = Title::newFromText($wgRequest->getVal('test_title'));
			$a = new Article($t);
		...
		elseif ($wgRequest->getVal('getsome')) {
			...
			$a->doEdit($r->getText(),'Auto-rollback from Starter Tool.',EDIT_UPDATE);
			$a->markpatrolled();
			...
			$wgOut->addScript("<script type='text/javascript' src='". wfGetPad('/extensions/min/f/extensions/wikihow/starter/starter.js')."'></script>");
		...
		$wgOut->setHTMLTitle(wfMsg('app-name').' - wikiHow');
