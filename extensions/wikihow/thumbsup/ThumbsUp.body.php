	...
	function thumb($revisionId, $thumbRecipientId, $thumbRecipientText, $pageId, &$recipients) {
		...
		*/ 
		if($wgUser->isLoggedIn() && !self::isThumbedByCurrentUser($revisionId) && $wgUser->getID() != $thumbRecipientId && self::isThumbableTitle($pageId)) {
			...
			if (is_object($t)) {
				if (!in_array($recipientIdOrIP, $recipients)) {
					// Add a log entry
				...
				// Send a talk page message to recipient of thumbs up (only if they have the option set) for logged in users
				if ($thumbRecipientId > 0) {
					...
					if (intVal($thumbsTalkOption) == 0 && !in_array($recipientIdOrIP, $recipients)) {
						self::notifyUserOfThumbsUp($t, $thumbRecipientId, $revisionLink, $revisionId, $thumbRecipientText); 
					}
				}
				else {
					// Send one talk page message to an anon user
					if (!in_array($recipientIdOrIP, $recipients)) {
						self::notifyUserOfThumbsUp($t, $thumbRecipientId, $revisionLink, $revisionId, $thumbRecipientText); 
					}
				}
