...
class Videoadder extends SpecialPage {
	...
	function getArticleCount(&$dbr){
		...
		$total = $dbr->selectField('videoadder', 'count(*)',  array ("va_inuse IS NULL or va_inuse < '{$ts}'", "va_skipped_accepted IS NULL", "va_page NOT In (5, 5791)", "va_template_ns IS NULL"));
		return $total;
	}
	...
	function setSideWidgets() {
		...
		$today 	= $dbr->selectField('videoadder', 'count(*)', array('va_user'=>$wgUser->getID(), "va_timestamp >= '{$ts}'", "(va_skipped_accepted = '0' OR va_skipped_accepted = '1')"));
		$ts 	= substr(wfTimestamp(TS_MW, time() - 7 * 24 * 3600), 0, 8) . "000000";
		$week 	= $dbr->selectField('videoadder', 'count(*)', array('va_user'=>$wgUser->getID(), "va_timestamp >= '{$ts}'", "(va_skipped_accepted = '0' OR va_skipped_accepted = '1')"));
		$total  = $dbr->selectField('videoadder', 'count(*)', array('va_user'=>$wgUser->getID(), "(va_skipped_accepted = '0' OR va_skipped_accepted = '1')"));
		$w = "<div class='iia_stats'><h3>" . wfMsg('va_yourstats'). "</h3>
			<table id='va_stats'>
				...
				<tr><td>" . wfMsg('va_total') . "</td><td class='va_stat'><span id='vastats_total'>" . $total . "</span></td></tr>
