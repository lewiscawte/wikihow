...
class Videoadder extends SpecialPage {
	...
	function getHighestVA(&$dbr, $period='7 days ago'){
		...
		$sql = "SELECT *, count(va_user) as va_count FROM `videoadder` WHERE va_skipped_accepted = 0 AND va_timestamp > '" . $starttimestamp . "' GROUP BY va_user ORDER BY va_count DESC";
		$res = $dbr->query($sql);
		$row = $dbr->fetchObject($res);
