...
class WikiHow {
	 ...
	 */	
	 function useWrapperForEdit ($article) {
		...
		foreach ($matches[0] as $m ){
			$x = trim(str_replace("=", "", $m));
			if ($x == wfMsg('steps')) {
				$foundSteps = true;
			}
	 		if (!in_array($x, $good_titles)) {
				return false;
			}
		}
		return $foundSteps;
	...
	function loadFromRequest ($request) {
		...
		foreach ($request->getValues() as $key=>$val) {
			if (preg_match("@alt_title@", $key)) {
				$index = preg_replace("@alt_title_@", "", $key);
				$alt_steps = $request->getVal('alt_text_' . $index);
				if (trim(preg_replace("@#@", "", $alt_steps)) == "") {
					// there were no steps entered anyway
					continue;
				}
				$alt_methods .= "=== " . $val . " ===\n" . $request->getVal('alt_text_' . $index) . "\n";
			}
		}
		$steps .= "\n\n" . trim($alt_methods);
		$whow->setSteps($steps);
