...
class Randomizer extends SpecialPage {
	...
	/**
	 * Check whether the intro wikitext given has an intro image
	 */
	private static function hasIntroImage($intro) {
		return preg_match('@\[\[Image:@', $intro) > 0;
	}
	 ...
	 */
	private static function getNumStepsImages($steps) {
	 ...
	 */
	private static function getWikitext(&$dbr, $title) {
	 ...
	 */
	private static function loadRows(&$dbr, $sql, $cachekey, $fname) {
		global $wgMemc;
...
CREATE TABLE page_randomizer (
	...
	pr_updated VARCHAR(14) NULL DEFAULT '',
	PRIMARY KEY(pr_id),
	INDEX(pr_random)
) Engine=InnoDB;
 ...
 */
	...
	private static function processArticles($from) {
		...
		foreach ($articles as $i => $article) {
			...
			if ($wikitext) {
				$intro = ArticleMetaInfo::getIntro($wikitext);
				$steps = ArticleMetaInfo::getStepsSection($wikitext);
				...
				if (self::excludeViaTemplates($intro)) {
					...
					//if (isset($views[$id]) && self::hasIntroImage($intro)) {
					//	$reason[] = 'views-and-intro-image';
					//}
