	...
	var onloadFunc = function () {
		...
		jQuery('#eiu-dialog-inner').load(url, function (data) {
			...
			if (!wgUserName) {
				...
				jQuery('#eiu_uploadform').attr('disabled', 'disabled');
				jQuery('#eiu-image-search').attr('disabled', 'disabled');
				jQuery('#eiu-step-selection').attr('disabled', 'disabled');
			}
...
// 
// show eiu in non modal for intro image adder
// IIA
//
EasyImageUpload.prototype.doEIU_IIA = function (title, searchterms, DIV, origin, currentStep) {

//alert('title '+title+' --- searchterms '+searchterms);
	this.m_title = title;
	this.m_product = 'iia';
	...
	var url = '/' + wfMsg('special-easyimageupload') + '?article-title=' + encodeURIComponent(title) +'&intro-image-adder=1&searchterms=' + encodeURIComponent(searchterms);
	jQuery.get(url, function(data) {
		jQuery('#' + DIV).html(data);

		var html = that.htmlBusyWheel();
		jQuery('#eiu_recently_uploaded').html(html);
		that.loadImages('flickr', searchterms, 1);
		jQuery('#iia_introlink').click(function(){
			if(jQuery('#iia_articleintro').is(':visible')){
				jQuery('#iia_articleintro').hide();
				jQuery('#iia_more').addClass('off');
				jQuery('#iia_more').removeClass('on');
			}
			else{
				jQuery('#iia_articleintro').show();
				jQuery('#iia_more').addClass('on');
				jQuery('#iia_more').removeClass('off');
			}
			return false;
		});
...
EasyImageUpload.prototype.storeImageDetails = function (formName, postData) {
	...
	jQuery.post(url, postData, function(html) {
		//IIA
		if (wgTitle == 'IntroImageAdder') {
			var json = jQuery.parseJSON(html);

			if (json['status'].match(/SUCCESS/)) {
				//introImageAdder.displayConfirmation(postData);
				introImageAdder.displayConfirmation(json);
				//IIA regardless of return code keep going and add more images...
				if (wgTitle == 'IntroImageAdder') {
					introImageAdder.getArticle();
				}
			} else {
				// do something on error, probably just log it and move on
			}
		} else {
		...
		var callback = function () {
			...
			if (node.length) {
				if (that.m_origin == 'callback') {
					...
					that.m_postCallback(imageDetails);
				} else {
...
EasyImageUpload.prototype.insertImage = function (src, imgID) {
	...
	//IIA
	if (wgTitle == 'IntroImageAdder') {
		var postData = 'intro-image-adder2=1&src=' + src + '&img-details=' + encodeURIComponent(imageDetails) + '&iiatitle=' + encodeURIComponent(this.m_title);
	} else {
...
EasyImageUpload.prototype.loadImagesFlickr = function (query, page) {
	...
	if (this.m_product == 'iia') {
		params += '&intro-image-adder=1';
	}
	var that = this;
...
// utility function to set the find box links to bold (or normal) depending
// what was clicked
EasyImageUpload.prototype.cssSetFindLinksWeight = function (wikiBold) {
	if (wikiBold) {
		var wb = true;
		var fb = false;
	} else {
	...
	jQuery('#eiu-this-wiki-link').css('font-weight', (wb ? 'bold' : 'normal'));
	jQuery('#eiu-flickr-link').css('font-weight', (fb ? 'bold' : 'normal'));
}
...
EasyImageUpload.prototype.loadSlider = function () {
	...
	var CENTER_MAX_DIM = 400;
	if (eiuOrigFullWidth <= CENTER_MAX_DIM) {
		eiuAutomaticWidthPercentCenter = 100;
	} else {
		eiuAutomaticWidthPercentCenter = Math.round(100 * 400 / eiuOrigFullWidth);
	}
