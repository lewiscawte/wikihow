...
class MQG extends UnlistedSpecialPage {
	...
	private function logEvent() {
		...
		$dbw = wfGetDB(DB_MASTER);
		$event = explode("-", trim($dbw->strencode($wgRequest->getVal('log'))));
		if (sizeof($event)) {
			$dbw->insert('event_logger', array('el_key' => 'mqg', 'el_type' => $event[0], 'el_value' => $event[1], 'el_timestamp' => wfTimestamp(TS_MW)), 'MQG::logEvent');
		}
		wfProfileOut(__METHOD__);
	}
}
