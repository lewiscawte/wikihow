	...
	function showForm() {
		...
		$dt = $wgTitle->getTalkPage();
     	...
     	$s .= "
			...
			function rateArticle(t, r) {
				var requester;
				try {
					requester = new XMLHttpRequest();
				} catch (error) {
					try {
						requester = new ActiveXObject('Microsoft.XMLHTTP');                } catch (error) {
						return false;
					}
				}
				...
				if (r == 0) {
					y.innerHTML = 'Thanks. Please <a href=\"" . htmlspecialchars($dt->getFullURL()) . 
					"#post\">click here</a> to provide specific details on how we can improve this article.';
				} else {
           	...
           	$s .=" <p>Was this article accurate?</p>
            <table style='width:100%;'><tr><td align='right'><a href='javascript:rateArticle(".$page_id.",1);' id='gatAccuracyYes' class='button white_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);'>Yes</a></td><td align='left'><a href='javascript:rateArticle(".$page_id.",0)' id='gatAccuracyNo' class='button white_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);'>No</a></td></tr></table>";
		return $s;
	}
