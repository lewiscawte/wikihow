...
$wgHooks['ArticleSaveComplete'][] = array("sendModNotification");


function sendModNotification(&$article, &$user, $text, $summary, $minoredit, &$watchthis, $sectionanchor, &$flags, $revision) {
	$articleTitle = $article->getTitle();
	
	if (!$articleTitle) return true;
	try {
		if ( $articleTitle && $articleTitle->getArticleID() != 0 )  {
			AuthorEmailNotification::notifyMod(&$article, &$user, $text, $summary, $minoredit, &$watchthis, $sectionanchor, &$flags, $revision, &$status, $baseRevId, &$redirect);
		}
	} catch (Exception $e) {
	}
	return true;
