...
require_once("$IP/extensions/wikihow/SphinxSearch.php");
...
class SuggestionSearch extends UnlistedSpecialPage {
	...
	function matchKeyTitles($text, $limit = 10) {
		$searcher = new SphinxSearch();
		$matches = $searcher->searchSuggestedTitles($text, $limit);
		$titles = array();
		if (!$matches['error'] && $matches['results']['matches']) {
			foreach ($matches['results']['matches'] as $match) {
				$titles[] = array($match['attrs']['title']);
			}
		}
		return $titles;
