...
function wfSajaxSearchArticleFCKeditor( $term ) {
	...
	$term1 = str_replace( ' ', '-', $wgContLang->ucfirst( $term ) );
	$term2 = str_replace( ' ', '-', $wgContLang->lc( $term ) );
	$term3 = str_replace( ' ', '-', $wgContLang->uc( $term ) );
	$term4 = str_replace( ' ', '-', $wgContLang->ucfirst( $term2 ) );
	$term = $term1;
