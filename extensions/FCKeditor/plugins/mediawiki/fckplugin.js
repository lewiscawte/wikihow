...
FCK.DataProcessor =
{
	...
	{
//alert('calling convert to html' + data);
						...
						case 'span' :
							switch ( htmlNode.className )
							{
								...
								case 'fck_mw_template' :
									stringBuilder.push( FCKTools.HTMLDecode(htmlNode.innerHTML).replace(/fckLR/g,'\r\n') ) ;
//XX VOOO
//alert('got to template to wikitext ' + FCKTools.HTMLDecode(htmlNode.innerHTML).replace(/fckLR/g,'\r\n'));
//alert('original was '+htmlNode.innerHTML);
									return ;
	...
	AddItems : function( contextMenu, tag, tagName )
	{
		if ( tagName == 'IMG' )
		{
			if ( tag.getAttribute( '_fck_mw_template' ) )
			{
				...
				contextMenu.AddItem( 'MW_Template', FCKLang.wikiMnuTemplate || 'Template Properties Vu' ) ;
			}
