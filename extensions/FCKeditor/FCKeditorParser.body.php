...
wfDebug('VOOO inner' . $inner . "\n");
					$this->fck_mw_strtr_span['href="Fckmw'.$this->fck_mw_strtr_span_counter.'fckmw"'] = 'href="'.$inner.'"';
					if($opened <= $closed) {						// {{template}} is NOT in [] or [[]]
						//$this->fck_mw_strtr_span['Fckmw'.$this->fck_mw_strtr_span_counter.'fckmw'] = '<div id="whTemplate" onmouseover="alert(\'hovering\');" ><span class="fck_mw_template">'.str_replace(array("\r\n", "\n", "\r"),"fckLR",$inner).'</span></div>';
...
wfDebug('VOOO whatever' . $this->fck_mw_strtr_span['Fckmw'.$this->fck_mw_strtr_span_counter.'fckmw'] . "\n");
					$startingPos = $pos + 19;
					$this->fck_mw_strtr_span_counter++;
				}
				$lastSum = $sum;
...
//wfDebug('VOOO parseroutput: '.$parserOutput."\n");
		return $parserOutput;
	}
