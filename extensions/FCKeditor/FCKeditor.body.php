...
class FCKeditor_MediaWiki {
	...
	function canUse() {
		...
		if (!$wgUser->isAllowed('wysiwygtesting') || !$wgUser->getOption('wysiwyg') 
			|| !$wgTitle || $wgTitle->getNamespace() != NS_MAIN
		) {
			return false;
		}
		return true;
	...
	public function registerHooks() {
		global $wgHooks, $wgExtensionFunctions, $wgVersion, $wgUser;
...
oFCKeditor.Config["CustomConfigurationsPath"] = "'.$wgScriptPath.'/'.$wgFCKEditorExtDir.'/fckeditor_config.js" ;';
		if ($wgContLang->isRTL()) {
			$script .= 'oFCKeditor.Config["EditorAreaCSS"] = "'
					.$wgScriptPath.'/'.$wgFCKEditorExtDir.'/css/fckeditor-rtl.css,/skins/WikiHow/newskin.css,"'
					.$wgScriptPath.'/'.$wgFCKEditorExtDir.'/css/fckeditor.css'
				.' ;';
		}
		else {
			$script .= 'oFCKeditor.Config["EditorAreaCSS"] = "'
				."{$wgScriptPath}/{$wgFCKEditorExtDir}/css/fckeditor.css,"
				. '/skins/WikiHow/newskin.css '
				.'";';
		}
		$script .= '
...
function onLoadFCKeditor()
{
	...
	if ( realTextarea )
	{
		...
		height = ( !height || height < 250 ) ? 250 : height ;
	...
	document.getElementById('wpSummary').value += "WYSIWYG";
}
