 ...
 *
 *   $Id: search.php,v 1.3 2006/05/31 11:24:12 cvsuser Exp $
 *
 *
 ***************************************************************************/
...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' )
	{
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || ( $search_author != '' && $search_keywords == '' )  )
		{
			...
			else
			{
				...
				$sql = "SELECT user_id
					FROM " . USERS_TABLE . "
					WHERE user_name LIKE '" . str_replace("\'", "''", $search_author) . "'";
				...
				for ($i = 0; $i < count($search_id_chunks); $i++)
				{
					...
					else
					{
						...
						if ( $search_author != '' )
						{
							...
							$where_sql .= " AND u.user_id = p.poster_id AND u.user_name LIKE '$search_author' ";
						}
		...
		if ( $result = $db->sql_query($sql) )
		{
			...
			if ( false && count($delete_search_ids) )
			{
	...
	if ( $search_results != '' )
	{
		...
		{
			$sql = "SELECT pt.post_text, pt.bbcode_uid, pt.post_subject, p.*, f.forum_id, f.forum_name, t.*, u.user_name, u.user_id, u.user_sig, u.user_sig_bbcode_uid  
		...
		{
			$sql = "SELECT t.*, f.forum_id, f.forum_name, u.user_name, u.user_id, u2.user_name as user2, u2.user_id as id2, p.post_username, p2.post_username AS post_username2, p2.post_time 
