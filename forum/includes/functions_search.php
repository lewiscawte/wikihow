...
*
*     $Id: functions_search.php,v 1.1 2005/08/17 14:47:43 Administrator Exp $
*
****************************************************************************/
...
function username_search($search_match)
{
	...
	if ( !empty($search_match) )
	{
		...
		$sql = "SELECT user_name 
			FROM " . USERS_TABLE . " 
			WHERE user_name LIKE '" . str_replace("\'", "''", $username_search) . "' AND user_id <> " . ANONYMOUS . "
			ORDER BY user_name";
		if ( !($result = $db->sql_query($sql)) )
		{
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			do
			{
				$username_list .= '<option value="' . $row['user_name'] . '">' . $row['user_name'] . '</option>';
			}
			while ( $row = $db->sql_fetchrow($result) );
		}
		else
