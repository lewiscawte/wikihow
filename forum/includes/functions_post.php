 ...
 *
 *   $Id: functions_post.php,v 1.4 2007/09/20 20:56:49 cvsuser Exp $
 *
 *
 ***************************************************************************/
...
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, &$post_username, &$post_subject, &$post_message, &$poll_title, &$poll_options, &$poll_length)
{
	...
	//XX block non-wikihow urls from being posted by anons
	if ($userdata['user_id'] == ANONYMOUS) {
		preg_match_all("/http:\/\/[^] \n'\"\>\<]*/im", $post_message, $matches);
		foreach ($matches[0] as $m) {
			if (preg_match("/^http:\/\/[a-z]*.wikihow/im", $m) == 0) {
				message_die(GENERAL_MESSAGE, "Posting external links requires a login.");
				return;
			}
		}
	}
...
function delete_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id)
{
	...
	$msg = "{$userdata['username']}({$userdata['user_id']}) deletes postid($post_id) pollid($poll_id) topicid ($topic_id) forumid($forum_id) postedby ({$post_data['poster_id']})";
	$sql = "SELECT post_text from phpbb_posts_text WHERE post_id=$post_id;";
    if ($result = $db->sql_query($sql)) {
		 if ($row = $db->sql_fetchrow($result)) {
			$msg .= " text: \n\n {$row['post_text']}";
		}
	...
	$sql = "INSERT INTO phpbb_deletelog(log) VALUES (' " . str_replace("'", "''", $msg) . "');";
	if (!$db->sql_query ($sql)) {
	}
