 ...
 *
 *   $Id: posting.php,v 1.2 2009/04/14 15:54:08 jderouin Exp $
 *
 *
 ***************************************************************************/
...
switch ( $mode )
{
	...
	case 'poll_delete':
		...
		$select_sql = ( !$submit ) ? ", t.topic_title, p.enable_bbcode, p.enable_html, p.enable_smilies, p.enable_sig, p.post_username, pt.post_subject, pt.post_text, pt.bbcode_uid, u.user_name, u.user_id " : '';
...
{
	$bbcode_on = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['disable_bbcode']) ) ? 0 : TRUE ) : ( ( $userdata['user_id'] == ANONYMOUS ) ? $board_config['allow_bbcode'] : true );
}
...
{
	$smilies_on = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['disable_smilies']) ) ? 0 : TRUE ) : ( ( $userdata['user_id'] == ANONYMOUS ) ? $board_config['allow_smilies'] : true );
}
...
else if ( $submit || $confirm )
{
	...
	if ( $error_msg == '' )
	{
		...
		// TDEROUIN : change this to redirect back to message automagically
		if ( $post_id )
		{
			$redirect = "viewtopic.$phpEx?" . POST_POST_URL . "=$post_id";
			$post_append = "#$post_id";
...
else
{
	...
	if ( $mode == 'newtopic' )
	{
		...
		$username = ($userdata['session_logged_in']) ? $userdata['user_name'] : '';
		$poll_title = '';
		$poll_length = '';
		$subject = '';
		$message = '';
	...
	else if ( $mode == 'quote' || $mode == 'editpost' )
	{
		...
		if ( $mode == 'quote' )
		{
			...
			$quote_username = ( trim($post_info['post_username']) != '' ) ? $post_info['post_username'] : $post_info['user_name'];
