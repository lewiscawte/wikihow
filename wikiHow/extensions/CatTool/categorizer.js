$(function() {
	...
	$('.cat_breadcrumb').live('click', function(e) {
		e.preventDefault();
		var cat = $(this).text();
	...
	$('.cat_breadcrumb_add').live('click', function(e) {
		e.preventDefault();
	...
	$('#cat_search_button').live('click', function(e) {
		e.preventDefault();
		$('#cat_search').autocomplete( "search" , $('#cat_search').val());
	});

	$('#cat_search').live('keypress', function(e) {
		/* ENTER PRESSED*/
		if (e.keyCode == 13) {
			$('#cat_search').autocomplete( "search" , $('#cat_search').val());
		}
	...
	function addCategory(category) {
		...
		if(isValid(category)) {
			...
			var closeSpan = $("<span/>").addClass("cat_close");
			$("<span/>").text(category).append(closeSpan).addClass("cat_category ui-widget-content ui-corner-all cat_nodisplay")
	...
	function updateButtonStates() {
		...
		if (numCats) {
			$("#cat_save, #cat_save_editpage").removeClass('cat_button_save_disabled').addClass('cat_button_save');
		} else {
			$("#cat_save, #cat_save_editpage").removeClass('cat_button_save').addClass('cat_button_save_disabled');
		}
	}
	...
	$('#cat_introlink').live('click', function(e) {
		e.preventDefault();
	...
	$('#cat_cancel').live('click', function (e) {
		e.preventDefault();
		$('#dialog-box').dialog('close');
	});

	$('#cat_skip').live('click', function (e) {
		e.preventDefault();
		var aid = $('#cat_aid').text();
		$.get(catUrl + '?a=skip&id=' + aid, function(result) {
			$('#cat_head_outer').replaceWith(result);
	...
	$('#cat_save_editpage').live('click', function(e) {
		e.preventDefault();

		if ($(this).hasClass('cat_button_save_disabled')) {
			return;
		}
		...
		$($('#cat_list .cat_category').get().reverse()).each(function(i, cat) {
			$('input[name=topcategory' + i + ']').val($(cat).text());
			$('input[name=category' + i + ']').val($(cat).text());
	...
	$('#cat_save').live('click', function (e) {
		e.preventDefault();
		saveCategories(this);
		incrementStats();
	});
	...
	function saveCategories(context) {
		if ($(context).hasClass('cat_button_save_disabled')) {
			return;
		}
	...
	$(".cat_close").live('click', function(e) {
	...
	$(".cat_subcat").live('click', function(e) {
		e.preventDefault();
