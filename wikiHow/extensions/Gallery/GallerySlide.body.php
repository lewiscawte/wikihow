...
class GallerySlide extends UnlistedSpecialPage {
	...
	function formatRelated($t) {
		...
		if ($t && $t->exists()) {		
			...
			$sk = $wgUser->getSkin();
			$img = $sk->getGalleryImage($t, 238, 139);
...
/*						$dbw = wfGetDB(DB_MASTER);
	...
	function getImagesFromSteps($articleId,$revid=''){
		...
		$result = $wgMemc->get(wfMemcKey("GallerySlide_getImagesFromSteps" . $articleId . "_" . $revid));
		if ($result) {
			return $result;
		}
		...
		if ($rev) {
			...
			if ($steps) {
				...
				$results = $results[0];
				if ($revid != '') {
					$wgMemc->set(wfMemcKey("GallerySlide_getImagesFromSteps" . $articleId . "_" . $revid), $results);
				}
			}
		}
	...
	function getImageFromIntro($articleId,$revid=''){
		...
		$result = $wgMemc->get(wfMemcKey("GallerySlide_getImageFromIntro" . $articleId . "_" . $revid));
		if ($result) {
			return $result;
		}
		...
		if ($rev) {
			...
			if ($intro) {
				...
				$results = $results[0];
				if ($revid != '') {
					$wgMemc->set(wfMemcKey("GallerySlide_getImageFromIntro" . $articleId . "_" . $revid), $results);
				}
			}
		}
	...
	public function thumbnail_exists( $file ) {
		...
		$thumbName = $file->thumbName( $normalisedParams );	
		$thumbPath = $file->getThumbPath( $thumbName, $params );
