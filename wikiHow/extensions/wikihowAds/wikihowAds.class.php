...
class wikihowAds {
	...
	public static function getAdUnitPlaceholder( $num, $isLinkUnit = false, $postLoad = true ) {
		...
		if ( $postLoad && $adID === 'au4' ) {
			return self::getPostLoadAd( $adID, $unit );
		} else {
			return $unit;
		}
	}
...
EOHTML;
			...
			if( !$found && $fe == 'WRM' ) {
				...
				if ( $r != null ) {
					...
					if ( preg_match( "/^(.*?)== " . wfMsg( 'tips' ) . '/ms', $text1, $sectionmatch ) ) {
						...
						$num_step_photos = preg_match_all( "/\[\[(:?Image|$fileNamespaceName):/im", $text1, $matches );
	...
	function getCustomGoogleChannels( $type, $use_chikita_sky ) {
		...
		if ( !class_exists( 'FeaturedArticles' ) ) {
			require_once( "$IP/extensions/FeaturedArticles/FeaturedArticles.php" );
		}
	...
	function getInternationalChannels() {
		...
		if ( $wgTitle->getNamespace() == NS_MAIN ) {
			...
			if ( in_array( $fe, array( 'Wilfredor', 'WikiHow Traduce' ) ) ) {
			   	$channels[] = '3957522669';
			} elseif( $fe == 'WikiHow Übersetzungen' ) {
			   	$channels[] = '6309209598';
			} elseif( $fe == 'Traduções wikiHow' ) {
				$channels[] = '3705134139';
			}
		}
