...
class TweetModel {
	...
	function getTweets( $lastTweetId = null ) {
		$sql = 'SELECT ' . self::TWEETS_TABLE . '.id,
				...
				FROM ' . $this->dbr->tableName( 'twitterreplier_tweets' ) . '
				...
				AND ( ' . self::TWEETS_TABLE . '.reply_status = 0 OR ' .
				self::TWEETS_TABLE . '.updated_on < ' .
				$this->dbr->addQuotes( date( 'Y-m-d H:i:s', strtotime( self::TIME_LOCK ) ) ) . ')';
	...
	function insertTweet( $tweet ) {
		...
		if ( !empty( $tweet ) && is_object( $tweet ) ) {
			...
			try {
				...
				// is there some sort of a handler?
				echo $e->getMessage();
			}
		} else {
			echo 'invalid tweet';
	...
	function repliedByWho( $replyStatusId ) {
		$twitterUserId = $this->dbr->selectField(
			self::REPLY_TABLE,
			'twitter_user_id',
	...
	function insertReplyLog( $replyTweet, $response, $origAuthor, $origTweet ) {
		...
		if ( empty( $fromTwitter ) ) {
			$fromTwitter = '';
		}

		$url = 'http://twitter.com/' . ( $fromTwitter ? $fromTwitter : 'x' ) .
			'/status/' . $replyId;
	...
	function lockTweet( $tweetId, $userIdentifier ) {
		...
		if ( !empty( $tweetId ) ) {
			...
			try {
				...
				// TODO: is there a execption handler?
				echo $e->getMessage();
			}
			*/
