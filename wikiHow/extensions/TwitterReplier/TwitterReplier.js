// declare wikiHow module if it hasn't been declared previously, because we're
// extending it below (where we define the TwitterReplier module)
if ( !WH ) {
	var WH = {};
}
...
WH.TwitterReplier = ( function( $ ) {
	...
	var POLL_PERIOD_SECS = 20,
		...
		EXTENSION_URL = mw.config.get( 'wgArticlePath' ).replace( '$1', SPECIAL_PAGE ),
		...
		$.post( EXTENSION_URL, {
			...
			if( tw && tw.tweets && tw.tweets.length > 0 ) {
				var clickMsg = tw.tweets.length > 1 ?
					mw.msg( 'twitterreplier-new-tweets', tw.tweets.length ) :
					mw.msg( 'twitterreplier-new-tweet', tw.tweets.length );
	...
	function displayDialogue( title, msg, width ) {
		...
		var inside = '<p>' + msg + '</p>';
		inside += '<hr><p class="ok_container"><a href="#" class="button white_button ok_button" onmouseover="button_swap(this);" onmouseout="button_unswap(this);">' +
			mw.msg( 'twitterreplier-ok' ) + '</a></p>';
	...
	function randomizeMessage( user, title, url ) {
		...
		user = $.trim( user );
		title = mw.msg( 'howto', $.trim( title ) );
		url = $.trim( url );

		msg[0] = mw.msg( 'twitterreplier-random-message-1', user, title, url );
		msg[1] = mw.msg( 'twitterreplier-random-message-2', user, title, url );
		msg[2] = mw.msg( 'twitterreplier-random-message-3', user, title, url );
	...
	function checkForOldBrowsers() {
		if( $.browser.msie && $.browser.version < 8.0 ) {
			displayDialogue(
				mw.msg( 'twitterreplier-ie-warning-title' ),
				mw.msg( 'twitterreplier-ie-warning-message' )
			);
		}
	}
	...
	function registerUIEventListeners() {
		...
		$( 'input[name="trSearch"]' ).focus( function( e ) {
			if( $( this ).val() == mw.msg( 'twitterreplier-default-search-title' ) ) {
				$( this ).removeClass( 'default_search_input' ).val( '' );
			}
		});
		...
		$( 'input[name="trSearch"]' ).blur( function( e ) {
			if( $( this ).val() == '' ) {
				$( this )
					...
					.val( mw.msg( 'twitterreplier-default-search-title' ) );
			}
		});
		...
		var doWikihowSearch = function( query ) {
			...
			$( '#suggestedTitles tbody' ).html( '<tr><td>&nbsp;</td><td>' +
				mw.msg( 'twitterreplier-searching' ) + '</td></tr>' );
		...
		//				responded = tweetId;
		//				$( '#reply_tweet' ).val( '' );
		//				$( 'input[name="trSearch"]' ).val( mw.msg( 'twitterreplier-default-search-title' ) );
						...
						$.post( EXTENSION_URL, {
							...
							if( response.length == 0 ) {
								...
								$.post( EXTENSION_URL, {
									...
									showHideSpinner( 'hide' );
									displayDialogue(
										mw.msg( 'twitterreplier-success-title' ),
										mw.msg( 'twitterreplier-success-message', tweet.html() ),
										692
									);
								});
								...
								showHideSpinner( 'hide' );
								displayDialogue( mw.msg( 'twittereplier-error-title' ), response );
							}
						});
					} else if( replyLen > MAX_TWEET_CHARS ) {
						showHideSpinner( 'hide' );
						var msg = mw.msg( 'twittereplier-error-reply-too-long' );
						displayDialogue( mw.msg( 'twittereplier-error-title' ), msg );
					} else {
						showHideSpinner( 'hide' );
						var msg = mw.msg( 'twittereplier-error-reply-too-short' );
						displayDialogue( mw.msg( 'twittereplier-error-title' ), msg );
					}
				}
			});
		...
		$( 'input[name="streamSubmit"]' ).click( function() {
			...
			$( '#streamLeft' ).html( mw.msg( 'twittereplier-loading' ) );
			$( '#streamRight' ).html( mw.msg( 'twittereplier-loading' ) );
			if( keywords.length > 0 ) {
				...
				displayDialogue(
					mw.msg( 'twittereplier-error-title' ),
					mw.msg( 'twittereplier-error-missing-keywords' )
				);
			}
		});
			...
			$.post(
				EXTENSION_URL, {
					...
					if ( data && data['removed'] ) {
						displayDialogue(
							mw.msg( 'twittereplier-accounts-unlinked-title' ),
							mw.msg( 'twittereplier-accounts-unlinked-message', mw.config.get( 'wgSiteName' ) )
						);
						$( '.reply_as' ).hide();
					}
				},
		...
		windowObj.scroll( function() {
			...
			if( scrollTop >= topOffset - 10 && newBottom <= bottomOffset ) {
				replyContent.addClass( 'scroll' )
					.removeClass( 'bottom' )
					.removeClass( 'top' );
			} else if( newBottom >= bottomOffset ) { // we've reached bottom of container
				replyContent.addClass( 'bottom' )
					.removeClass( 'top' )
					.removeClass( 'scroll' );
			} else { // we've reached the top of the container
	...
	function lockTweet( tweetId ) {
		...
		$.post(
			...
			//			})
					...
					displayDialogue(
						mw.msg( 'twittereplier-error-title' ),
						mw.msg( 'twittereplier-error-already-replying' )
					);
				}
			},
			'json'
...
$( WH.TwitterReplier.onDOMReady );
$( window ).unload( WH.TwitterReplier.onWindowUnload );