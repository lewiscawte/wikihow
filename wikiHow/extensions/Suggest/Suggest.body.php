	...
	function execute($par) {
		...
		if (!$wgRequest->getVal('st_search') && !$wgRequest->getVal('category')) {
			...
			if ($wgRequest->getVal('st_search') && $wgRequest->getVal('st_search') != "all") {
				...
				$sql = "select st_title, st_user_text, st_user from suggested_titles where st_used = 0 and st_category = " . $dbr->addQuotes($category) . " 
...
class RecommendedArticles extends SpecialPage {
	...
	function getTopLevelSuggestions($map, $cats) {
		...
		$av = '<img src="' . Avatar::getAvatarURL($users[0]) . '"/>';
		$s .= $av;
		$s .= "
		<p>Requested By<br />";
		if ($userids[0] > 0) {
		...
		<td class='requestor'>";
		$av = '<img src="' . Avatar::getAvatarURL($users[1]) . '"/>';
		$s .= $av;
		$s .= "<p>Requested By<br />";
		if ($userids[1] > 0) {
					...
					for ($i = 0; $i < count($suggestionsArray); $i++) {
						if (!empty($suggestionsArray)) {
							...
							if ($suggestionsArray[$i]->user > 0) {
								$u = User::newFromId($suggestionsArray[$i]->user);
