...
//		if (!isset($log[$userid])) {
	...
	public static function getNextToPatrol($type,$by_username) {
		...
		if (!empty($type)) {
			...
			//get all (just image and video right now)
			$sql .= " AND qc_key IN ('changedintroimage','changedvideo') ";
		}
	...
	function getChangedBy($action_str) {
		...
		if ($u) {
			...
			if (preg_match('@patrol@',$action_str)) {				
				...
				$class = " style='float: right;font-size:.9em' ";
...
class QCRCPatrol extends QCRule {
	...
	function flagAction() {
		...
		if ($count < 500) {
			$logqc = true;
		} else if ($count >= 500 && $count < 1500 && rand(0,99) <= 60) {
			$logqc = true;
		} else if (rand(0, 99) <= 2) {
			$logqc = true;
		}
...
class QG extends SpecialPage {
	...
	function getQuickEditLink($title) {
		...
		$editURL = $wgServer . '/Special:Newarticleboost?type=editform&target=' . urlencode($url);
	...
	function getButtons() {
		$buttons =	"<div id='qg_submenu'><div id='qg_options'></div></div>
					<div id='qc_head'>
						<h1 id='question'></h1>
						<div id='qc_skip_div'>
	...
	function getOptionMenu($menu_name,$chosen,$username) {	
		...
		if ($menu_name == 'options') {
			...
			$html = "<a href='#' class='button white_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);' id='qcrules_submit'>Done</a>";
			$html .= '<div>';
			...
			$html = 'Username: ' . 
					...
					'<div id="qg_byuser_buttons">
						<a id="qg_byuser_off" class="button white_button" onmouseover="button_swap(this);" onmouseout="button_unswap(this);" href="#">Off</a>
						<a id="qg_byuser_go" class="button button52" onmouseover="button_swap(this);" onmouseout="button_unswap(this);" href="#">Go</a>
					</div>';
		}
	...
	function getTabs($qc_type) {
		...
		$html = "<div id='qg_tabs'>
					<div class='tableft' id='qgtab_byuser'>
						<div class='tabright'>
							<a href='#'><div class='arrow'></div>" . wfMsg('qc_byuser') . "</a>
						</div>
					</div>
					<div class='tableft' id='qgtab_options'>
						<div class='tabright'>
							<a href='#'><div class='arrow'></div>" . wfMsg('qc_rulestocheck') . "</a>
						</div>
					</div>
				</div>";
	...
	function execute($par) {
		...
		$wgOut->addScript("<script type='text/javascript' src='". wfGetPad('/extensions/min/f/extensions/wikihow/qc.js?rev=') . WH_SITEREV ."'></script>");
		$wgOut->addStyle('../extensions/wikihow/qc.css?rev='. WH_SITEREV);
...
class NoVotesAgainst extends UnlistedSpecialPage {
	function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage( 'NoVotesAgainst' );
	}
	function execute() {
		global $wgOut;
		$dbr = wfGetDB(DB_MASTER);
		$wgOut->addHTML("<h2>Top Users with No Votes for Rollback Edits</h2>");
		$res = $dbr->query("SELECT qc_user_text, count(*) as C from qc left join qc_vote on qc_id=qcv_qcid
