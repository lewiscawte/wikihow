...
function wfSetupSession() {
	...
	$oldid = $_COOKIE[$wgCookiePrefix.'_session'];
	$size  = sizeof($_COOKIE);
	@session_start();
	if (!isset($_SESSION['started'])) {
		$ids = implode(",", array_keys($_COOKIE));
		$_SESSION['started'] = wfTimestampNow();
		$_SESSION['id'] 	 = session_id();
		$_SESSION['cookieids'] 	 = $ids; 
		$_SESSION['oldid'] 	 = $oldid;
		$_SESSION['cookiesize']  = $size; 
	}
}
...
function wfGetPad($relurl = '') {
	global $wgServer, $wgIsDomainTest;
	...
	// Special case for www.wikihow.com urls being requested on babel/international server
	if (strpos($hostname, 'babel') === 0 && preg_match('@http://www.wikihow.com@', $relurl)) {
		$relurl = str_replace('http://www.wikihow.com', '', $relurl);
    } else {
		// Don't translate CDN URLs in 3 cases:
		//  (1) if the URL is non-relative (starts with http://),
		//  (2) if the hostname of the machine doesn't end in .wikihow.com and
		//      virtual host isn't www.wikihow.com or m.wikihow.com
		//  (3) if the hostname is that of an international server and
		//      virtual host isn't www.wikihow.com or m.wikihow.com
		if (strpos($relurl, 'http://') === 0 ||
			(!$isCanonicalServer &&
			 (!preg_match('@\.wikihow\.com$@', $hostname) ||
			  strpos($hostname, 'babel') === 0)))
		{
			return $relurl;
		}
	}
    ...
    $pad = ($crc % $numPads) + 1;
	return "http://pad{$pad}.whstatic.com{$relurl}";
}
