...
function add_alt_method() {
	var index = 0; 
	while (true) {
		if ($("#alt_method_" + index).length  == 0) {
			break;
		}
		index++;
	}

	var rows = $('textarea[name^="steps"]').attr('rows');

	var html = "<div id='alt_method_" + index + "' class='alt_method_edit'><h3>" + wfMsg('alt_method') + ": <input type='text' name='alt_title_" + index + "' value='" + wfMsg('alt_title_default') + "'/></h3><div class='article_inner'><div class='taButtons'> <p><a onclick='javascript:expandtext(\"alt_text_" + index + "\");' class='button expand_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);'></a></p><p><a onclick='javascript:compresstext(\"alt_text_" + index + "\");' class='button contract_button' onmouseover='button_swap(this);' onmouseout='button_unswap(this);'></a></p></div><textarea name='alt_text_" + index + "' rows='" + rows + "' cols='100' wrap='virtual' onKeyUp=\"addNumToSteps(event, this);\" id='alt_text_" + index + "'>#  </textarea></div></div>";

	if (index == 0) {
		$("#steps").append(html);
	} else {
		$("#alt_method_" + (index-1)).append(html);
	}
	reTab();
