...
jQuery('#fb_login_mainpage,#fb_login').live('click', function() {
	...
	var fbs = document.location.protocol + '//connect.facebook.net/' + locale + '/all.js';

	var oldHtml = jQuery('#fb_connect_header').html();
	...
	jQuery.getScript(fbs, function() {
		FB.init({
			appId  : '257d28cd1eeae6b453a151e2538f45d8',
		...
		FB.login(function(response) {
		  ...
		  if (response.session) {
			...
			window.location='/Special:FBConnect';
			 ...
			 jQuery('#fb_connect_header').html(oldHtml);
		  }
		});
	});
});
