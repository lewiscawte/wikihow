...
function restoreToolbarButtons() {
	...
	if (mwEditButtons.length > 0) {
		...
		for (i = 0; i < parts.length; i++) {
			// split it up
			var part = parts[i].substring(10,parts[i].length-1);
			part = part.replace(/\"/g,'');
			part = part.replace(/\\/g,'');
...
var ts_image_path = WH_CDN_BASE + "/skins/common/images/";
...
function rateArticle(r) {
	...
	var msg = 'Thanks. Your vote has been counted.';
	if (r == 0) {
		msg = 'Thanks. Please <a href="/Discussion:' + wgPageName + '#post">click here</a> to provide specific details on how we can improve this article.';
	}
	alert(r + msg);
