...
//used for adding channels based on browser
var is_ff = clientPC.indexOf('firefox');
var is_chrome = clientPC.indexOf('chrome');
var is_ie6 = false;
var is_ie7 = false;
var is_ie8 = false;
var is_ie9 = false;
...
var isiPad = clientPC.indexOf('ipad');
var isiPhone = clientPC.indexOf('iphone');
...
jQuery('#fb_login_mainpage,#fb_login').live('click', function() {
	var locale = 'en_US';
	if (wgUserLanguage == 'es' || wgUserLanguage == 'de') {
		locale = wgUserLanguage + '_' + wgUserLanguage.toUpperCase();
	}
	var fbs  = 'http://static.ak.connect.facebook.com/js/api_lib/v0.4/FeatureLoader.js.php/' + locale;

	jQuery('#fb_connect_header').html('[Logging in <img src="http://pad1.whstatic.com/skins/WikiHow/images/fb_loading.gif"/>]');

	// Load facebook connect script
	jQuery.getScript(fbs, function() {
		FB_RequireFeatures(["Connect"], function() {
			FB.init('bbff37db145344ae435797f9810464d5', '/extensions/wikihow/xd_receiver.htm');

			FB.Connect.requireSession(function() {
				var today = new Date();
				var expires_date = new Date( today.getTime() + 3600*1000 );
				document.cookie = 'wiki_returnto=' + escape(document.referrer) + ';expires=' + expires_date.toGMTString();
				window.location='/Special:FBConnect';
			});
		});
	});
});
...
$(document).ready(function(){
	//code for turning right rail html links into js links
	/*$('#navigation_list a').click(function(e){
		e.preventDefault();
		window.location.href = window[$(this).attr('id')];
	});

	$('#visit_list a').click(function(e){
		e.preventDefault();
		window.location.href = window[$(this).attr('id')];
	});*/


	//code for testing the swfs on article pages
	/*swfDiv = $(".swf_div");
	swfId = $(swfDiv).attr("id");
	swfWidth = $(swfDiv).width();
	swfHeight = $(swfDiv).height();

	var flashvars = {};
	flashvars.class = "left";
	...
	if(typeof(swfobject) !== 'undefined')
		swfobject.embedSWF("/skins/WikiHow/images/" + swfId + ".swf", swfId, swfWidth, swfHeight, "9.0.0", flashvars, params, attributes);
	*/
});