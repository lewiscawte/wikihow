...
// Example:
highlighter.addStyle('', '#FFFFCC', ''); // bold, black text on yellow background.
...
function SearchTermHighlighter()
{
	...
	{
		if (!ok) return;
		this.loadSearchTerms(document.referrer);
		var style = 0;
	...
	function getSearchTerms(referrerString){
		this.loadSearchTerms(referrerString);
		return searchTerms;
	}

	function loadSearchTerms(referrerString)
	{
		var a = new Array();
		var params = getParamValues(referrerString, this.parameters);
		var terms;
		var index = 0;
	...
	function highlight() 
	{
		...
		if(this.tempHighlight){
			this.timeoutVar = setTimeout("highlighter.unhighlight();",this.highlightTime);
		}
	}
