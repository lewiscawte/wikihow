...
while ($row = $dbr->fetchObject($res)) {
	...
	if ($thumbPageId == $row->thumb_page_id && $thumbRecipientText == $row->thumb_recipient_text && $thumbGiverId == $row->thumb_giver_id && $time > $before && $time < $after && $thumbRevId != $row->thumb_rev_id) {
		$sql = "UPDATE thumbs SET thumb_exclude=1 WHERE thumb_giver_id = " . $row->thumb_giver_id . " AND thumb_rev_id = " . $row->thumb_rev_id . " and thumb_recipient_text = '" . mysql_real_escape_string($row->thumb_recipient_text) . "' and thumb_timestamp='" . $row->thumb_timestamp . "' and thumb_page_id = " . $row->thumb_page_id;
		echo $sql . "\n";
		$dbw->query($sql);
	}
	else {
