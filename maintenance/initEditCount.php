...
if( $backgroundMode ) {
	...
	$sql = "UPDATE $user SET user_editcount=(SELECT COUNT(*) FROM $revision left join page on rev_page=page_id WHERE rev_user=user_id and page_namespace not in (2,3,18))"; // WHERE user_id=1236204";
	echo $sql . "\n";
	$dbw->query( $sql );
}
