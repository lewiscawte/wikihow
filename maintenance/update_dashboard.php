...
*/
	...
	$start = wfTimestamp(TS_MW, $argv[0] . "000000");
	...
	$sql = "select rev_user, rev_user_text, max(rev_timestamp) as M, count(*) as C from rev_tmp group by rev_user having C >= 25 and M >'{$m_cutoff}' and M <'{$start}';";
	wfDebug("Dashboard new editors who became active this month: $sql\n");
	$res = $dbw->query($sql);
    while ($row = $dbw->fetchObject($res)) {
    ...
    $sql = "select rev_user, rev_user_text, max(rev_timestamp) as M, count(*) as C from rev_tmp group by rev_user having C >= 10 and M >'{$w_cutoff}' and M <'{$start}';";
	wfDebug("Dashboard new editors who became active this week: $sql\n");
    $res = $dbw->query($sql);
    while ($row = $dbw->fetchObject($res)) {
...
*/
	...
	$result .= getTopCreators($dbw, $m_cutoff, $start);	
	$result .= "</td><td {$tdstyle}>";
	...
	$result .= getTopCreators($dbw, $m_cutoff, $start);	
	$result .= getTopCreators($dbw, $w_cutoff, $start);	
	$result .= "</td></tr></table>";
	...
	$result .= "<ul><li>Articles in problem categories " . ($now != $start ? " as of $now " : "") .  nf(selectField($dbw, $sql)) . "</li></ul>";
	...
	$result .= "<ul><li>Number of Rising Stars: " . ($now != $start ? " as of $now " : "") . nf(selectField($dbw, $sql)) . "</li></ul>";
	
	$result .= "<table class='dashboard' style='font-family: Arial; margin-left:auto; margin-right:auto; width: 90%;'><tr><td {$tdstyle}>";
	$result .= "<h3>Article stats for the past week</h3>" . articleStats($dbw, $w_cutoff, $start) . "\n";	
	$result .= "</td><td {$tdstyle}>";
	$result .= "<h3>Article stats for the past month</h3>" . articleStats($dbw, $m_cutoff, $start) . "\n" ;	
