...
function getDomainRange($domain, $r, $dt) {
	global $dbr;
	$sql = "select count(*) from google_serps where datediff('{$dt}', gs_timestamp) = 0 and ";
	if (sizeof($r) == 1) 
		$sql .= " gs_position > {$r[0]} ";
	else
...
$dbr->selectDB('serps');
$res = $dbr->query("select count(*) from google_serps where datediff('$dt', gs_timestamp) = 0;");
...
$old = array(); 
$sql = "select substr(gs_timestamp, 1, 10) as date, count(*) as C from google_serps where datediff('$dt', gs_timestamp) != 0 group by date having C > 50000 order by date  desc limit 1;";
$res = $dbr->query($sql);
$row = $dbr->fetchRow($res);
$lastdate = $row[0];

echo "Last report: {$lastdate}: " . number_format($row[1], 0, "", ",") . " results.<br/></center>\n";

$sql = "select gs_domain as domain, count(*) as results, avg(gs_position) as AVG from google_serps where datediff('{$lastdate}', gs_timestamp) = 0 group by gs_domain order by AVG;";
$res = $dbr->query($sql);
...
$sql= "select gs_domain as domain, count(*) as results, avg(gs_position) as AVG from google_serps where datediff('{$dt}', gs_timestamp) = 0 group by gs_domain order by AVG;";
$res = $dbr->query($sql);
$s = "";
while ($row = $dbr->fetchObject($res)) {
	$domain = $row->domain;
...
$old = array();
$sql = "select gs_domain as domain, count(*) as num from google_serps where datediff('{$lastdate}', gs_timestamp) = 0 and gs_position=1 group by gs_domain order by num desc;";
$res = $dbr->query($sql);
while ($row = $dbr->fetchObject($res)) {
	$old[$row->domain] = $row->num;
}

$sql = "select gs_domain as domain, count(*) as num from google_serps where datediff('$dt', gs_timestamp) = 0 and gs_position=1 group by gs_domain order by num desc;";
$res = $dbr->query($sql);
$s = "";
while ($row = $dbr->fetchObject($res)) {
...
$sql = "select gs_query from google_serps where datediff('{$lastdate}', gs_timestamp) = 0 and gs_position=1 and gs_domain='wikihow.com';";
$res = $dbr->query($sql);
...
foreach ($numones_queries as $q) {
	$sql = "select gs_position from google_serps where datediff('$dt', gs_timestamp) = 0 and gs_domain='wikihow.com' and gs_query=" . $dbr->addQuotes($q);
	$res = $dbr->query($sql);
...
$sql = "select gs_query, gs_position from google_serps where datediff('{$lastdate}', gs_timestamp) = 0 and gs_position between 1 and 10  and gs_domain='wikihow.com';";
$res = $dbr->query($sql);
$numones_queries = array();
while ($row = $dbr->fetchObject($res)) {
