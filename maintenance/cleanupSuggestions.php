...
#echo "going over suggestions\n";
	...
	$clauses = array("datediff(now(), last_processed) > 0 or last_processed = '0000-00-00 00:00:00'");
	if (isset($argv[2])) 
...
#echo "updating results\n";
	...
	foreach ($updates as $u) {
		...
		$sql .= "UPDATE goog set clean_title=" . $dbw->addQuotes($u[1]) . ", exclude={$u[2]}, tskey = " . $dbw->addQuotes($u[3]) . ", last_processed=now() where id={$u[0]};\n";
		#if ($count > 0 && $count % 100 == 0) {
			#echo "current at $count\n";
			echo $sql;
