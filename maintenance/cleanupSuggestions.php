...
#echo "going over suggestions\n";
	...
	$clauses = array("datediff(now(), lastupdated) > 0 or lastupdated = '0000-00-00 00:00:00'");
	if (isset($argv[2])) 
		$options = array("LIMIT" => $argv[2]); // 'ORDER BY'=>'rand()');
	$res = $dbr->select('ru_stcl', array('id', 'raw_title'), $clauses, "maintenance:cleanupSuggestions", $options);
	$updates = array();
	$count = 0;
#echo "got res\n";
	while ($row = $dbr->fetchObject($res)) {
		...
		//echo $row->raw_title . "\n";
        $title = preg_replace('/^how to[ ]?/', '', $row->raw_title);
		#echo $title; exit;
		$excluded = 0;
...
#echo "formatting title bad_re for {$title}, {$row->id}\n";
		...
		try {
			$title = preg_replace($ac_re, $ac_re2,  $title);	
		} catch (Exception $e) {
			echo "caught e $e\n";
			print_r($ac_re);
			print_r($ac_re2);
...
#echo "updating results\n";
	...
	foreach ($updates as $u) {
		...
		$sql .= "UPDATE ru_stcl set clean=" . $dbw->addQuotes($u[1]) . ", excluded={$u[2]}, tskey = " . $dbw->addQuotes($u[3]) . ", lastupdated=now() where id={$u[0]};\n";
		if ($count > 0 && $count % 100 == 0) {
			#echo "current at $count\n";
			#echo $sql;
