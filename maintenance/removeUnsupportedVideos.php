...
$res = $dbr->select('page', array('page_id'), array('page_namespace' => 24, 'page_is_redirect' => 0), "removeUnsupportedVideos.php", array("LIMIT" => 50));
#$res = $dbr->select('page', array('page_id'), array('page_namespace' => 24, 'page_is_redirect' => 0), "removeUnsupportedVideos.php");

$wgUser = User::newFromName('wikiHow video removal');
...
function removeVideoSection(&$article) {
	...
	if ($t) {
		...
		//Wikitext::saveWikitext($t, $wikitext, "wikiHow bot removing video section");
...
function removeVideoPage(&$video) {	
	...
	if ($t && $t->exists()) {
		...
		if ($a && $a->exists()) {
			$dbw->begin();
			$reason = "Deleting Video Page - wikiHow bot";
...
function isSupported5min($id) {
	...
	$res = curl($url);
	$notFound = "@The video request you sent is incorrect@";
...
function isSupportedWonderHowto($id) {
	...
	if(!preg_match("@videojug|youtube|howcast|5min@", $id)) {
		...
		if (preg_match($url_re, $id, $url)) {
			...
			// No match. Ignore.
			error("oops, couldn't find a url for \n $id");
			return $ret;
		}
