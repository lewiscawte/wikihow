<?
	...
	*/
	...
	$res = $dbr->query("SELECT * FROM recentchanges 
		...
		WHERE rc_namespace = 0 AND rc_new = 1 AND cv_page is null AND page_is_redirect=0 
	...
	while ($row = $dbr->fetchObject($res)) {
		...
		/// ignore this if we have checked it recently	
		$lastedit = $dbr->selectField('recentchanges', 'max(rc_timestamp)', array('rc_cur_id'=>$t->getArticleID()));
		$lastcheck  = $dbr->selectField('copyviocheck', 'cv_timestamp', array('cv_page'=>$t->getArticleID()));
		if ($lastcheck && $lastcheck > $lastedit) {
			echo "Checked {$t->getArticleID()} since it's been edited.\n";
			continue;
		}
		...
		*/
		...
		if ($results['count']) {
			...
			foreach($results['result'] as $r) {
				if (!preg_match("@wikihow.com@", $r['url'])) {
