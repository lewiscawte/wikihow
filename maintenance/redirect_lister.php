...
function main($out_csv) {
	...
	while (true) {
		...
		$res = $dbr->query('SELECT page_title,page_catinfo FROM page WHERE page_namespace=' . NS_MAIN . ' AND page_is_redirect=1 LIMIT ' . $offset . ',' . $batch_size);
		$count = 0;
		while ($row = $dbr->fetchRow($res)) {
			$count++;
			$articles[] = array(
				'title' => $row['page_title'],
				'isteen' => $row['page_catinfo'] & CAT_TEEN,
			);
		}
	...
	foreach ($articles as $article) {
		...
		if (preg_match('@^\s*#REDIRECT[: *]*\[\[(.*)\]\]@i', $body, $matches)) {
			...
			$is_teen_topic = $article['isteen'] ? 'Yes' : 'No';
			$line = array('', $title->getText(), $title_dest->getText(), $inbound_links, $is_teen_topic);
			fputcsv($fp, $line);
