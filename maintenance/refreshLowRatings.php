...
if (!empty($notin)) {
	$notin = "and rat_page NOT IN (".$notin.")";
}

$dbw->query("delete from rating_low;", 'refreshLowRatings.php:delete');
$dbw->query("insert into rating_low select rat_page, avg(rat_rating) as R, count(*) as C from rating, page where rat_page=page_id $notin and rat_isdeleted = 0 and page_is_redirect = 0 group by rat_page having R <= $avg and C >= $minvotes ; ",
