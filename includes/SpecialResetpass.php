	...
	function showForm() {
		...
		$form  =
			...
			'<tr>' .
				'<td></td>' .
				'<td>' .
					wfSubmitButton( wfMsgHtml( 'resetpass_submit' ), array( 'class' => 'button button220 submit_button', 'onmouseout' => "button_unswap(this);", 'onmouseover' => "button_swap(this);") ) .
				'</td>' .
			'</tr>' .
			'</table>' .
			wfCloseElement( 'form' ) .
		...
		foreach( $fields as $list ) {
			...
			if( $type == 'text' ) {
				...
				$field = Xml::input( $name, 20, $value,
					array( 'id' => $name, 'type' => $type, 'class' => 'input_med' ) );
			}
			$out .= '<tr>';
			$out .= '<td class="mw-label">';
			$out .= '<label for="' . $name . '">' . wfMsg ($label) . '</label>';//Xml::label( wfMsg( $label ), $name );
			$out .= '</td>';
