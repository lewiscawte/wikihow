	...
	function doTransform( $image, $dstPath, $dstUrl, $params, $flags = 0 ) {
		...
		if ( $scaler == 'im' ) {
			...
			if (!$crop) {
				...
				$cmd = wfEscapeShellArg($wgImageMagickConvertCommand) 
					. " " . wfEscapeShellArg($srcPath) 
					. " -thumbnail " . wfEscapeShellArg( "{$physicalWidth}x") 
					. " -gravity center -crop " . wfEscapeShellArg( "{$physicalWidth}x{$physicalHeight}+0+0 ")
					. " -quality 100 "
					. wfEscapeShellArg($dstPath) . " 2>&1";
			}
			...
			wfProfileIn( 'convert' );
			if (wfRunHooks("ImageConvert", array($cmd))) {
				$err = wfShellExec( $cmd, $retval );
				wfRunHooks("ImageConvertComplete", $cmd);
			}
