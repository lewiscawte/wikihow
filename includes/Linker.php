...
class Linker {
	...
	function makeImageLink2( Title $title, $file, $frameParams = array(), $handlerParams = array(), $time = false ) {
		...
		if ( !$thumb ) {
			...
			$s = $thumb->toHtml( array(
				'desc-link' => true,
				'href' => '',
				'onclick'=>'return loadimg("' . $thumb->path . '", "' . $thumb->url . '" );',
				'alt' => $fp['alt'],
		...
		if ( '' != $fp['align'] ) {
			$s = "<div class='mwimg'><div class=\"float{$fp['align']}\"><span>{$s}</span></div></div>";
		}
		return str_replace("\n", ' ',$prefix.$s.$postfix);
	}
	...
	function makeThumbLink2( Title $title, $file, $frameParams = array(), $handlerParams = array(), $time = false ) {
		...
		$s = "<div class=\"thumb t{$fp['align']}\" style=\"width:{$outerWidth}px;\">
		<div class=\"rounders\" style=\"width:{$w}px;height:{$h}px\"> ";
		...
		if( !$exists ) {
			...
			$s .= $thumb->toHtml( array(
				...
				'desc-link' => true ) ) 		
				. '<div class="corner top_left"></div><div class="corner top_right"></div><div class="corner bottom_left"></div><div class="corner bottom_right"></div></div>';
			if ( isset( $fp['framed'] ) ) {
				$zoomicon="";
			} else {
				$zoomicon =  '<a href="'.$url.'" class="internal" title="'.$more.'">'.
					'<img src="' . wfGetPad($wgStylePath.'/common/images/magnify-clip.png') . '" ' .
					'width="16" height="16" alt="" /></a> ';
			}
		}
		$s .= $zoomicon."<span class='caption'>" .$fp['caption']."</span></div>";
		#echo $s; exit;
		$s = "<div class='mwimg'>$s</div>";
		return str_replace("\n", ' ', $s);
	}
