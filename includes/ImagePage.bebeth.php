...
class ImagePage extends Article {
	...
	function view() {
		...
		$wgOut->addHTML('<div style="text-align:center">'.wfMsg('image_upload', $userLink).'</div>');

		$wgOut->addHTML(wfMsg('image_instructions', $wgTitle->getFullText()));
...
//echo $this->current->getUser();
		...
		if ( $this->getID() ) {
			...
			$pos = stripos($content, "== Licensing ==");
			if($pos === false){
				$wgOut->addHTML("<h2>Licensing/Attribution</h2>");
			}
			else{
				$content = str_replace("== Licensing ==", "== Licensing/Attribution ==", $content);
			}
...
class ImageHistoryList {
	...
	public function beginImageHistoryList() {
		...
		$s .= Xml::element( 'h2', array( 'id' => 'filehistory' ), wfMsg( 'filehist' ) )
			...
			. Xml::openElement( 'table', array( 'class' => 'filehistory history_table', 'style' => 'width:100%; border-collapse:collapse;' ) ) . "\n";
			//. '<tr><td></td>'
