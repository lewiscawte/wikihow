...
#------------------------------------------------------------------------------
	...
	function tableName( $name ) {
		...
		if ( $name{0} != '`' ) {
			...
			if ( isset( $wgSharedDB ) 
				&& ("{$this->mTablePrefix}user" == $name || "{$this->mTablePrefix}facebook_connect" == $name ||  "{$this->mTablePrefix}user_openid" == $name)
			) {
	...
	function getLag() {
		...
		while ( $row = $this->fetchObject( $res ) ) {
			...
			if ( $row->User == 'system user' && 
				...
				$row->State != 'Has read all relay log; waiting for the slave I/O thread to update it' &&
