...
class PageHistory {
	...
	function beginHistoryList() {
		...
		$s .= '<form action="' . $wgTitle->escapeLocalURL( '-' ) . '" method="get" id="history_form">';
	...
	function submitButton( $bits = array() ) {
		...
		if ( $this->linesonpage > 1 ) {
			return Xml::submitButton( wfMsg( 'compareselectedversions' ),
				$bits + array(
					//'class'     => 'historysubmit',
					'class'     => 'button white_button_200 submit_button',
	 ...
	 */
	function historyLine( $row, $next, $counter = '', $notificationtimestamp = false, $latest = false, $firstInList = false, $idx = false ) {
		...
		$s .= "<td class='history_curlast'>($curlink) ($lastlink)</td> $arbitrary";
		...
		$s .= "<td class='history_link'>$link</td><td class='history_user'>";
		...
		$s .= " <span class='history-user'>$user</span></td></tr>";
	...
	function diffButtons( $rev, $firstInList, $counter ) {
		if( $this->linesonpage > 1) {
			...
			$second = Xml::element( 'input', array_merge(
				...
				$checkmark,
				array( 'name'  => 'diff' ) ) );
			return "<td class='diffbtn'>$first</td><td class='diffbtn'> $second</td>";
		} else {
			return '';
		}
	}
...
class PageHistoryPager extends ReverseChronologicalPager {
	...
	function formatRow( $row, $idx = false ) {
		...
		if ( $this->mLastRow ) {
			...
			$s = $this->mPageHistory->historyLine( $this->mLastRow, $row, $this->mCounter++, 
				$this->mPageHistory->getNotificationTimestamp(), $latest, $firstInList, $idx );
		} else {
			$s = '';
		}
		$this->mLastRow = $row;
	...
	function getStartBody() {
		$this->mLastRow = false;
		$this->mCounter = 1;
		return '<table width="100%" border="0" class="history_table" cellspacing="0">';
	}
