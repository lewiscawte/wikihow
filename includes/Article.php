...
class Article {
		...
		if ( 0 == $this->getID() ) {
			...
			if ( $this->mTitle->getNamespace() == NS_MEDIAWIKI ) {
				...
				if ($editor == 'visual'
					...
					&& hasHtml5Browser())
				{
					$appendParam = 'create-new-article=true';
				} else {
					$appendParam = 'action=edit';
				}

				if ($this->mTitle->getNamespace() == NS_USER_TALK)
					$ret = wfMsg( 'noarticletext_user_talk', $this->mTitle->getText(), $this->mTitle->getFullURL() . "?action=edit"   );		 
				else if (MWNamespace::isTalk($this->mTitle->getNamespace()))  
					...
					$ret = wfMsg( 'noarticletext', $this->mTitle->getText(), $this->mTitle->getFullURL() . "?$appendParam", urlencode($this->mTitle->getText())  );					 
				else 
	...
	*/
	function view()	{
		...
		if ($this->mTitle->getNamespace() == NS_USER_TALK
			|| ($this->mTitle->getNamespace() == NS_USER && preg_match("@profilebox@", $this->mTitle->getText()))) {
			$policy = "noindex,follow";
		}
...
//SpamDiffTool::getDiffLink ($this->mTitle)
	...
	function insertOn( $dbw ) {
		...
		$dbw->insert( 'page', array(
			...
			'page_catinfo'		=> $this->mTitle->getCategoryMask(),
...
#echo wfBacktrace(); exit;
		...
		if ( $flags & EDIT_UPDATE ) {
			...
			if ( $changed ) {
				...
				if( !$ok ) {
					...
					if( !( $flags & EDIT_SUPPRESS_RC ) ) {
						...
						if( ($GLOBALS['wgUseRCPatrol'] && $wgUser->isAllowed( 'autopatrol' ) && $wgUser->getOption('autopatrol'))
							|| in_array("bot", $wgUser->getGroups())	
						) {
			...
			if( !( $flags & EDIT_SUPPRESS_RC ) ) {
				...
				if( ($GLOBALS['wgUseRCPatrol'] || $GLOBALS['wgUseNPPatrol']) && $wgUser->isAllowed( 'autopatrol' ) && $wgUser->getOption('autopatrol') ) {
					RecentChange::markPatrolled( $rcid, $this );
					PatrolLog::record( $rcid, true );
				}
			}
	...
	function markpatrolled() {
	   ...
	   if ($wgRequest->getInt('skip', null) != 1) {
		   ...
		   foreach ($rcids as $id) {
			   RecentChange::markPatrolled( $id, $this );
			   PatrolLog::record( $id, false );
		   }
		   wfRunHooks( 'MarkPatrolledBatchComplete', array(&$this, &$rcids, &$wgUser));
 ...
 //TODO: shorten this to a selectRow call
	   ...
	   if ($show_namespace != null && $show_namespace != '')  {
		   ...
		   $sql .= " AND rc_namespace NOT IN ( " 
				#. KALTURA_NAMESPACE_ID .","
				. NS_VIDEO 
				.") ";
		}
   ...
   function skipPatrolled() {
			...
			"<tr id=\"wpDeleteReasonListRow\">
				...
				<td>" .
					Xml::listDropDown( 'wpDeleteReasonList',
						...
						wfMsgForContent( 'deletereasonotherlist' ), $defaultReason, 'wpReasonDropDown', 1 ) .
				"</td>
			</tr>
	...
	public function doRollback( $fromP, $summary, $token, $bot, &$resultDetails ) {
		...
		if(!$wgUser->matchEditToken( $token, array( $this->mTitle->getPrefixedText(), $fromP ) ) ) {
			...
			$id = $_COOKIE[$wgCookiePrefix.'_session'];

			$email = new MailAddress("alerts@wikihow.com");
			$subject = "WHDEBUG Match edit token failure from " . wfHostname() . " on " . date("r");
			$body = "Now: " . wfTimestampNow() . "\n";
			$body .= "Cookie id {$id} session id " . session_id() . "\n";
			$body .= "Cookies: \n" . print_r($_COOKIE, true) . "\n";
			$body .= "Server vars: \n" . print_r($_SERVER, true) . "\n";
			$body .= "Submitted token $token\nSalt: {$this->mTitle->getPrefixedText()}, $fromP\n";
			#$body .= "Server vars: ". print_r($_SERVER, true) . "\n";
			$body .= "Current token: " . $wgUser->editToken(array( $this->mTitle->getPrefixedText(), $fromP )) . "\n";;
			$body .= "Fresh generated token: " . $wgUser->generateToken(array( $this->mTitle->getPrefixedText(), $fromP )) . "\n";
			$body .= "Session: " . print_r($_SESSION, true) . "\n";
			$body .= "Get: " . print_r($_GET, true) . "\n";
			$body .= "Hostname: " . wfHostname() . "\n";
	...
	public function commitRollback($fromP, $summary, $bot, &$resultDetails) {
		...
		//XXCHANGED: for RCPatrol feature
		global $wgRequest;
		if ($wgRequest->getVal('old')) 
			$target = Revision::newFromId( $wgRequest->getVal('old'));
		else
		...
		{
			global $wgLang;
			$userText = $target ? $target->getUserText() : wfMsg('Anonymous');
...
*/
	...
	*/
	...
	public function outputWikiText( $text, $cache = true ) {
		...
		wfRunHooks( 'ArticleBeforeOutputWikiText', array( &$this, &$text ) ) ;
		...
		wfRunHooks( 'ArticleAfterOutputWikiText', array(&$parserOutput)) ;
