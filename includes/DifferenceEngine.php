...
class DifferenceEngine {
	...
	var $mOldtitle, $mNewtitle, $mPagetitle, $mOldDate;
	var $mOldtext, $mNewtext;
	var $mOldPage, $mNewPage, $mNewDate;
...
//******* RC Changes *****/
 ...
 //******* RC Changes *****
	...
	function showDiffPage( $diffOnly = false ) {
		global $wgUser, $wgOut, $wgUseExternalEditor, $wgUseRCPatrol, $wgRequest, $wgTitle;
...
CONTROL;
		...
		if ($this->mOldRev) {
			...
			$av = '<img src="' . Avatar::getAvatarURL($this->mOldRev->mUserText) . '"/>';
			$userToolsString = $sk->revUserTools( $this->mOldRev, true );
			$userParts = explode("(", $userToolsString, 2);
			$userName = $userParts[0];
			$userTools = "(" . $userParts[1];

			$oldHeader = '<table cellpadding="0" cellspacing="0" style="margin:0 auto;"><tr><td colspan=2" style="text-align:center;"><div id="mw-diff-otitle1"><h4>'. $prevlink . $this->mOldtitle.'</h4></div></td></tr>' .
			'<tr><td style="text-align:right; padding-right:5px; vertical-align:top;">' . $av . '</td><td style="text-align:left"><div id="mw-diff-otitle1a">by: ' . $userName . '</div>' .
			'<div id="mw-diff-otitle2">'. $this->mOldDate . "</div>" .
			'<div id="mw-diff-otitle2b">' . $userTools . "</div></td>";
			if($comment != "")
			    $oldHeader .= '<tr><td colspan="2"><div id="mw-diff-otitle3" class="rccomment_top"><div class="rccomment_middle">' . $comment . '</div><div class="rccomment_bottom"></div></div></td></tr>';

			$oldHeader .= '</table>';
		} else {
			$oldHeader = "No previous revision";
		}
		...
		$av = '<img src="' . Avatar::getAvatarURL($this->mNewRev->mUserText) . '"/>';
		$userToolsString = $sk->revUserTools( $this->mNewRev, true );
		$userParts = explode("(", $userToolsString, 2);
		$userName = $userParts[0];
		$userTools = "(" . $userParts[1];
		
		$newHeader = '<table cellpadding="0" cellspacing="0" style="margin:0 auto;"><tr><td colspan="2" style="text-align:center;"><div id="mw-diff-ntitle1"><h4>'.$this->mNewtitle . ' ' . $nextlink .  '</h4></div></td></tr>' .
			'<tr><td style="text-align:right; padding-right:5px; vertical-align:top;">' . $av . '</td><td style="text-align:left"><div id="mw-diff-ntitle1a">by: ' . $userName . '</div>' .
			'<div id="mw-diff-ntitle2">' . $this->mNewDate . "</div>" .
			'<div id="mw-diff-ntitle2b">' . $userTools . "</div>" .
			'<div id="mw-diff-ntitle4" style="text-align:left">' . $rollback . $nextlink . $patrol . '</div></td></tr>';
		if($comment != "")
		    $newHeader .= '<tr><td colspan=2><div id="mw-diff-ntitle3" class="rccomment_top"><div class="rccomment_middle">' . $comment . '</div><div class="rccomment_bottom"></div></div></td></tr>';
	...
	function renderNewRevision() {
		...
		$wgOut->addHTML( "<hr /><br /><br /><h2>{$this->mPagetitle}</h2>\n" );
	...
	function loadRevisionData() {
		...
		$timestamp = wfTimeAgo($wgLang->timeanddate( $this->mNewRev->getTimestamp(), false));
		...
		if( $this->mNewRev->isCurrent() ) {
			...
			$this->mNewtitle = "<a href='$newLink'>{$this->mPagetitle}</a>" . " (<a href='{$newEdit}'>edit</a>)";
		...
		if ( $this->mOldRev ) {
			...
			$t = wfTimeAgo($wgLang->timeanddate( $this->mOldRev->getTimestamp(), false));
			...
			$oldTitle = "<a href='{$oldLink}'>Old Revision</a> ";
			
			$this->mOldtitle = $oldTitle . " (<a href='{$oldEdit}'>edit</a>)";
			$this->mOldDate = $t;
