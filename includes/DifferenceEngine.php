...
CONTROL;
		...
		if ($this->mOldRev) {
			...
			'<tr><td style="text-align:right; padding-right:5px; vertical-align:top;">' . $av . '</td><td style="text-align:left"><div id="mw-diff-otitle1a">' . wfMsg('diff_by') . ' ' . $userName . '</div>' .
		...
		$thumbHeader = "";
		$th_diff_div = "";
		if ($wgTitle->getText() != "RCPatrol" && $wgTitle->getText() != "RCPatrolGuts" && $this->mNewRev->getTitle()->getNamespace() == NS_MAIN) {
				$oldId = $this->mNewRev->getPrevious();
				$oldId = $oldId ? $oldId->getId() : -1;
				// Only show thumbs up for diffs that look back one revision
				if ($oldId == -1 || $oldId == $this->mOldRev->getId())  {
					$params = array ('title' => $this->mNewRev->getTitle(), 'new' => $this->mNewid, 'old' => $oldId, 'vandal' => 0);
					$thumbsHtml = RCPatrol::getThumbsUpButton($params, true);
					$thumbHeader = 'class="th_diff_h4"';
					$th_diff_div = 'class="th_diff_div"';
				}
		}
		$newHeader = '<table cellpadding="0" cellspacing="0" style="margin:0 auto;"><tr><td colspan="2" style="text-align:center;"><div id="mw-diff-ntitle1" ' . $th_diff_div . '><h4 ' . $thumbHeader . '>'.$this->mNewtitle . ' ' . $nextlink . "</h4>" . $thumbsHtml . '</div></td></tr>' .
			'<tr><td style="text-align:right; padding-right:5px; vertical-align:top;">' . $av . '</td><td style="text-align:left"><div id="mw-diff-ntitle1a">' . wfMsg('diff_by') . ' ' . $userName . '</div>' .
