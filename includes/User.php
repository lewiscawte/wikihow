...
class User {
	...
	//XXADDED
	var $mNewKudos, $mLoadedFacebook = false, $mIsFacebook;
	...
	/**
	 * XXADDED -- Reuben added for login by email
	 *
	...
	function pingLimiter( $action='edit' ) {
		...
		if( $this->isNewbie() ) {
			...
			if( isset( $limits['ip'] ) ) {
				$keys[wfMemcKey("mediawiki:limiter:$action:ip:$ip")] = $limits['ip'];
			}
			$matches = array();
			if( isset( $limits['subnet'] ) && preg_match( '/^(\d+\.\d+\.\d+)\.\d+$/', $ip, $matches ) ) {
				$subnet = $matches[1];
				$keys[wfMemcKey("mediawiki:limiter:$action:subnet:$subnet")] = $limits['subnet'];
			}
		}
	...
	function setCookies() {
		...
		$_SESSION['wsToken'] = $this->mToken;
		if ( 1 == $this->getOption( 'rememberpassword' ) || $this->isFacebookUser()) {
	...
	function addToDatabase() {
        ...
        // debug lc username problem
        if (preg_match("@^[a-z]@", $this->mName) ) {
            global $wgContLang;
            $dbw = wfGetDB(DB_MASTER);
			$text = "{$this->mName} " 
                    . "\n\n wgContLang: " . print_r($wgContLang, true)
                    . "\n\n _POST: " . print_r($_POST, true)
                    . "\n\n user : " . print_r($this, true)
                    . "\n\n backtrace: " . print_r(debug_backtrace(), true);
            $dbw->insert('stupidlog', array('log' => $text));
        }
	 ...
	 * @public
	 */
	function sendMail( $subject, $body, $from = null, $replyto = null, $isHTMLEmail = true ) {
		if( is_null( $from ) ) {
		...
		$mimeType = $isHTMLEmail ? 'text/html' : 'text/plain';
		return UserMailer::send( $to, $sender, $subject, $body, $replyto, "$mimeType;" );
	}
	 ...
	 */
	static function getEmailCount($addr) {
		$addr = trim($addr);
		if (empty($addr)) {
			return array(0, '');
		}
		...
		$row = $dbr->selectRow(
			...
			array('user_email = ' . $dbr->addQuotes($addr)),
			__METHOD__
		);
		return (array)$row;
	}
	...
	function isFacebookUser() {
		...
		if (!$this->mLoadedFacebook) {
			$this->load();
			$dbr = wfGetDB(DB_SLAVE);
			$count = $dbr->selectField('facebook_connect',
				array('count(*)'),
	...
	function getCatFilter($columnName = 'page_catinfo', $anonTeenFilter = false) {
		$cat = "";
		$anon = $this->getID() == 0;
		if (!$anon && $this->getOption('contentfilter') != 0) {
			$filter = $this->getOption('contentfilter');
			if ($filter == 1) $cat = " AND $columnName & " . CAT_TEEN . " = " . CAT_TEEN;
			if ($filter == 2) $cat = " AND $columnName & " . CAT_TEEN . " = 0 ";
		} elseif ($anonTeenFilter && $anon) {
			$cat = " AND $columnName & " . CAT_TEEN . " = 0 ";
		}
		return $cat;
	}
