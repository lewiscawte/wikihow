...
class Skin extends Linker {
	...
	function initPage( &$out ) {
		...
		$out->addLink( array( 
			...
			//XXCHANGED
			'href' => wfGetPad("$wgScriptPath/opensearch_desc{$wgScriptExtension}"),
	...
	static function makeGlobalVariablesScript( $data ) {
		...
		global $wgVersion, $wgEnableAPI, $wgEnableWriteAPI, $wgFBAppId, $wgFBAppNamespace;
		...
		$vars = array( 
			...
			'wgUserID' => $wgUser->isAnon() ? NULL : $wgUser->getID(),
			...
			'wgForceAdvancedEditor' => $forceAdvanced,
			'wgWikihowSiteRev' => WH_SITEREV,
			'wgFBAppId' => $wgFBAppId,
			'wgFBAppNamespace' => $wgFBAppNamespace,
		);
...
END;
	...
	function drawCategoryBrowser($tree, &$skin) {
		...
		foreach ($tree as $element => $parent) {
			if (empty($parent)) {
				...
				$return .= Skin::drawCategoryBrowser($parent, $skin) . ' <li> / </li> ';
			}
	...
	function editUrlOptions() {
		...
		$useDefault = false;
		wfRunHooks('EditURLOptions', array(&$useDefault));
		if( !$useDefault && $this->mRevisionId && !$wgArticle->isCurrent() ) {
			$editOptions = "action=edit&oldid=" . intval( $this->mRevisionId );
		} else {
