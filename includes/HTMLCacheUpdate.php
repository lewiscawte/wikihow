...
class HTMLCacheUpdate
{
	...
	function doUpdate() {
		...
		$this->mResult = $res;
		wfDebug("HTMLCacheUpdate::doUpdate {$this->mTable} table {$dbr->numRows( $res )} rows {$this->mRowsPerJob} max \n");
		if ( $dbr->numRows( $res ) != 0 ) {
			// redirects can stay cached for awhile, don't make everyone sit around waiting, sheesh
			if ( $this->mTable == "redirect" || $dbr->numRows( $res ) > $this->mRowsPerJob ) {
				 wfDebug("HTMLCacheUpdate::doUpdate inserting jobs \n");
				$this->insertJobs( $res );
			} else {
				 wfDebug("HTMLCacheUpdate::doUpdate invaliding ids \n");
