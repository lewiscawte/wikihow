...
'<head><title>test</title></head><body>'.$text.'</body></html>';
	...
	function replaceInternalLinks( $s ) {
		...
		for ($k = 0; isset( $a[$k] ); $k++) {
			...
			if( $ns == NS_MEDIA ) {
				...
				//taking out of hooks because that's causing errors
				//wfRunHooks( 'ParserGetArticleWidget', array( &$this, &$nt, &$ret ) );
				$ret = '<iframe src="'.wfGetPad('/Special:ArticleWidgets/BMI').'" scrolling="no" frameborder="0" class="article_widget" allowTransparency="true"></iframe>';
				$ret = '<div class="widget_br"></div>'.$ret.'<div class="widget_br"></div>';
				# Cloak with NOPARSE to avoid replacement in replaceExternalLinks
	...
	function formatHeadings( $text, $isMain=true ) {
		...
		$enoughToc = $this->mShowToc &&
			(($numMatches >= 7) || $this->mForceTocPosition);
		...
		//XXXADDED
            ...
            /// XXCHANGED
			/*
            if ($wgTitle && !is_a($wgTitle, "FakeTitle") && MWNamespace::isTalk($wgTitle->getNamespace())) {
                ...
                $s = getDivIdForAnchor($sectionName); 
               	if (strpos($head[$headlineCount], "<h2>") !== false && strpos($head[$headlineCount], wfMsg('steps')) !== false)
               		@$head[$headlineCount] = "{$head[$headlineCount]}\n<div id=\"$s\">";
               	else if (strpos($head[$headlineCount], "<h2>") !== false)
               		#@$head[$headlineCount] = "\n<div id=\"$s\">" .$head[$headlineCount];
               		@$head[$headlineCount] = "{$head[$headlineCount]}\n<div id=\"$s\" class='article_inner'>";
               	else
...
function getDivIdForAnchor ($anchor) {
  ...
  switch ($anchor)  {
      ...
      case wfMsg('video') : $result = 'video'; break;
  }
  return $result;
}
