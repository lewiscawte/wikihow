	...
	*/
	...
	function doCategoryQuery() {
		...
		*/
		...
		$t_arr = split("\n", $templates);
		$t_str = "";
		foreach ($t_arr as $tplat) {
			$t_str .= $dbr->addQuotes($tplat) . " ";
		}
		$t_str = str_replace(" ", ",", trim($t_str));
		...
		$sql = "SELECT page_title, page_namespace, page_len, cl1.cl_sortkey, page_counter, tl_title , cl2.cl_to as page_is_featured
				...
				LEFT OUTER JOIN templatelinks ON page_id = tl_from and tl_title IN ($t_str)
...
#echo $sql; exit; 
		...
		$sql2 = "SELECT page_title
				FROM (page, categorylinks cl1) LEFT OUTER JOIN skey on (page_title=skey_title AND page_namespace=skey_namespace)
				LEFT OUTER JOIN templatelinks ON page_id = tl_from and tl_title = ".$dbr->addQuotes($this->title->getDBKey())."
				LEFT OUTER JOIN categorylinks cl2 ON page_id = cl2.cl_from and cl2.cl_to = 'Featured-Articles'
			WHERE
			cl1.cl_from = page_id 
			AND cl1.cl_to = " . $dbr->addQuotes($this->title->getDBKey()) 
			. " GROUP BY page_id " ;
		$res2 = $dbr->query($sql2);
		$this->articlecount = $dbr->numRows ( $res2 ) ;
...
//print_r($randomFAs);
		...
		if (sizeof($articles_with_templates) > 0) {
			...
			if (in_array($this->title->getDBKey(), array_keys($templates))) {
				$r .= "<p>Below are ".count($articles_with_templates)." (of ". $this->articlecount .") articles in this category that require attention.</p>\n";
			} else {
