...
<script type=\"text/javascript\" src=\"{$wgStylePath}/common/upload.js?{$wgStyleVersion}\"></script>
        ...
        // ADDED
        if ($wgRequest->getVal('desc', null) != null) {
            $this->mComment = $wgRequest->getVal('desc');
        }
        $url = '';
        $url_enabled = "DISABLED";
        $c1 = "checked";
        $c2 = "";
        if ($wgRequest->getVal('desturl', null) != null) {
            $url = htmlspecialchars($wgRequest->getVal('desturl', null));
            $url_enabled = ""; 
            $c1 = "";
            $c2 = "checked";
            $wgOut->setArticleBodyOnly(true);
            $wgOut->addHTML('<style type="text/css" media="all">/*<![CDATA[*/ @import "/skins/WikiHow/newskin.css"; /*]]>*/</style>');
        }
        $destname = '';
        if ($wgRequest->getVal('destname', null) != null) {
            $encDestFile  = htmlspecialchars($wgRequest->getVal('destname'));
        }
		...
		if( $wgAllowCopyUploads && $wgUser->isAllowed( 'upload_by_url' ) ) {
			$filename_form =
				"<input type='radio' id='wpSourceTypeFile' name='wpSourceType' value='file' " .
				   "onchange='toggle_element_activation(\"wpUploadFileURL\",\"wpUploadFile\")' {$c1} />" .
				...
				"<input type='radio' id='wpSourceTypeURL' name='wpSourceType' value='web' " .
				  "onchange='toggle_element_activation(\"wpUploadFile\",\"wpUploadFileURL\")' {$c2} />" .
				...
				($this->mDesiredDestName?"":"onchange='fillDestFilename(\"wpUploadFileURL\")' ") . "size='40' {$url_enabled} value='{$url}'/>" .
		...
		<tr>
			...
			<td align='$align2'>
				<textarea tabindex='3' name='wpUploadDescription' id='wpUploadDescription' rows='6' 
					cols='50'{$width}>$encComment</textarea>
	   {$this->uploadFormTextAfterSummary}
			</td>
		</tr>
		<tr>
EOT
	...
	<tr>
		<td></td>
		<td align='$align2'><input id='gatWPUpload' tabindex='9' type='submit' name='wpUpload' value=\"{$ulb}\"" . $wgUser->getSkin()->tooltipAndAccesskey( 'upload' ) . " /></td>
	</tr>
	<tr>
		<td></td>
		<td align='$align2'>
