...
abstract class MediaTransformOutput {
	...
	function getDescLinkAttribs( $alt = false ) {
		...
		if ($wgTitle && $wgTitle->getNamespace() == NS_CATEGORY) {
			$opts['href']   = $this->file->getTitle()->getLocalURL( $query );
		} else {
			//$opts['onclick']='return loadimg("' . $this->file->getURL(). '", "' . $this->file->title->getFullURL(). '", ' . $this->file->height . ', ' . $this->file->width . ' );';
			$opts['href']   = $this->file->getTitle()->getLocalURL( $query );
		}
		return $opts;
	}
...
class ThumbnailImage extends MediaTransformOutput {
	...
	function toHtml( $options = array() ) {
		...
		/// XXCHANGED added PAD
		$pad = wfGetPad($this->url);
		wfRunHooks("GeneratingUrl", array($this->url, $pad));
		$attribs = array(
			'alt' => $alt,
			'src' => $pad,
			'width' => $this->width,
			'height' => $this->height,
			'border' => 0,
			'class' => 'mwimage101', // used for jquery
		);
