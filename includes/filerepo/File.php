...
abstract class File {
	...
	*/
	...
	//XXCHANGED FOR GALLERY IMAGES
	public function getThumbnail( $width, $height=-1, $render = true, $crop = false ) {
		$params = array( 'width' => $width );
		if ( $height != -1 ) {
			$params['height'] = $height;
		}
	...
	function transform( $params, $flags = 0 ) {
		...
		do {
			...
			$thumbPath = $this->getThumbPath( $thumbName, $params );
			$thumbUrl = $this->getThumbUrl( $thumbName, $params );
			...
			if ( $wgUseSquid && $thumb->url == $thumbUrl) {
	...
	function getThumbRel( $suffix = false, $params = array()) {
		$path = 'thumb/';
		$path .= $this->getRel();
		if ( $suffix !== false ) {
			$path .= '/' . $suffix;
	...
	function getThumbPath( $suffix = false, $params = array() ) {
		$path = $this->repo->getZonePath('public') . '/' . $this->getThumbRel( $suffix );
        if ($params['crop'] == 1){
            $last = strrpos($path, "/");
            if ($last !== false) 
                $path = substr($path, 0, $last+1) . "-crop-{$params['width']}-{$params['height']}-" . substr($path, $last+1, strlen($path));
        }  
		return $path;
	}
	...
	function getThumbUrl( $suffix = false, $params = array() ) {
