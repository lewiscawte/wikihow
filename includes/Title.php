...
#----------------------------------------------------------------------------
	...
	private function secureAndSplit() {
		...
		if ( ':' == $dbkey{0} ) {
			...
			$dbkey = trim( $dbkey, '-' ); # remove any subsequent whitespace
		}
	...
	function getCategoryMask() {
		global $wgCategoryNames;
		if ($this->getNamespace() != NS_MAIN) return 0;

		$topcats = array_flip($wgCategoryNames);
    	$filter = preg_split("@\n@", wfMsg('teen_title_filter_keywords'), -1, PREG_SPLIT_NO_EMPTY);
    	$filter_re = "@\b" . implode($filter, "\b|\b") . "\b@i";
    	$catfilter = preg_split("@\n@", wfMsg('teen_title_filter_categories'), -1, PREG_SPLIT_NO_EMPTY);

       	$val = 0;
        // do we have a category match to trigger the teen filter? 
        $top = $this->getParentCategoryTree();
        $flat = wfFlattenArrayCategoryKeys($top);
        $clean = array();
        foreach ($flat as $f) {
            $f = preg_replace("@^Category:@", "", $f);
            $x = Title::makeTitle(NS_CATEGORY, $f);
            $clean[] = $x->getText();
        }
        if (sizeof(array_intersect($clean, $catfilter)) > 0) {
            $val = $val | CAT_TEEN;
        }

        // set the usual category params
        $top = $this->getTopLevelCategories();
        foreach ($top as $c) {
            $val = $val | $topcats[$c->getText()];
        }

        // do we have a keyword trigger?
        if (preg_match($filter_re, $this->getText())) {
            $val = $val | CAT_TEEN;
        }
		return $val;
	}
