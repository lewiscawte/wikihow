	...
	private function showIntro() {
		...
		if( !$this->showCustomIntro() && !$this->mTitle->exists() ) {
			if( $wgUser->isLoggedIn() ) {
				$wgOut->addHTML( '<div class="mw-newarticletext">' . wfMsg('newarticletext') . '</div>');
			} else {
				$wgOut->addHTML( '<div class="mw-newarticletext">' . wfMsg('newarticletextanon') . '</div>');
			}
	...
	function showEditForm( $formCallback=null ) {
		...
		if (WikiHow::articleIsWikiHow($this->mArticle) || ($this->mTitle->getArticleID() == 0 && $this->mTitle->getNamespace() == NS_MAIN) ) {
			if (class_exists('Html5editor') && isHtml5Editable(true)) {
				...
				$guidedHTML = $sk->makeKnownLinkObj($this->mTitle, wfMsg('html5_editor'), $relURL, '','','id="othereditlink"') . wfMsg('h5e_save_first');
			} else {
				$guidedHTML = $sk->makeKnownLinkObj($this->mTitle, wfMsg('guided_editing'), 'action=edit&override=yes', '','','id="othereditlink"');
			}
		}
		...
		$jsUrl = wfGetPad('/extensions/min/f/skins/common/clientscript.js?') . WH_SITEREV;
...
" );
	...
	function doLivePreviewScript() {
		...
		$wgOut->addHTML( '<script type="'.$wgJsMimeType.'" src="' .
			wfGetPad('/extensions/min/f/skins/common/preview.js?rev=') . WH_SITEREV .
			'"></script>' . "\n" );
	...
	function getPreviewText() {
		...
		if ( $this->isCssJsSubpage ) {
			...
			#$previewHTML = "<div class='article_inner'>" . WikiHowTemplate::mungeSteps($parserOutput->getText()) . "</div>";
			$previewHTML = WikiHowTemplate::mungeSteps($parserOutput->getText());
	...
	function spamPage( $match = false ) {
		...
		if ( $match ) {
			...
			$wgOut->addHTML( wfMsg('spamprotectionreturnto'));
		}
		$wgOut->addHtml( '</div>' );
	}
	...
	function getEditToolbar() {
		...
		$toolbar = "<div id='toolbar' style='float: left;'>\n";
		$toolbar.="<script type='$wgJsMimeType'>\n/*<![CDATA[*/\n";

		foreach($toolarray as $tool) {
			// XXCHANGED -- use CDN
			$params = array(
				$image = wfGetPad($wgStylePath.'/common/images/'.$tool['image']),
		...
		$toolbar.="\n</div>";
		$toolbar .= PopBox::getToolbarButton() . ($wgLanguadeCode == 'en' ?  wfGetFixCapsButton() : "");

		//XXADDED
		wfRunHooks( 'EditPageToolbarPost', array( &$toolbar, NULL ) );
	...
	public function getEditButtons(&$tabindex) {
		...
		$temp = array(
           ...
           'onclick'   => 'needToConfirm = false',
            'class'         => 'button blue_button_100 submit_button',
		...
		$temp = array(
           ...
           //XXCHANGED
            	...
            	'onmouseout'    => 'button_unswap(this);',
            	'onmouseover'   => 'button_swap(this);',
            	'style'         => 'background-position: 0pt 0pt;',
		);
