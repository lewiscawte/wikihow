	...
	function addNewAccount() {
		...
		if( $wgUser->isAnon() ) {
			...
			if (!isset($_COOKIE[$wgCookiePrefix . 'acctType'])) {
				...
				setcookie( $wgCookiePrefix.'acctType', $u->getID() . "|" . $cohortType, $cookieExp, $wgCookiePath, $wgCookieDomain, $wgCookieSecure );
			}
...
"); 
	...
	function mainLoginForm( $msg, $msgtype = 'error' ) {
        ...
        // clear the name if the user is a facebook user
        if (FBConnect::userNameIsFacebookUser($this->mName))
            $this->mName = null;
