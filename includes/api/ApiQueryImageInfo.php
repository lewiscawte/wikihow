...
class ApiQueryImageInfo extends ApiQueryBase {
	...
	protected function getInfo($f) {
		...
		if($this->fld_url) {
			if($this->scale && !$f->isOld()) {
				...
				if($thumb)
				{
					# XXCHANGED for ForeignAPIRepo
					$vals['thumburl'] = $wgServer . $thumb->getURL();
					$vals['thumbwidth'] = $thumb->getWidth();
					$vals['thumbheight'] = $thumb->getHeight();
				}
			}
			# XXCHANGED for ForeignAPIRepo
			$vals['url'] = $wgServer . $f->getURL();
		}
		if($this->fld_comment) 
	...
	public function getVersion() {
		return __CLASS__ . ': $Id: ApiQueryImageInfo.php,v 1.2 2009/11/25 19:57:34 cvsuser Exp $';
	}
}
