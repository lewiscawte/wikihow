...
class TwitterAuthModel {
	...
	public function getUserTwitterIdByWHUserId( $whUserId ) {
		...
		$joins = array();
		$joins[self::OAUTH_TABLE] = array(
			'LEFT JOIN' => self::OAUTH_TABLE . '.twitter_user_id = ' .
				self::COOKIE_TABLE . '.twitter_user_id'
		);

		try {
			$twitterUserId = $this->dbr->selectField(
				self::COOKIE_TABLE, $fields, $where, __METHOD__, null, $joins
			);
		} catch ( Exception $e ) {
			echo $e->getMessage();
		}
		...
		if( !empty( $whUserId ) && $whUserid > 0 ) {
			...
			try {
				$results = $this->dbr->query( $sql, __METHOD__ );
	...
	public function generateTwitterCookie( $twitterUserId ) {
		...
		// TODO: is there some other unique ID generator?
